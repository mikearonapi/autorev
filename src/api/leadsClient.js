 ...
 * Leads API Client
 * 
 * Provides functions to manage lead capture for SuperNatural Motorsports.
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        ...
        return await updateExistingLead(email, leadRecord);
      }
...
/**
 * Update an existing lead with new interaction data
 * @param {string} email - The email to update
 * @param {Object} newData - New data to merge
 * @returns {Promise<{success: boolean, data?: Object, error?: string}>}
 */
async function updateExistingLead(email, newData) {
  if (!isSupabaseConfigured || !supabase) {
    return { success: true, data: { message: 'Lead updated locally' } };
  }
  ...
  try {
    ...
    if (fetchError) {
      console.error('[leadsClient] Error fetching existing lead:', fetchError);
    ...
    const mergedMetadata = {
      ...
      ...newData.metadata,
      interaction_history: [
        ...(existingLead.metadata?.interaction_history || []),
        {
          source: newData.source,
          timestamp: new Date().toISOString(),
          car_interest: newData.car_interest_slug,
        },
      ],
    };
    ...
    const { data, error } = await supabase
      .from('leads')
      .update({
        name: newData.name || existingLead.name,
    ...
    if (error) {
      console.error('[leadsClient] Error updating lead:', error);
      return { success: false, error: 'Failed to update lead' };
    }

    console.log('[leadsClient] Lead updated successfully:', data.id);
    return { success: true, data };
  } catch (err) {
    console.error('[leadsClient] Unexpected error updating lead:', err);
