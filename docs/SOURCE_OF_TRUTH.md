...
.button { padding: 8px; }
@media (min-width: 768px) { .button { padding: 16px; } }

/* ❌ WRONG - Desktop-first (max-width) */
.button { padding: 16px; }
@media (max-width: 767px) { .button { padding: 8px; } }
```
...
5. [Service Files (lib/)](#service-files)
...
10. [Build & Vehicle Data Model](#build--vehicle-data-model)
...
.text { color: var(--color-text-secondary, #94a3b8); }
```
...
if (enabled) { /* render new checkout */ }
...
Garage sub-pages (my-specs, my-build, my-performance, my-parts, my-photos, my-install) use a fallback pattern:
