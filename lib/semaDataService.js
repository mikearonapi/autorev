...
export async function mapVcdbToAutoCars(vehicles) {
  ...
  for (const vehicle of vehicles) {
    ...
    if (carId) {
      ...
      const { error } = await supabaseServiceRole.from('vcdb_vehicles').upsert(
        {
...
async function findMatchingCar({ year, make, model, subModel }) {
  ...
  if (exactMatch && exactMatch.length > 0) {
    // Filter by year - handle new schema (year integer) or old (years string)
    for (const car of exactMatch) {
      if (car.year && year === car.year) {
        return car.id;
      } else if (car.years && isYearInRange(year, car.years)) {
        return car.id;
      }
    }
  }
 ...
 * @private
 */
function parseACESXml(_xmlString) {
...
export async function ingestACESFitments(fitments, options = {}) {
  ...
  const { brandName: _brandName = null, sourceUrl = null, dryRun = false } = options;
  ...
  for (const fitment of fitments) {
    ...
    try {
      ...
      if (existing) {
        await supabaseServiceRole.from('part_fitments').update(fitmentRow).eq('id', existing.id);
        results.updated++;
      } else {
        await supabaseServiceRole.from('part_fitments').insert(fitmentRow);
        results.created++;
      }
    } catch (err) {
      results.errors.push({
 ...
 */
export async function runFullSync(_options = {}) {
  ...
  return {
    success: false,
    message:
      'SEMA Data Co-Op sync requires API credentials. Please configure SEMA_API_KEY and SEMA_API_SECRET.',
...
export const DATA_CONFIDENCE_TIERS = {
  ...
  shopify_vendor: { minConfidence: 0.6, label: 'extracted', source: 'Vendor Feed' },

  // Community sources (forums, YouTube, Reddit)
  community: { minConfidence: 0.5, label: 'community', source: 'Community' },
...
export function getConfidenceTier(confidence) {
  ...
  if (confidence >= 0.6) return DATA_CONFIDENCE_TIERS.shopify_vendor;
...
export default semaDataService;
