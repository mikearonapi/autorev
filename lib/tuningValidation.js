...
import { identifyEnginePlatform, validateStockWhp } from './enginePlatforms.js';
...
export function validateTuningData(car, profileData) {
  ...
  if (profileData.engine_family && identifiedPlatform) {
    ...
    if (
      !normalizedProfile.includes(normalizedExpected) &&
      !normalizedExpected.includes(normalizedProfile)
    ) {
      ...
      if (profileDisplacement !== expectedDisplacement) {
        errors.push(
          `Engine family mismatch: Profile says "${profileData.engine_family}" but car has "${car.engineType}" ` +
  ...
  if (profileData.stock_whp && car.hp) {
    ...
    if (identifiedPlatform) {
      const platformValidation = validateStockWhp(
        identifiedPlatform.platform,
        profileData.stock_whp
      );
  ...
  if (profileData.tuning_platforms && profileData.tuning_platforms.length > 0) {
    ...
    if (appropriatePlatforms.length > 0) {
      ...
      const hasAppropriate = profilePlatformNames.some((pName) =>
        appropriatePlatforms.some(
          (ap) => ap.toLowerCase() === pName || pName.includes(ap.toLowerCase())
        )
      );
...
export function generateValidatedProfile(car) {
  ...
  if (!platform) {
    return {
      success: false,
      error: `Could not identify engine platform for ${car.name} (${car.engineType})`,
      profile: null,
      validation: null,
    };
  }
