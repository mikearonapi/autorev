 ...
 */
async function doWebResearch(carInfo, upgradeType, budgetMin, budgetMax) {
  if (!EXA_API_KEY) {
  ...
  // 4 parallel searches - cast a wide net
  const searches = [
    // 1. General "best" search - reviews and recommendations
    {
      query: `best ${upgradeType} for ${carContext} 2024 2025`,
      num_results: 15,
      label: 'best_recommendations',
    },
    // 2. Comparison/review search - forum discussions
    {
      query: `${carContext} ${upgradeType} comparison review reddit forum`,
      num_results: 12,
      label: 'forum_discussions',
    },
    // 3. Shopping search - find actual products with prices
    {
      query: `${carContext} ${upgradeType} buy price${budgetHint}`,
      num_results: 12,
      label: 'shopping_results',
    },
    // 4. Specific product search
    {
      query: `${upgradeType} ${carInfo.model || carInfo.make} compatible fits`,
      num_results: 10,
    ...
    const searchPromises = searches.map((s) =>
      fetch('https://api.exa.ai/search', {
        ...
        body: JSON.stringify({
          ...
          use_autoprompt: true,
          type: 'auto',
          contents: { text: { max_characters: 1200 } }, // More text for AI to analyze
        }),
      })
        .then((r) => r.json())
        .then((r) => ({
 ...
 */
export async function persistResearchResults(carId, extractedParts) {
  ...
  for (const part of extractedParts) {
    try {
      ...
      // 1. Upsert to parts table
      const { data: partRecord, error: partError } = await client
        .from('parts')
        .upsert(
          {
      ...
      let finalPartRecord = partRecord;
      if (partError) {
        // Try to find existing part instead
        const { data: existing } = await client
          .from('parts')
          .select('id')
          .eq('brand_name', part.brand_name)
          .ilike('name', `%${part.product_name.slice(0, 30)}%`)
          .limit(1)
          .single();

        if (!existing) {
          errors.push({ part: part.product_name, error: partError.message });
          continue;
        }
        finalPartRecord = existing;
      }

      const partId = finalPartRecord?.id;
      if (!partId) continue;

      // 2. Upsert to part_fitments table
      ...
      // 3. Insert pricing snapshot if price available
      ...
      saved.push({ part_id: partId, name: part.product_name });
    } catch (err) {
...
export async function researchPartsLive({
  ...
  console.log(`[researchPartsLive] Researching ${upgrade_type} for: ${carContext}`);
  ...
  try {
    ...
    // =========================================================================
    // STEP 3: Do wide-net web research (no domain restrictions)
    // =========================================================================
    const webResearch = await doWebResearch(carInfo, upgrade_type, budget_min, budget_max);

    const researchTime = Date.now() - startTime;
    console.log(`[researchPartsLive] Research completed in ${researchTime}ms`);

    // =========================================================================
    // STEP 4: Return results for AI to analyze
    // =========================================================================
    return {
      success: true,
...
3. CREATE your top 5 recommendations with reasoning
   - Consider brand reputation, user reviews, value
   - ALWAYS include purchase URLs from the research

4. FORMAT your response for the user with clickable purchase links

5. At the END of your response, include a JSON block for database persistence:
...
This data will be saved to our database for future queries.
      `.trim(),
    };
  } catch (err) {
