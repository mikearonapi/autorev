...
export async function searchKnowledge({ query, car_slug = null, topic = null, limit = 6 }) {
  ...
  try {
    ...
    if (embedErr) {
      console.warn('[AL Tools] searchKnowledge: Embedding failed, using keyword-only search:', embedErr);
      
      const { data: keywordMatches, error: kwErr } = await client.rpc('search_document_chunks_keyword', {
        p_query: query,
        p_car_id: carId,
        p_topic: topic || null,
        p_limit: safeLimit,
      ...
      let results = Array.isArray(keywordMatches) ? keywordMatches : [];
    ...
    if (vectorMatches.length === 0 && keywordMatches.length === 0) {
      return {
        ...
        count: 0,
        results: [],
        instruction: 'No matching documents found. Try rephrasing the query or using different terms.',
      };
    }
    ...
    if (vectorMatches.length === 0) {
      ...
      return formatKnowledgeResults(query, car_slug, results.slice(0, safeLimit), 'keyword_only', null);
    }
    ...
    if (keywordMatches.length === 0) {
      ...
      return formatKnowledgeResults(query, car_slug, results.slice(0, safeLimit), 'vector_only', model);
    }
...
export async function getMaintenanceSchedule({ car_slug, car_variant_key = null, mileage = null }) {
  ...
  if (isSupabaseConfigured && supabase) {
    ...
    if (!maintenanceSummary && car?.id) {
      try {
        const MAINT_COLS = 'car_id, oil_type, oil_viscosity, oil_spec, oil_capacity_liters, oil_capacity_quarts, oil_change_interval_miles, oil_change_interval_months, oil_filter_oem_part, coolant_type, coolant_color, coolant_spec, coolant_capacity_liters, coolant_change_interval_miles, coolant_change_interval_years, brake_fluid_type, brake_fluid_spec, brake_fluid_change_interval_miles, brake_fluid_change_interval_years, fuel_type, fuel_octane_minimum, fuel_octane_recommended, trans_fluid_type, trans_fluid_spec, trans_fluid_interval_miles, diff_fluid_type, diff_fluid_spec, diff_fluid_interval_miles, spark_plug_type, spark_plug_gap, spark_plug_interval_miles, tire_pressure_front, tire_pressure_rear, air_filter_interval_miles, cabin_filter_interval_miles';
 ...
 * Returns full part objects ready for one-click acceptance in the UI.
 * 
 * This is designed for the Garage flow where users select an upgrade
 * and want AL to find the best parts to buy.
 * 
 * @param {Object} params
 * @param {string} params.car_slug - Required: which car to find parts for
 * @param {string} params.upgrade_type - Upgrade category (e.g., 'downpipe', 'intake', 'exhaust')
 * @param {string} [params.budget] - Budget range: 'value', 'mid', 'premium'
 * @param {number} [params.limit] - Max results (default 5, max 10)
 * @returns {Object} Parts ready for one-click acceptance
 */
export async function findBestParts({ car_slug, upgrade_type, budget = 'mid', limit = 5 }) {
  const client = getDbClient({ requireServiceRole: true });
  if (!isSupabaseConfigured || !client) return { error: 'Supabase not configured' };
  if (!car_slug) return { error: 'car_slug is required to find fitting parts' };
  if (!upgrade_type) return { error: 'upgrade_type is required (e.g., downpipe, intake, exhaust)' };

  const safeLimit = Math.max(1, Math.min(Number(limit) || 5, 10));

  try {
    // Resolve car_slug to car_id for indexed queries
    const carId = await resolveCarId(car_slug);
    if (!carId) {
      return { error: `Car not found: ${car_slug}` };
    }
    ...
    const { data: fitmentData, error: fitmentErr } = await client
      ...
      .limit(safeLimit * 3); // Get more to filter/sort

    if (fitmentErr) {
      console.error('[AL Tools] findBestParts fitment query error:', fitmentErr);
    ...
    if (!fitmentData || fitmentData.length === 0) {
      return {
        ...
        count: 0,
        parts: [],
        message: `No verified parts found for ${upgrade_type} on this vehicle. Try searching with search_parts for more options.`,
      };
    }

    // Get latest pricing for all found parts
    const partIds = [...new Set(fitmentData.map(f => f.parts?.id).filter(Boolean))];
    const { data: priceData, error: priceErr } = await client
      .from('part_pricing_snapshots')
      .select('part_id, price_cents, currency, vendor_name, product_url, recorded_at')
      .in('part_id', partIds)
      .order('recorded_at', { ascending: false })
      .limit(partIds.length * 5); // Multiple vendors per part

    // Build price map (latest per part)
    const priceMap = new Map();
    const allPricesMap = new Map(); // Track all prices for "best price" detection
    for (const p of priceData || []) {
      if (!priceMap.has(p.part_id)) {
        priceMap.set(p.part_id, p);
      }
      if (!allPricesMap.has(p.part_id)) {
        allPricesMap.set(p.part_id, []);
      }
      allPricesMap.get(p.part_id).push(p);
    }
    ...
    const scoredParts = fitmentData
      ...
      .map(f => {
        ...
        const allPrices = allPricesMap.get(part.id) || [];
        const lowestPrice = allPrices.length > 0 
          ? Math.min(...allPrices.map(p => p.price_cents))
          : null;
        const isBestPrice = price && lowestPrice && price.price_cents === lowestPrice;
    ...
    // Transform to the one-click acceptance format
    const results = scoredParts.map(({ fitment, part, price, isBestPrice }) => ({
      ...
      // Pricing (for one-click acceptance)
      price_cents: price?.price_cents || null,
      currency: price?.currency || 'USD',
      vendor_name: price?.vendor_name || null,
      product_url: price?.product_url || null,
      
      // Fitment data
      fitment_verified: fitment.verified || false,
      fitment_confidence: fitment.confidence || 'low',
      fitment_notes: fitment.fitment_notes || null,
      requires_tune: fitment.requires_tune || false,
      install_difficulty: fitment.install_difficulty || null,
      estimated_labor_hours: fitment.estimated_labor_hours || null,
      ...
      source_url: fitment.source_url || (Array.isArray(part.source_urls) ? part.source_urls[0] : null),
    }));
    ...
    return {
      ...
      count: results.length,
      parts: results,
      instruction: 'These parts are ready for one-click acceptance. The user can click "Add to Build" to add any part to their shopping list with all data pre-filled.',
    };
  } catch (err) {
    console.error('[AL Tools] findBestParts failed:', err);
    return { error: err.message || 'findBestParts failed' };
  }
}
  ...
  try {
    ...
    if (attachment_id) {
      const { data: attachment, error: attachError } = await db
        .from('al_attachments')
        .select('id, public_url, file_type, file_name, file_size, user_id, conversation_id, created_at')
        .eq('id', attachment_id)
        .single();
...
function extractSourcesFromToolResult(toolName, result) {
  ...
  if (toolName === 'search_cars' && result.cars?.length > 0) {
    ...
    const carNames = result.cars.slice(0, 2).map(c => c.name).filter(Boolean);
    // Detail shows what was found (for transparency)
    const detail = carNames.length > 0 
  ...
  if ((toolName === 'get_car_ai_context' || toolName === 'get_car_details') && result.car) {
    const carName = result.car.name || result.car.slug?.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    sources.push({
      id: sourceId++,
      type: 'database',
      label: 'AutoRev Database',
  ...
  if (result.source === 'db_fts' || result.source === 'car_issues' || result.source === 'local_data') {
    sources.push({
      id: sourceId++,
      type: 'database',
      label: 'AutoRev Database',
  ...
  // Community insights / forum search
  if ((toolName === 'search_community_insights' || toolName === 'search_forums') && result.insights) {
...
export async function getUserBuilds({ vehicle_id = null, include_parts = false, user_id = null }) {
  ...
  const PROJECT_COLUMNS = 'id, user_id, car_id, car_slug, car_name, project_name, selected_upgrades, total_hp_gain, total_cost_low, total_cost_high, final_hp, stock_hp, notes, is_favorite, created_at, updated_at';
  ...
  try {
    ...
    if (vehicle_id) {
      ...
      if (vehicle?.matched_car_id) {
        ...
        if (resolvedCarId) {
          ...
          console.warn('[AL Tools] Could not resolve car_id for vehicle slug:', vehicle.matched_car_slug);
        }
      }
    }
    ...
    if (include_parts && projects.length > 0) {
      ...
      const { data: parts } = await client
        ...
        .select('project_id, part_id, quantity, notes, parts(name, brand, category, price_low, price_high)')
      ...
      if (parts) {
        parts.forEach(p => {
          ...
          projectParts[p.project_id].push({
            ...
            price_range: p.parts ? `$${p.parts.price_low || '?'}-$${p.parts.price_high || '?'}` : null,
            quantity: p.quantity,
            notes: p.notes,
          });
        });
    ...
    const builds = projects.map(p => ({
      ...
      cost_range: p.total_cost_low ? `$${p.total_cost_low.toLocaleString()}-$${p.total_cost_high?.toLocaleString() || '?'}` : null,
 ...
 */
export async function getUserPerformanceGoals({ vehicle_id = null, status = 'active', user_id = null }) {
  const client = getDbClient({ requireServiceRole: true });
  if (!isSupabaseConfigured || !client) {
    return { error: 'Supabase not configured' };
  }
  ...
  const GOAL_COLUMNS = 'id, user_id, user_vehicle_id, goal_type, target_value, target_unit, current_value, status, priority, notes, target_date, achieved_at, created_at, updated_at';
  ...
  try {
    ...
    const formattedGoals = goals.map(g => ({
      ...
      progress_percent: g.target_value && g.achieved_value 
 ...
 */
export async function getUserVehicleDetails({ vehicle_id = null, include_service_history = true, user_id = null }) {
  const client = getDbClient({ requireServiceRole: true });
  if (!isSupabaseConfigured || !client) {
    return { error: 'Supabase not configured' };
  }
  ...
  try {
    ...
    if (include_service_history) {
      const { data: logs } = await client
        ...
        .select('service_date, service_type, service_description, odometer_reading, total_cost, performed_by, shop_name')
