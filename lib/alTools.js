...
export async function searchCars({ query, filters = {}, sort_by = 'value', limit = 5 }) {
  ...
  if (filters.brand) {
    ...
    results = results.filter(
      (c) =>
        c.brand?.toLowerCase().includes(brandLower) || c.name?.toLowerCase().includes(brandLower)
    );
  }
 ...
 */
function formatKnowledgeResults(query, car_slug, results, searchMethod, embeddingModel) {
  return {
    query,
    car_slug,
    embeddingModel,
 ...
 *
 * @param {Object} params
 * @param {string} params.car_slug - Car identifier (required)
 * @param {string} [params.period] - Time period: '1y', '3y', '5y', 'all' (default: '3y')
 * @returns {Object} Price history with trends
 */
export async function getPriceHistory({ car_slug, period = '3y' }) {
  const client = getDbClient({ requireServiceRole: true });
  if (!isSupabaseConfigured || !client) return { error: 'Supabase not configured' };
  if (!car_slug) return { error: 'car_slug is required' };

  try {
    // Resolve car_id for indexed queries
    ...
    return { error: err.message || 'get_price_history failed', car_slug };
  }
}
    ...
    try {
      ...
      if (!carId) {
        ...
        return {
          ...
          issueCount: 0,
          message: 'Could not resolve car for issues lookup.',
        };
      }
...
export async function compareCars({ car_slugs, focus_areas = [] }) {
  ...
  const allCars = await fetchCars();
  const cars = car_slugs.map((slug) => allCars.find((c) => c.slug === slug)).filter(Boolean);
 ...
 */
export async function calculateModImpact({ car_slug, mods }) {
  if (!car_slug) {
    return { error: 'car_slug is required' };
  }
  ...
  try {
    ...
    return { error: err.message || 'Calculation failed', car_slug, mods };
  }
}
...
// =============================================================================
// LIVE PARTS RESEARCH - Multi-Phase Investigation System
// =============================================================================

/**
 * Brand reputation database - used for scoring and recommendations
 * Scores are 1-10: 10 = industry leader, 7-9 = well-respected, 5-6 = decent, <5 = budget/unknown
 */
const BRAND_REPUTATION = {
  ...
  'S&B': {
    ...
    tier: 'premium',
    specialty: 'intake',
    notes: 'Industry leader for truck/diesel intakes',
  },
  'K&N': {
    score: 8,
    tier: 'mid',
    specialty: 'intake',
    notes: 'Most recognized intake brand, wide fitment',
  },
  aFe: {
    score: 9,
    tier: 'premium',
    specialty: 'intake',
    notes: 'Premium quality, excellent filtration',
  },
  AFE: {
    score: 9,
    tier: 'premium',
  ...
  Volant: { score: 7, tier: 'mid', specialty: 'intake', notes: 'Good value closed-box systems' },
  Injen: { score: 7, tier: 'mid', specialty: 'intake', notes: 'Popular import tuner brand' },
  Spectre: {
    score: 5,
    tier: 'budget',
    specialty: 'intake',
    notes: 'Budget option, basic performance',
  },
  AEM: { score: 7, tier: 'mid', specialty: 'intake', notes: 'Good quality, import focused' },
  Banks: {
    score: 9,
    tier: 'premium',
    specialty: 'intake',
    notes: 'Diesel/truck specialist, excellent engineering',
  },
  STM: {
    score: 8,
    tier: 'premium',
    specialty: 'intake',
    notes: 'Enthusiast-grade, max flow designs',
  },
  ...
  Borla: {
    ...
    tier: 'premium',
    specialty: 'exhaust',
    notes: 'Premium sound quality, lifetime warranty',
  },
  MagnaFlow: {
    score: 8,
    tier: 'mid',
    specialty: 'exhaust',
    notes: 'Great value, consistent quality',
  },
  Flowmaster: {
    score: 7,
    tier: 'mid',
    specialty: 'exhaust',
    notes: 'Classic American muscle sound',
  },
  MBRP: { score: 7, tier: 'mid', specialty: 'exhaust', notes: 'Good value, especially for trucks' },
  AWE: {
    score: 9,
    tier: 'premium',
    specialty: 'exhaust',
    notes: 'European car specialist, excellent fitment',
  },
  Milltek: { score: 9, tier: 'premium', specialty: 'exhaust', notes: 'UK-made, OEM+ quality' },
  Akrapovic: {
    score: 10,
    tier: 'premium',
    specialty: 'exhaust',
    notes: 'Top-tier, race-derived technology',
  },
  ...
  Bilstein: {
    ...
    notes: 'OEM supplier, excellent quality',
  },
  KW: {
    score: 10,
    tier: 'premium',
    specialty: 'suspension',
    notes: 'German engineering, top coilovers',
  },
  Eibach: { score: 8, tier: 'mid', specialty: 'suspension', notes: 'OEM supplier, great springs' },
  'H&R': { score: 8, tier: 'mid', specialty: 'suspension', notes: 'German quality, good value' },
  'BC Racing': {
    score: 7,
    tier: 'mid',
    specialty: 'suspension',
    notes: 'Popular budget coilovers',
  },
  'Fortune Auto': {
    score: 8,
    tier: 'premium',
    specialty: 'suspension',
    notes: 'Track-focused, US-made',
  },
  Ohlins: { score: 10, tier: 'premium', specialty: 'suspension', notes: 'Race-grade, top tier' },
  ...
  Brembo: {
    ...
    tier: 'premium',
    specialty: 'brakes',
    notes: 'OEM for supercars, industry standard',
  },
  StopTech: { score: 9, tier: 'premium', specialty: 'brakes', notes: 'Excellent big brake kits' },
  EBC: { score: 7, tier: 'mid', specialty: 'brakes', notes: 'Good pads and rotors, wide range' },
  Hawk: { score: 8, tier: 'mid', specialty: 'brakes', notes: 'Excellent pads, track proven' },
  ...
  APR: {
    ...
    tier: 'premium',
    specialty: 'tune',
    notes: 'VAG specialist, conservative reliable tunes',
  },
  Cobb: { score: 9, tier: 'premium', specialty: 'tune', notes: 'Excellent Accessport platform' },
  Unitronic: {
    score: 8,
    tier: 'premium',
    specialty: 'tune',
    notes: 'VAG specialist, aggressive tunes',
  },
  IE: { score: 8, tier: 'premium', specialty: 'tune', notes: 'VAG performance parts and tunes' },
  'Integrated Engineering': {
    score: 8,
    tier: 'premium',
    specialty: 'tune',
    notes: 'VAG performance specialist',
  },
  Diablo: { score: 7, tier: 'mid', specialty: 'tune', notes: 'American muscle specialist' },
  SCT: { score: 7, tier: 'mid', specialty: 'tune', notes: 'Ford specialist' },
  ...
  Mishimoto: {
    ...
    tier: 'mid',
    specialty: 'cooling',
    notes: 'Wide range, good quality radiators/intercoolers',
  },
  CSF: { score: 9, tier: 'premium', specialty: 'cooling', notes: 'Race-grade cooling' },
};

/**
 * Vendor configuration for parts research
 * trustScore: 1-10 (10 = highly trusted retailer)
 */
const PARTS_VENDORS = {
  ...
  summit: {
    ...
    trustScore: 9,
    categories: ['all'],
    notes: 'Largest performance parts retailer, excellent service',
  },
  amazon: {
    name: 'Amazon',
    domains: ['amazon.com'],
    trustScore: 6, // Lower because of counterfeits/third-party sellers
    categories: ['all'],
    notes: 'Convenient but verify seller authenticity',
  },
  realtruck: {
    name: 'RealTruck',
    domains: ['realtruck.com'],
    trustScore: 8,
    categories: ['intake', 'exhaust', 'suspension', 'wheels', 'brakes'],
    notes: 'Truck specialist, good fitment verification',
  },
  carid: {
    name: 'CARiD',
    domains: ['carid.com'],
    trustScore: 7,
    categories: ['all'],
    notes: 'Large selection, decent service',
  },

  // European Specialists
  ecstuning: {
    name: 'ECS Tuning',
    domains: ['ecstuning.com'],
    trustScore: 9,
    categories: ['all'],
    makes: ['volkswagen', 'audi', 'bmw', 'porsche', 'mercedes', 'mini'],
    notes: 'VAG specialist, excellent fitment data',
  },
  fcpeuro: {
    name: 'FCP Euro',
    domains: ['fcpeuro.com'],
    trustScore: 9,
    categories: ['all'],
    makes: ['volkswagen', 'audi', 'bmw', 'porsche', 'mercedes', 'volvo', 'saab'],
    notes: 'Lifetime replacement guarantee, excellent service',
  },
  ...
  jegs: {
    ...
    trustScore: 8,
    categories: ['all'],
    notes: 'Long-standing performance retailer',
  },

  // Manufacturer Direct (highest trust)
  knfilters: {
    name: 'K&N (Direct)',
    domains: ['knfilters.com'],
    trustScore: 10,
    categories: ['intake', 'filters'],
    notes: 'Manufacturer direct - guaranteed authentic',
  },
  afepower: {
    name: 'aFe Power (Direct)',
    domains: ['afepower.com'],
    trustScore: 10,
    categories: ['intake', 'exhaust'],
    notes: 'Manufacturer direct - guaranteed authentic',
  },
  sbfilters: {
    name: 'S&B Filters (Direct)',
    domains: ['sbfilters.com'],
    trustScore: 10,
    categories: ['intake'],
    notes: 'Manufacturer direct - guaranteed authentic',
  },
  ...
  bilstein: {
    name: 'Bilstein (Direct)',
    domains: ['bilstein.com'],
    trustScore: 10,
    categories: ['suspension', 'coilovers', 'shocks'],
    notes: 'Manufacturer direct - guaranteed authentic',
  },
};

/**
 * Map upgrade types to search terms and categories
 */
const UPGRADE_TYPE_MAP = {
  // Intake
  'cold air intake': { terms: ['cold air intake', 'CAI'], category: 'intake' },
  'cold-air-intake': { terms: ['cold air intake', 'CAI'], category: 'intake' },
  intake: { terms: ['cold air intake', 'intake system', 'air intake'], category: 'intake' },

  // Exhaust
  exhaust: {
    terms: ['cat-back exhaust', 'exhaust system', 'performance exhaust'],
    category: 'exhaust',
  },
  'cat-back': { terms: ['cat-back exhaust', 'catback exhaust'], category: 'exhaust' },
  catback: { terms: ['cat-back exhaust', 'catback exhaust'], category: 'exhaust' },
  downpipe: { terms: ['downpipe', 'down pipe', 'turbo downpipe'], category: 'exhaust' },
  headers: { terms: ['headers', 'exhaust headers', 'long tube headers'], category: 'exhaust' },

  // Forced Induction
  turbo: { terms: ['turbo kit', 'turbocharger', 'turbo upgrade'], category: 'forced_induction' },
  supercharger: {
    terms: ['supercharger', 'supercharger kit', 'blower'],
    category: 'forced_induction',
  },
  intercooler: { terms: ['intercooler', 'FMIC', 'front mount intercooler'], category: 'cooling' },

  // Suspension
  coilovers: {
    terms: ['coilovers', 'coilover kit', 'adjustable coilovers'],
    category: 'suspension',
  },
  suspension: {
    terms: ['coilovers', 'lowering springs', 'suspension kit'],
    category: 'suspension',
  },
  'lowering springs': { terms: ['lowering springs', 'sport springs'], category: 'suspension' },
  shocks: { terms: ['shocks', 'struts', 'shock absorbers'], category: 'suspension' },
  sway: { terms: ['sway bar', 'anti-roll bar', 'sway bar kit'], category: 'suspension' },

  // Brakes
  brakes: { terms: ['brake kit', 'big brake kit', 'performance brakes'], category: 'brakes' },
  'big brake kit': { terms: ['big brake kit', 'BBK', 'brake upgrade'], category: 'brakes' },
  'brake pads': { terms: ['brake pads', 'performance brake pads'], category: 'brakes' },
  rotors: { terms: ['brake rotors', 'slotted rotors', 'drilled rotors'], category: 'brakes' },

  // Wheels
  wheels: { terms: ['wheels', 'aftermarket wheels', 'performance wheels'], category: 'wheels' },
  tires: { terms: ['performance tires', 'summer tires', 'track tires'], category: 'wheels' },

  // Tune
  tune: { terms: ['ECU tune', 'tuner', 'flash tune', 'performance tune'], category: 'tune' },
  'ecu tune': { terms: ['ECU tune', 'flash tune', 'performance tuner'], category: 'tune' },
  tuner: { terms: ['tuner', 'flash tuner', 'ECU tuner'], category: 'tune' },
};

/**
 * Select relevant vendors based on car make and part category
 */
function selectVendorsForSearch(carMake, partCategory) {
  const normalizedMake = (carMake || '').toLowerCase();
  const normalizedCategory = (partCategory || 'all').toLowerCase();

  const selectedVendors = [];

  for (const [key, vendor] of Object.entries(PARTS_VENDORS)) {
    // Check if vendor supports this category
    const supportsCategory =
      vendor.categories.includes('all') || vendor.categories.includes(normalizedCategory);

    // Check if vendor is make-specific and matches
    const makeMatch = !vendor.makes || vendor.makes.includes(normalizedMake);

    if (supportsCategory && makeMatch) {
      selectedVendors.push({ key, ...vendor });
    }
  }

  // Sort by trust score (highest first)
  return selectedVendors.sort((a, b) => (b.trustScore || 5) - (a.trustScore || 5));
}

/**
 * Get brand reputation info
 */
function getBrandReputation(brand) {
  if (!brand) return null;

  // Normalize brand name for lookup
  const normalizedBrand = brand.toUpperCase().trim();

  for (const [name, info] of Object.entries(BRAND_REPUTATION)) {
    if (name.toUpperCase() === normalizedBrand || normalizedBrand.includes(name.toUpperCase())) {
      return { name, ...info };
    }
  }
...
/**
 * Calculate a comprehensive score for a part based on multiple factors
 * Score range: 0-100
 */
function calculatePartScore(part, vendors, allParts) {
  let score = 0;
  const factors = [];

  // Factor 1: Brand reputation (0-30 points)
  const brandRep = getBrandReputation(part.brand);
  if (brandRep) {
    const brandPoints = brandRep.score * 3; // 0-30 points
    score += brandPoints;
    factors.push({
      factor: 'Brand Reputation',
      points: brandPoints,
      detail: `${part.brand}: ${brandRep.notes}`,
    });
  } else if (part.brand) {
    // Unknown brand - give some points for having a brand
    score += 10;
    factors.push({ factor: 'Brand', points: 10, detail: `${part.brand} (not in our database)` });
  }

  // Factor 2: Vendor trust (0-20 points)
  const vendorInfo = vendors.find((v) => v.name === part.vendor);
  if (vendorInfo) {
    const vendorPoints = (vendorInfo.trustScore || 5) * 2; // 0-20 points
    score += vendorPoints;
    factors.push({
      factor: 'Vendor Trust',
      points: vendorPoints,
      detail: `${part.vendor}: ${vendorInfo.notes || 'Trusted retailer'}`,
    });
  } else {
    score += 6; // Unknown vendor baseline
    factors.push({ factor: 'Vendor', points: 6, detail: `${part.vendor}` });
  }

  // Factor 3: Price availability (0-15 points)
  if (part.price) {
    score += 15;
    factors.push({ factor: 'Price Available', points: 15, detail: `$${part.price}` });
  }

  // Factor 4: Part number available (0-10 points) - indicates specific product
  if (part.partNumber) {
    score += 10;
    factors.push({ factor: 'Part Number', points: 10, detail: part.partNumber });
  }

  // Factor 5: Multiple vendor availability (0-15 points)
  // Check if same brand/product appears at multiple vendors
  const sameProductCount = allParts.filter(
    (p) =>
      p.brand === part.brand &&
      p.title.toLowerCase().slice(0, 30) === part.title.toLowerCase().slice(0, 30) &&
      p.vendor !== part.vendor
  ).length;
  if (sameProductCount > 0) {
    const multiVendorPoints = Math.min(sameProductCount * 5, 15);
    score += multiVendorPoints;
    factors.push({
      factor: 'Multi-Vendor',
      points: multiVendorPoints,
      detail: `Found at ${sameProductCount + 1} vendors`,
    });
  }

  // Factor 6: Fitment indicators in text (0-10 points)
  const fitmentKeywords = [
    'direct fit',
    'bolt-on',
    'no modifications',
    'exact fit',
  ...
  const hasFitmentIndicator = fitmentKeywords.some(
    (kw) =>
      (part.excerpt || '').toLowerCase().includes(kw) ||
      (part.title || '').toLowerCase().includes(kw)
  );
  if (hasFitmentIndicator) {
    score += 10;
    factors.push({ factor: 'Fitment Verified', points: 10, detail: 'Direct fit indicator found' });
  }

  return { score, factors, tier: brandRep?.tier || 'unknown' };
}
...
function parsePrice(text) {
  ...
  // Match patterns like $699.99, $1,299.00, USD 599
...
/**
 * Extract part info from search result
 */
function extractPartInfo(result, vendorName) {
  const title = result.title || '';
  const url = result.url || '';
  const excerpt = result.text || result.snippet || '';

  // Try to extract price from excerpt or title
  const price = parsePrice(excerpt) || parsePrice(title);

  // Try to extract brand from title - use all known brands from BRAND_REPUTATION
  const knownBrands = Object.keys(BRAND_REPUTATION);
  ...
  // Try to extract part number (common patterns)
  const partNumberMatch =
    title.match(/\b([A-Z]{2,4}[-\s]?\d{3,6}[-\s]?[A-Z0-9]*)\b/i) ||
    excerpt.match(/Part\s*#?\s*:?\s*([A-Z0-9-]+)/i);
...
/**
 * Group similar parts from different vendors together
 */
function groupPartsByProduct(parts) {
  const groups = new Map();

  for (const part of parts) {
    // Create a grouping key based on brand + normalized product name
    const brandKey = (part.brand || 'unknown').toLowerCase();
    const titleKey = (part.title || '')
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '')
      .slice(0, 40);
    const groupKey = `${brandKey}:${titleKey}`;
    ...
    if (part.price) {
      group.prices.push(part.price);
    }
    // Keep the most detailed info
    if (part.partNumber && !group.partNumber) {
      group.partNumber = part.partNumber;
    }
    if (part.excerpt && (!group.excerpt || part.excerpt.length > group.excerpt.length)) {
      group.excerpt = part.excerpt;
    }
  }
...
/**
 * Deduplicate parts by URL and similar titles
 */
function deduplicateParts(parts) {
  ...
  for (const part of parts) {
    // Create a key from the URL domain + normalized title
    const urlHost = part.url ? new URL(part.url).hostname : 'unknown';
    const normalizedTitle = (part.title || '')
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '')
      .slice(0, 50);
...
/**
 * Research parts across multiple vendors with live pricing
 *
 * MULTI-PHASE RESEARCH APPROACH:
 * 1. Discovery Phase: Search multiple vendor sites and general web
 * 2. Analysis Phase: Score and rank parts based on brand reputation, vendor trust, etc.
 * 3. Comparison Phase: Group same products from different vendors
 * 4. Recommendation Phase: Generate ranked recommendations with reasoning
 *
 * @param {Object} params - Search parameters
 * @param {number} [params.car_year] - Model year
 * @param {string} [params.car_make] - Make (e.g., "Ram")
 * @param {string} [params.car_model] - Model (e.g., "TRX")
 ...
 * @param {string[]} [params.specific_brands] - Brands to prioritize
 * @param {boolean} [params.include_used] - Include used listings
 * @returns {Object} Comprehensive parts research results with analysis
 */
export async function researchPartsLive({
  ...
  const researchLog = []; // Track what research was done
  ...
  // =========================================================================
  // PHASE 0: Resolve car information
  // =========================================================================
  let carInfo = { year: car_year, make: car_make, model: car_model };

  if (car_slug && (!car_year || !car_make || !car_model)) {
    try {
      const car = await fetchCarBySlug(car_slug);
      if (car) {
        // Extract year from "years" field (e.g., "2018-2024" → 2022 or middle year)
        let parsedYear = car_year;
        if (!parsedYear && car.years) {
          ...
          if (yearMatch) {
            ...
            // Use middle of range or most recent year
            parsedYear = Math.floor((startYear + endYear) / 2);
          }
        }

        // Extract make/model from car name (e.g., "Ram TRX" → make: "Ram", model: "TRX")
        // First check if car has explicit brand field
        let parsedMake = car_make || car.brand || car.make;
        let parsedModel = car_model || car.model;

        // Fallback: parse from car name (e.g., "Porsche 911 GT3" → "Porsche", "911 GT3")
        if ((!parsedMake || !parsedModel) && car.name) {
          const nameParts = car.name.split(' ');
          if (!parsedMake && nameParts.length > 0) {
            parsedMake = nameParts[0];
          }
          if (!parsedModel && nameParts.length > 1) {
            parsedModel = nameParts.slice(1).join(' ');
          }
        }
        ...
        carInfo = {
          ...
          make: parsedMake,
          model: parsedModel,
          fullName: car.name, // Keep original name for display
        };
        researchLog.push(
          `Resolved car: ${car.name} (${car.years}) → ${parsedYear} ${parsedMake} ${parsedModel}`
        );
      }
    } catch (e) {
  ...
  if (!carInfo.year && !carInfo.make && !carInfo.model) {
    return {
      ...
      message:
        'Please provide car_year, car_make, and car_model (or car_slug) to search for fitting parts.',
    };
  }

  // Build search context - use full name if available, otherwise construct from parts
  ...
  researchLog.push(`Researching ${upgrade_type} for: ${carContext}`);

  // Get upgrade type info
  const normalizedUpgrade = upgrade_type.toLowerCase().trim();
  const upgradeInfo = UPGRADE_TYPE_MAP[normalizedUpgrade] || {
    terms: [upgrade_type],
    category: 'other',
  };

  // Select relevant vendors
  const vendors = selectVendorsForSearch(carInfo.make, upgradeInfo.category);
  const vendorDomains = vendors.flatMap((v) => v.domains);
  researchLog.push(`Selected ${vendors.length} vendors: ${vendors.map((v) => v.name).join(', ')}`);

  // =========================================================================
  // PHASE 1: DISCOVERY - Multi-source search
  // =========================================================================
  researchLog.push('--- PHASE 1: DISCOVERY ---');

  const searchTerms = upgradeInfo.terms[0];
  const primaryQuery = `${carContext} ${searchTerms}`;

  // Additional search variations for thorough research
  const searchVariations = [
    primaryQuery,
    `best ${searchTerms} for ${carContext}`,
    `${carContext} ${searchTerms} review`,
    `top rated ${searchTerms} ${carInfo.make} ${carInfo.model}`,
  ];

  console.log(
    `[researchPartsLive] Phase 1: Searching ${searchVariations.length + 2} queries across ${vendors.length} vendors`
  );

  try {
    const searchPromises = [];

    // 1. Trusted vendor search (site-restricted to known good vendors)
    searchPromises.push(
      fetch('https://api.exa.ai/search', {
        ...
        body: JSON.stringify({
          query: primaryQuery,
          num_results: 20, // More results for better analysis
          use_autoprompt: true,
          type: 'auto',
          includeDomains: vendorDomains.slice(0, 10),
          contents: { text: { max_characters: 800 } },
        }),
      })
        .then((r) => r.json())
        .then((r) => ({ source: 'vendor_search', ...r }))
        .catch((e) => ({ source: 'vendor_search', results: [], error: e.message }))
    );
    researchLog.push(`Searching trusted vendors: ${vendorDomains.slice(0, 10).join(', ')}`);

    // 2. "Best" query for review/recommendation content
    searchPromises.push(
      fetch('https://api.exa.ai/search', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          query: `best ${searchTerms} for ${carContext} 2024`,
          num_results: 15,
          use_autoprompt: true,
          type: 'auto',
          contents: { text: { max_characters: 800 } },
        ...
        .then((r) => r.json())
        .then((r) => ({ source: 'best_search', ...r }))
        .catch((e) => ({ source: 'best_search', results: [], error: e.message }))
    );
    researchLog.push('Searching for "best" recommendations and reviews');

    // 3. Shopping-focused search
    searchPromises.push(
      fetch('https://api.exa.ai/search', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          query: `${primaryQuery} buy price in stock`,
          num_results: 15,
          use_autoprompt: true,
          type: 'auto',
        ...
        .then((r) => r.json())
        .then((r) => ({ source: 'shopping_search', ...r }))
        .catch((e) => ({ source: 'shopping_search', results: [], error: e.message }))
    );
    researchLog.push('Searching shopping/pricing sources');

    // 4. Brand-specific searches for requested brands
    for (const brand of specific_brands.slice(0, 3)) {
      searchPromises.push(
        fetch('https://api.exa.ai/search', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            query: `${carContext} ${brand} ${searchTerms}`,
            num_results: 8,
            use_autoprompt: true,
            type: 'auto',
            contents: { text: { max_characters: 600 } },
          ...
          .then((r) => r.json())
          .then((r) => ({ source: `brand_${brand}`, ...r }))
          .catch((e) => ({ source: `brand_${brand}`, results: [], error: e.message }))
      );
      researchLog.push(`Searching specifically for: ${brand}`);
    }

    // 5. Manufacturer direct search
    const manufacturerDomains = vendors
      .filter((v) => v.trustScore === 10)
      .flatMap((v) => v.domains);
    if (manufacturerDomains.length > 0) {
      searchPromises.push(
        fetch('https://api.exa.ai/search', {
          method: 'POST',
          headers: {
          ...
          .then((r) => r.json())
          .then((r) => ({ source: 'manufacturer_direct', ...r }))
          .catch((e) => ({ source: 'manufacturer_direct', results: [], error: e.message }))
      );
      researchLog.push(`Searching manufacturer sites: ${manufacturerDomains.join(', ')}`);
    }

    // 6. Used/eBay search if requested
    if (include_used) {
      searchPromises.push(
        fetch('https://api.exa.ai/search', {
          method: 'POST',
          ...
          body: JSON.stringify({
            ...
            use_autoprompt: true,
            type: 'auto',
            includeDomains: ['ebay.com', 'facebook.com/marketplace'],
            contents: { text: { max_characters: 400 } },
          }),
        })
          .then((r) => r.json())
          .then((r) => ({ source: 'used_market', ...r }))
          .catch((e) => ({ source: 'used_market', results: [], error: e.message }))
      );
      researchLog.push('Including used market search (eBay)');
    }

    // Execute all searches in parallel
    const searchResults = await Promise.all(searchPromises);
    researchLog.push(`Completed ${searchResults.length} parallel searches`);

    // =========================================================================
    // PHASE 2: ANALYSIS - Extract and score parts
    // =========================================================================
    researchLog.push('--- PHASE 2: ANALYSIS ---');
    ...
    for (const data of searchResults) {
      ...
      sourceCounts[source] = data.results?.length || 0;

      if (data.results && Array.isArray(data.results)) {
        for (const result of data.results) {
          // Determine vendor from URL
          let vendorName = 'Other';
          let vendorInfo = null;
          ...
          if (result.url) {
            try {
              ...
              const matchedVendor = vendors.find((v) =>
                v.domains.some((d) => host.includes(d.replace('www.', '')))
              );
              if (matchedVendor) {
                vendorName = matchedVendor.name;
                vendorInfo = matchedVendor;
              } else {
                vendorName =
          ...
          const partInfo = extractPartInfo(result, vendorName);

          // Filter by budget if specified
          if (partInfo.price) {
            if (budget_min && partInfo.price < budget_min) continue;
            if (budget_max && partInfo.price > budget_max) continue;
          }

          partInfo.source = source;
          partInfo.vendorInfo = vendorInfo;
          allParts.push(partInfo);
        }
      }
    }

    researchLog.push(`Extracted ${allParts.length} potential parts from search results`);
    researchLog.push(`Sources: ${JSON.stringify(sourceCounts)}`);

    // Deduplicate
    allParts = deduplicateParts(allParts);
    researchLog.push(`After deduplication: ${allParts.length} unique parts`);

    // =========================================================================
    // PHASE 3: SCORING - Calculate comprehensive scores
    // =========================================================================
    researchLog.push('--- PHASE 3: SCORING ---');

    // Score each part
    const scoredParts = allParts.map((part) => {
      const scoring = calculatePartScore(part, vendors, allParts);
      return {
        ...part,
        score: scoring.score,
    ...
    researchLog.push(`Scored and ranked ${scoredParts.length} parts`);
    researchLog.push(
      `Top score: ${scoredParts[0]?.score || 0}, Bottom score: ${scoredParts[scoredParts.length - 1]?.score || 0}`
    );

    // =========================================================================
    // PHASE 4: COMPARISON - Group same products across vendors
    // =========================================================================
    researchLog.push('--- PHASE 4: COMPARISON ---');

    const productGroups = groupPartsByProduct(scoredParts);
    researchLog.push(`Grouped into ${productGroups.size} distinct products`);

    // =========================================================================
    // PHASE 5: RECOMMENDATION - Build final recommendations
    // =========================================================================
    researchLog.push('--- PHASE 5: RECOMMENDATIONS ---');

    // Take top 10 scored parts for recommendations
    const topParts = scoredParts.slice(0, 10);

    // Build recommendations with reasoning
    const recommendations = topParts.map((part, index) => {
      const brandRep = getBrandReputation(part.brand);
      const vendorInfo = vendors.find((v) => v.name === part.vendor);
      ...
      if (brandRep) {
        reasons.push(
          `${part.brand} is ${brandRep.tier === 'premium' ? 'a premium brand' : 'well-regarded'}: ${brandRep.notes}`
        );
      }
      if (vendorInfo && vendorInfo.trustScore >= 8) {
        reasons.push(`Available from trusted retailer ${part.vendor}`);
      }
      if (part.price) {
        reasons.push(`Priced at $${part.price}`);
      }

      // Find same product at other vendors
      const otherVendors = [];
      for (const otherPart of scoredParts) {
        if (
          otherPart.brand === part.brand &&
          otherPart.title.toLowerCase().slice(0, 30) === part.title.toLowerCase().slice(0, 30) &&
          otherPart.vendor !== part.vendor
        ) {
          otherVendors.push({
            vendor: otherPart.vendor,
      ...
      return {
        ...
        url: part.url,
        score: part.score,
        tier: part.tier || 'unknown',
        recommendation_reasons: reasons,
        scoring_breakdown: part.scoringFactors,
        also_available_at: otherVendors.slice(0, 3),
      };
    });

    // Build price analysis
    const prices = scoredParts.map((p) => p.price).filter(Boolean);
    const priceAnalysis =
      prices.length > 0
        ? {
            min: Math.min(...prices),
    ...
    const tierBreakdown = {
      premium: scoredParts.filter((p) => p.tier === 'premium').length,
      mid: scoredParts.filter((p) => p.tier === 'mid').length,
      budget: scoredParts.filter((p) => p.tier === 'budget').length,
      unknown: scoredParts.filter((p) => !p.tier || p.tier === 'unknown').length,
    };
    ...
    researchLog.push(`Research completed in ${researchTime}ms`);
    ...
    return {
      ...
      research_summary: {
        ...
        total_results_found: allParts.length,
        unique_products: productGroups.size,
        research_time_ms: researchTime,
      },

      // Top recommendations (scored and analyzed)
      recommendations: recommendations.slice(0, 5),

      // Extended results for more options
      additional_options: recommendations.slice(5, 10),

      // Price analysis
      price_analysis: priceAnalysis,

      // Brand analysis
      brands_found: Object.entries(brandCounts)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 10)
        .map(([brand, count]) => ({
          brand,
          count,
          reputation: getBrandReputation(brand),
        })),
      ...
      // Vendors searched
      vendors_searched: vendors.map((v) => ({
        name: v.name,
        trust_score: v.trustScore,
        notes: v.notes,
      })),

      // Research log (for transparency)
      research_log: researchLog,

      // Quick picks (pre-analyzed summaries)
      quick_picks: {
        best_overall: recommendations[0] || null,
        best_value:
          recommendations.find((r) => r.tier === 'mid' || r.tier === 'budget') ||
          recommendations[1] ||
          null,
        premium_choice: recommendations.find((r) => r.tier === 'premium') || null,
      },

      // Instructions for AL on how to present
      presentation_instructions: `
IMPORTANT: Present these as RESEARCHED recommendations, not just search results.

Format your response like this:

## Top 5 ${upgrade_type} Recommendations for ${carContext}

Based on my research across ${searchResults.length} sources and ${allParts.length} products found:

**1) [Brand] [Product Name]** - RECOMMENDED
- **Why:** [Use recommendation_reasons from the data]
- **Score:** [X]/100 based on brand reputation, vendor trust, and availability
- **Price:** $XXX
- **Buy from:** [Primary Vendor](url)
- **Also at:** [Other vendors if available]
...
### Research Summary
- Searched ${vendors.length} trusted vendors
- Analyzed ${scoredParts.length} products
- Compared prices across ${prices.length} listings
- Found ${tierBreakdown.premium} premium, ${tierBreakdown.mid} mid-tier, ${tierBreakdown.budget} budget options

### Quick Picks
- **Best Overall:** [quick_picks.best_overall]
- **Best Value:** [quick_picks.best_value]
- **Premium Choice:** [quick_picks.premium_choice]

Always include the actual URLs from the data. Mention that users should verify fitment.
      `.trim(),
    };
  } catch (err) {
    ...
    return {
      ...
      research_log: researchLog,
      suggestion:
        'Try using search_parts for our internal database or search_web for general results.',
    };
  }
}
 ...
 */
async function searchPartsRecommendation({ car_slug, upgrade_type, budget, limit, client }) {
  if (!car_slug) {
    return { error: 'car_slug is required when using upgrade_type recommendations' };
  }
 ...
 * @param {string} [params.car_slug] - Car to filter parts for fitment
...
export async function searchParts({
  ...
  if (upgrade_type && !query) {
    return searchPartsRecommendation({ car_slug, upgrade_type, budget, limit: safeLimit, client });
  }
 ...
 */
export async function findBestParts({ car_slug, upgrade_type, budget = 'mid', limit = 5 }) {
  console.warn(
 ...
 * @param {string} params.car_slug - Optional car context for better analysis
...
export async function analyzeUploadedContent({
  ...
  try {
    ...
    let carContext = null;
    if (car_slug) {
      const { data: carData } = await db.rpc('get_car_ai_context_v2', { p_car_slug: car_slug });
      if (carData?.[0]) {
...
export async function getUserBuilds({ vehicle_id = null, include_parts = false, user_id = null }) {
  ...
  const PROJECT_COLUMNS =
    'id, user_id, car_id, car_slug, car_name, project_name, selected_upgrades, total_hp_gain, total_cost_low, total_cost_high, final_hp, stock_hp, notes, is_favorite, created_at, updated_at';
  ...
  try {
    ...
    if (vehicle_id) {
      ...
      const { data: vehicle } = await client
        .from('user_vehicles')
        .select('matched_car_id, matched_car_slug')
        .eq('id', vehicle_id)
        .single();

      if (vehicle?.matched_car_id) {
        query = query.eq('car_id', vehicle.matched_car_id);
      } else if (vehicle?.matched_car_slug) {
        // Per SOURCE_OF_TRUTH Cardinal Rule 1: Resolve slug to car_id
        const resolvedCarId = await resolveCarId(vehicle.matched_car_slug);
        if (resolvedCarId) {
          query = query.eq('car_id', resolvedCarId);
        } else {
          console.warn(
            '[AL Tools] Could not resolve car_id for vehicle slug:',
...
async function matchVinToCar(decoded) {
  ...
  try {
    ...
    for (const car of cars) {
      if (car.years && year) {
        // Parse years like "2018-2023" or "2020+"
