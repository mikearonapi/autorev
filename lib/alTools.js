...
export async function persistResearchResults(carId, extractedParts) {
  ...
  for (const part of extractedParts) {
    try {
      ...
      if (existingPart) {
        // Part exists - just update source_urls if we have a new URL
        partId = existingPart.id;
        if (part.source_url && !existingPart.source_urls?.includes(part.source_url)) {
          const updatedUrls = [...(existingPart.source_urls || []), part.source_url];
          await client.from('parts').update({ source_urls: updatedUrls }).eq('id', partId);
        }
        console.log(
          `[persistResearchResults] Found existing part: ${normalizedBrand} ${normalizedName}`
        );
      } else {
        // Insert new part
        const category =
      ...
      saved.push({ part_id: partId, name: normalizedName, brand: normalizedBrand });
    } catch (err) {
