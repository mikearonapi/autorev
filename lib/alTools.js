...
async function doWebResearch(carInfo, upgradeType, budgetMin, budgetMax, onProgress) {
  ...
  try {
    ...
    // We use Promise.all for speed but track completion for progress
    const searchResults = [];
    const completedSearches = new Set();

    // Send "started" events for all searches BEFORE they begin
    // This shows users the spinners immediately, giving accurate progress feedback
    if (onProgress) {
    ...
    // Create promises that report progress when complete
    ...
    // Run all searches in parallel but with progress tracking
