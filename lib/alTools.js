...
export async function extractAndSavePartsFromResponse(
  ...
  try {
    ...
    try {
      ...
      console.warn(`[alTools:${correlationId}] Failed to parse parts_to_save JSON:`, parseErr.message);
      return;
    }

    // Handle both old format (array) and new format (object with upgrade_key)
    let partsData;
    let upgradeKey;
    ...
    console.log(
      `[alTools:${correlationId}] Extracting ${partsData.length} parts to save for car ${carId}` +
        (upgradeKey ? ` (upgrade: ${upgradeKey})` : '')
    );

    // Save parts async (don't await - fire and forget)
    persistResearchResults(carId, partsData, { upgradeKey, conversationId })
      .then((result) => {
        if (result.saved > 0) {
...
6. At the END, include JSON for database (INCLUDE ALL FIELDS - this is what gets saved):
