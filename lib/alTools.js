...
export async function searchForums({ query, car_context = null, sources = ['all'] }) {
  ...
  // DEFERRED: Exa API integration for forum search
  // Priority: Low | Tracking: GitHub Issue recommended
  // Current behavior: Returns placeholder with suggested forums
  // Future: Real-time web/forum search via Exa or similar API
  ...
  try {
    ...
    const formattedEvents = (data.events || []).map(event => {
      ...
      const result = {
        ...
        type_icon: event.event_type?.icon || '📅',
        date: formattedDate,
