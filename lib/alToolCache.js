...
const STATS = globalThis.__AUTOREV_AL_CACHE_STATS__ || {
  hits: 0,
  misses: 0,
  redisHits: 0,
  redisMisses: 0,
...
export function setCached(key, value, ttlMs) {
  ...
  if (REDIS_ENABLED && ttl >= 30000) {
    // Only cache items with 30s+ TTL to Redis
  ...
  return {
    ...
    hitRate:
      STATS.hits + STATS.misses > 0
        ? (
            ((STATS.hits + STATS.redisHits) /
              (STATS.hits + STATS.misses + STATS.redisHits + STATS.redisMisses)) *
            100
          ).toFixed(1) + '%'
        : 'N/A',
  };
