...
const prefetchCache = {
  ...
  builds: null,
  compareLists: null,
  heroImages: null, // Map of carId -> heroImageUrl
  carsList: null, // Cars list for garage vehicle matching
  dashboard: null, // Dashboard data
  insights: null, // Insights data for instant Insights page load
  lastUserId: null,
};
...
function loadCacheFromStorage() {
  ...
  if (now - stored.timestamp > CACHE_TTL_MS) {
    console.log(
      '[Prefetch] Cache expired (age:',
      Math.round((now - stored.timestamp) / 1000),
      'seconds), clearing'
    );
    sessionStore.remove(CACHE_KEY);
    return;
  ...
  prefetchCache.insights = stored.data.insights || null;
  ...
  if (stored.data.insights) prefetchedResources.add('insights');

  const age = Math.round((now - stored.timestamp) / 1000);
  console.log(
    '[Prefetch] Cache loaded from sessionStorage (age:',
    age,
    'seconds, user:',
    stored.userId,
    ')'
...
function saveCacheToStorage() {
  ...
  const cacheData = {
    ...
    data: {
      ...
      insights: prefetchCache.insights,
    },
  };
...
export async function prefetchAllUserData(userId) {
  ...
  // Includes cars list, dashboard, and insights for instant page rendering after splash
  await withTimeout(
    Promise.allSettled([
      ...
      prefetchInsights(userId), // Insights data for instant insights page rendering
    ]),
...
async function prefetchHeroImages(userId) {
  ...
  try {
    ...
    // Build a map of carId -> heroImageUrl
    const heroImagesMap = {};
    const imagesToPreload = [];

    for (const row of data) {
      if (row.hero_image_url && row.car_id) {
        heroImagesMap[row.car_id] = {
...
/**
 * Prefetch insights data for instant Insights page rendering
 * @param {string} userId - User ID
 */
async function prefetchInsights(userId) {
  if (prefetchedResources.has('insights')) return;
  if (!userId) return;

  try {
    const response = await withTimeout(
      fetch(`/api/users/${userId}/insights`),
      PREFETCH_TIMEOUTS.vehicles, // Use same timeout as vehicles
      '[Prefetch] Insights prefetch'
    );

    if (!response.ok) {
      console.warn('[Prefetch] Insights fetch failed:', response.status);
      return;
    }
    ...
    if (result.data) {
      prefetchCache.insights = result.data;
      prefetchedResources.add('insights');
      console.log('[Prefetch] Insights prefetched');
    }
  } catch (err) {
    console.warn('[Prefetch] Insights prefetch failed:', err);
  }
}
...
export function getPrefetchedHeroImage(carId, userId) {
  ...
  return prefetchCache.heroImages[carId] || null;
}
