...
/**
 * Normalize selected_upgrades to an array of upgrade keys (strings)
 * Handles both array format and object format { upgrades: [...] }
 * Also handles items that are objects with upgrade_key/key properties
 */
function normalizeUpgradeKeys(selectedUpgrades) {
  ...
  if (Array.isArray(selectedUpgrades)) {
    upgradesList = selectedUpgrades;
  } else if (selectedUpgrades.upgrades && Array.isArray(selectedUpgrades.upgrades)) {
    upgradesList = selectedUpgrades.upgrades;
  }
  
  // Normalize each item to a string key
  return upgradesList.map(item => {
    if (typeof item === 'string') return item;
    if (typeof item === 'object' && item !== null) {
      // Handle common object formats: {upgrade_key: '...'} or {key: '...'}
      return item.upgrade_key || item.key || item.id || null;
    }
    return null;
  }).filter(Boolean);
}
...
function formatModName(modId) {
  ...
  if (typeof modId !== 'string') {
    // Try to extract from object if possible
    if (typeof modId === 'object') {
      modId = modId.upgrade_key || modId.key || modId.id;
    }
    if (typeof modId !== 'string') return 'Upgrade';
  }
  return modId
    .replace(/-/g, ' ')
