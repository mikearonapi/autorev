...
// Maps FastestLaps track slugs to AutoRev track slugs
// This ensures lap time data is correctly attributed to the right configuration
// ============================================================================

const FASTESTLAPS_TRACK_MAPPING = {
  // VIR - Virginia International Raceway configurations
  // IMPORTANT: FastestLaps has confusing URLs - verify track length before mapping
  'virginia-international-raceway': 'vir-full', // 3.27 mi full course
  'virginia-international-raceway-full-course': 'vir-full', // 3.27 mi
  'virginia-international-raceway-north': 'vir-north', // 2.25 mi
  'virginia-international-raceway-south': 'vir-south', // 1.65 mi
  'virginia-international-raceway-patriot': 'vir-patriot', // 1.10 mi
  'virginia-international-raceway-grand-east-course-pre-01-2014': 'vir-grand-east', // 4.20 mi
  'virginia-international-raceway-grand-east-course-post-01-2014': 'vir-grand-east', // 4.20 mi (same config, just date change)
  'virginia-international-raceway-grand-west-course-pre-01-2014': 'vir-grand-west', // 3.97 mi
  'virginia-international-raceway-grand-west-course-post-01-2014': 'vir-grand-west', // 3.97 mi (FIXED: was incorrectly mapped to vir-full)

  // Laguna Seca
  'laguna-seca-post-1988': 'laguna-seca',
  'laguna-seca-pre-1988': 'laguna-seca', // Map historical to current

  // Willow Springs
  'willow-springs': 'willow-springs-big',
  'willow-springs-horse-thief-mile': 'willow-springs-htm',
  'willow-springs-streets-of-willow': 'willow-springs-streets',
  'willow-springs-streets-of-willow-extended': 'willow-springs-streets',

  // Buttonwillow - Map FastestLaps configs to our slugs
  'buttonwillow-configuration-13': 'buttonwillow-13cw',
  'buttonwillow-configuration-13-a': 'buttonwillow-13a-cw',
  'buttonwillow-configuration-13-a-ccw': 'buttonwillow-13a-cw', // Map CCW to CW entry
  'buttonwillow-configuration-13-ccw': 'buttonwillow-13-ccw',
  'buttonwillow-configuration-1': 'buttonwillow-1',
  'buttonwillow-configuration-1-ccw': 'buttonwillow-1',
  'buttonwillow-configuration-26': 'buttonwillow-25-cw',
  'button-willow-configuration-13-a-ccw': 'buttonwillow-13a-cw',

  // Thunderhill
  'thunderhill-raceway': 'thunderhill-west',
  'thunderhill-park': 'thunderhill-east',
  'thunderhill-park-east': 'thunderhill-east',
  'thunderhill-5-mile-double-bypass': 'thunderhill-full',

  // COTA
  'circuit-of-the-americas': 'cota-full',

  // Road America
  'road-ameica': 'road-america', // Note: FastestLaps has typo in their slug
  'road-america-east-route': 'road-america', // Map to full course (no separate East config)

  // Watkins Glen
  'watkins-glen-international-grand-prix-circuit-1992-present': 'watkins-glen',
  'watkins-glen': 'watkins-glen',
  'watkins-glen-international-short-post-1979': 'watkins-glen-short',

  // Sebring
  'sebring-international-raceway': 'sebring-full',
  'sebring-short': 'sebring-short',

  // Mid-Ohio
  'mid-ohio-sports-car-course': 'mid-ohio',
  'mid-ohio-sports-car-course-short': 'mid-ohio-short',

  // Sonoma
  'sonoma-raceway-full': 'sonoma-full',
  'sonoma-1968-1997-sears-point-golden-state-infineon-raceway': 'sonoma-full',
  ...
  'road-atlanta-1970-1997': 'road-atlanta',
  'road-atlanta-short': 'road-atlanta', // Map to full (we don't have short)

  // NCM Motorsports Park
  'ncm-motorsport-park': 'ncm-motorsports-park',
  'ncm-motorsports-park': 'ncm-motorsports-park',
  'ncm-motorsports-park-grand-full': 'ncm-motorsports-park',

  // Nürburgring
  nordschleife: 'nurburgring-nordschleife',
  'nordschleife-btg': 'nurburgring-nordschleife',
  'nrburgring-gp': 'nurburgring-gp',
...
/**
 * Resolve a FastestLaps track slug to our AutoRev slug
 * Uses explicit mapping first, then falls back to direct match
 * @param {string} fastestLapsSlug - The slug from FastestLaps URL
 * @returns {string} The corresponding AutoRev track slug
 */
function resolveTrackSlug(fastestLapsSlug) {
  // Check explicit mapping first
  if (FASTESTLAPS_TRACK_MAPPING[fastestLapsSlug]) {
    return FASTESTLAPS_TRACK_MAPPING[fastestLapsSlug];
  }
  // Fall back to the original slug (may need normalization)
  return fastestLapsSlug;
}
...
async function getCarLookup() {
  ...
  const { data, error } = await client.from('cars').select('id, slug, name, years');
 ...
 * IMPORTANT: Uses 'tracks' table (not 'track_venues') because car_track_lap_times
 * references tracks.id. This ensures lap times are correctly associated with
 * specific track configurations (e.g., VIR Full vs VIR Grand East).
...
async function getTrackLookup() {
  ...
  const { data, error } = await client
    .from('tracks')
    .select('id, slug, name, country, length_km, length_miles, configuration')
    .eq('is_active', true);
  ...
  for (const track of data || []) {
    ...
    // Also index by slug without hyphens for fuzzy matching
    lookup.set(normalizeName(track.slug), track);
  }

  console.log(`[LapTimesScraper] Loaded ${data?.length || 0} tracks from database`);
  return lookup;
}
...
/**
 * Check if a car name is a race car (not a production car)
 * Race cars are excluded from scraping since we focus on consumer vehicles
 */
function isRaceCar(carName) {
  if (!carName) return false;

  const raceCarPatterns = [
    // Formula 1 teams/cars (comprehensive)
    /^(Red Bull|McLaren MCL|Ferrari SF\d|Mercedes-Benz W\d{1,2}|Williams FW|Haas VF|Alpine A\d{3}|Sauber|Renault R(S\d{2}|[0-9])|Honda RA)/i,
    /\b(Racing Bulls|VCARB|AlphaTauri|Toro Rosso|Racing Point|Force India|Alfa Romeo C\d|AMR2[0-9])\b/i,
    /W1[0-6] E Performance/i,

    // Prototype/LMP/Hypercar (Le Mans classes)
    /\b(LMP|LMH|DPi|GTP|Hypercar|Prototype)\b/i,
    /(Peugeot 90[0-9]|9x8|Audi R\d{1,2}|Porsche 9(19|63)|Toyota (TS|GR010)|Acura ARX|Cadillac (DPi|V-Series\.R))/i,
    /(Oreca|Ligier|Lola|Zytek|Pescarolo|HPD|Creation|Rebellion|BR Engineering|Dallara)/i,
    /(Ferrari 499P|Ferrari 296 GT3|BMW M Hybrid)/i,
    /RS Spyder/i,

    // GT3/GTE/Cup Race Cars (track-only)
    /\b(GT3|GTE|GTD|GT4|Cup|RSR|GT2|GTLM|GT1)\b/i,
    /\b(FIA GT|Super GT|DTM|TCR)\b/i,

    // Spec/Formula/Single-seaters
    /\b(Formula|Spec Miata|Spec E30|Radical SR|Caterham R500|BAC Mono|Ariel Atom V8|KTM X-Bow GT)\b/i,

    // NASCAR/IndyCar/Stock Car/Touring
    /\b(NASCAR|IndyCar|Stock Car|Xfinity|Truck Series|BTCC|WTCC)\b/i,

    // IMSA/WEC specific
    /(Mazda RT24|Corvette C[5-9]\.R|Ford GT LM|Aston Martin Vantage AMR|Glickenhaus)/i,
  ];

  return raceCarPatterns.some((pattern) => pattern.test(carName));
}
 ...
 */
export async function scrapeFastestLapsCarPage(carSlug, _options = {}) {
...
export async function scrapeFastestLapsTestPage(testUrl) {
  ...
  const result = await firecrawlClient.extractStructured(testUrl, schema, {
    prompt:
      'Extract the lap time test data from this FastestLaps page. Include car name, track, lap time, and any available metadata.',
  });
...
export async function discoverAndScrapeFastestLaps(options = {}) {
  ...
  if (carSlugs) {
    ...
    const client = getDbClient();
    const { data: cars } = await client.from('cars').select('slug, name').limit(limit);
  ...
  for (const carInfo of carsToProcess) {
    try {
      ...
      const _ourSlug = typeof carInfo === 'string' ? carInfo : carInfo.ourSlug;
      ...
      for (const modelLink of modelLinks) {
        ...
        if (actualCarName) {
          ...
          if (!car) {
            const nameNoYear = actualCarName
        ...
        const _searchNorm = normalizeName(searchName);
        ...
        const commonWords = actualWords.filter((w) =>
          matchedWords.some((mw) => mw.includes(w) || w.includes(mw))
        );
        ...
        for (const lt of carPageResult.lapTimes) {
          ...
          if (dryRun) {
            console.log(
              `[LapTimesScraper][DRY-RUN] Would insert: ${car.name} at ${lt.track_name}: ${lt.lap_time_text}`
            );
...
function parseTrackPageMarkdown(markdown, sourceUrl, trackName) {
  ...
  const tableRowPattern =
    /\|\s*\d+\s*\|\s*\[([^\]]+)\]\(([^)]+)\)\s*\|\s*\[?(\d{1,2}:\d{2}\.\d{1,3})\]?\(?([^)|]*)\)?/;

  // Alternative simpler pattern
  const _simpleRowPattern = /\|\s*\d+\s*\|([^|]+)\|([^|]*\d{1,2}:\d{2}\.\d{1,3}[^|]*)\|/;
  ...
  for (const line of lines) {
    ...
    if (tableMatch) {
      ...
      if (lapTimeMs && carName) {
        lapTimes.push({
          car_name: carName,
          car_model_url: modelUrl.startsWith('http')
            ? modelUrl
            : `https://fastestlaps.com${modelUrl}`,
          lap_time_text: lapTimeText,
          lap_time_ms: lapTimeMs,
          track_name: trackName,
          source_url:
            testUrl && testUrl.includes('/tests/')
              ? testUrl.startsWith('http')
                ? testUrl
                : `https://fastestlaps.com${testUrl}`
              : sourceUrl,
          raw_line: line.trim(),
        });
        continue;
 ...
 * @param {string} fastestLapsSlug - The track slug from FastestLaps (will be mapped to AutoRev slug)
 * @param {Object} options - Import options
 * @param {boolean} options.dryRun - If true, don't insert data
 * @param {boolean} options.includeRaceCars - If true, include race cars (default: false)
 */
export async function importLapTimesForTrack(fastestLapsSlug, options = {}) {
  const { dryRun = false, includeRaceCars = false } = options;

  const carLookup = await getCarLookup();
  const trackLookup = await getTrackLookup();

  // Scrape the track page first to get data (using FastestLaps slug)
  const scrapeResult = await scrapeFastestLapsTrackPage(fastestLapsSlug);

  if (!scrapeResult.success) {
    return scrapeResult;
  }

  // Filter out race cars (F1, prototypes, GT3, etc.) unless explicitly requested
  if (!includeRaceCars) {
    const originalCount = scrapeResult.lapTimes.length;
    scrapeResult.lapTimes = scrapeResult.lapTimes.filter((lt) => !isRaceCar(lt.car_name));
    const filtered = originalCount - scrapeResult.lapTimes.length;
    if (filtered > 0) {
      console.log(
        `[LapTimesScraper] Filtered ${filtered} race car times (F1, prototypes, GT3, etc.)`
      );
    }
    scrapeResult.lapTimeCount = scrapeResult.lapTimes.length;
  }

  // Resolve FastestLaps slug to AutoRev slug using explicit mapping
  const autoRevSlug = resolveTrackSlug(fastestLapsSlug);
  const slugChanged = autoRevSlug !== fastestLapsSlug;

  if (slugChanged) {
    console.log(
      `[LapTimesScraper] Mapped FastestLaps slug "${fastestLapsSlug}" -> AutoRev slug "${autoRevSlug}"`
    );
  }

  // Find track in our database using resolved slug
  let track = trackLookup.get(autoRevSlug) || trackLookup.get(normalizeName(autoRevSlug));

  // Also try the original FastestLaps slug as fallback
  if (!track) {
    track = trackLookup.get(fastestLapsSlug) || trackLookup.get(normalizeName(fastestLapsSlug));
  }

  if (!track && !dryRun) {
    // Create new track from scraped data using the resolved slug
    track = await upsertTrack({
      slug: autoRevSlug,
      name: scrapeResult.trackName || autoRevSlug,
      metadata: { source: 'fastestlaps', fastestlaps_slug: fastestLapsSlug },
    });
    if (track) {
      trackLookup.set(autoRevSlug, track);
    }
  }
  ...
  if (!track) {
    return {
      success: false,
      error: `Could not resolve/create track: ${fastestLapsSlug} (mapped to ${autoRevSlug})`,
    };
  }
  ...
  for (const lt of scrapeResult.lapTimes) {
    ...
    // Validate lap time makes sense for track length (if known)
    // This prevents data from wrong configurations being inserted
    if (track.length_miles && lt.lap_time_ms) {
      const avgSpeedMph = track.length_miles / (lt.lap_time_ms / 3600000);

      // Production cars typically average 40-140 mph on track
      // Flag times that seem impossible for track length
      if (avgSpeedMph < 30 || avgSpeedMph > 160) {
        console.log(
          `[LapTimesScraper] ⚠️ Suspicious time for ${car.name}: ${lt.lap_time_text} ` +
            `(avg ${avgSpeedMph.toFixed(0)} mph on ${track.length_miles.toFixed(2)} mi track) - SKIPPING`
        );
        stats.skipped++;
        continue;
      }
    ...
    if (dryRun) {
      console.log(
        `[LapTimesScraper][DRY-RUN] Would insert: ${car.name} (matched from "${lt.car_name}"): ${lt.lap_time_text}`
      );
      continue;
    }
  ...
  for (const trackSlug of tracksToProcess) {
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `[LapTimesScraper] ${trackSlug}: ${result.stats.lapTimesFound} found, ${result.stats.matched} matched, ${result.stats.inserted} inserted`
        );
      } else {
