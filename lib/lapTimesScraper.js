...
export async function importLapTimesForTrack(fastestLapsSlug, options = {}) {
  ...
  if (slugChanged) {
    console.log(`[LapTimesScraper] Mapped FastestLaps slug "${fastestLapsSlug}" -> AutoRev slug "${autoRevSlug}"`);
  }
  ...
  if (!track) {
    return { success: false, error: `Could not resolve/create track: ${fastestLapsSlug} (mapped to ${autoRevSlug})` };
  }
