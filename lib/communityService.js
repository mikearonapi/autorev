...
export async function fetchCommunityPostBySlug(slug) {
  ...
  try {
    ...
    // Note: carImageUrl is already included in result.post from the SQL function
    // Do NOT override it here - result.car_image_url doesn't exist as a top-level column
    return {
      data: {
        ...result.post,
 ...
 * 
 * Uses the dedicated get_most_viewed_builds RPC which sorts by view_count DESC
 * at the database level, ensuring correct results regardless of limit.
 * 
 * @param {Object} options - Query options
 * @returns {Promise<{data: Array|null, error: Error|null}>}
 */
export async function fetchMostViewedBuilds(options = {}) {
  ...
  try {
    // Use the dedicated RPC that sorts by view_count DESC at database level
    // This ensures we get the truly most-viewed builds, not just the most recent
    ...
    // RPC returns data already sorted by view_count DESC, no client-side sort needed
    return { data: data || [], error: null };
