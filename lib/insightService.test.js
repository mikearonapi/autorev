...
describe('Insight Service', () => {
  ...
  describe('calculateBuildProgress', () => {
    ...
    it('computes currentHpGain from installed mods and targetHpGain from planned mods (SOURCE OF TRUTH)', () => {
      const progress = calculateBuildProgress(mockVehicles, mockProjects);
      
      const vehicle1Progress = progress.find(p => p.vehicleId === 'vehicle-1');
      
      // SOURCE OF TRUTH: current HP gain = installed_modifications (dynamic calculation)
      const expectedCurrent = calculateAllModificationGains(mockVehicles[0].installed_modifications, null).hpGain;
      assert.strictEqual(vehicle1Progress.currentHpGain, expectedCurrent);
      
      // SOURCE OF TRUTH: target HP gain = selected_upgrades on the active build (dynamic calculation)
      const plannedKeys = normalizeUpgradeKeys(mockProjects[0].selected_upgrades);
      const expectedTarget = calculateAllModificationGains(plannedKeys, null).hpGain;
      assert.strictEqual(vehicle1Progress.targetHpGain, expectedTarget);
    });
  ...
  describe('calculatePerformanceSummary', () => {
    it('sums HP gain from installed mods (dynamic) across vehicles', () => {
      const vehicles = [
        {
          id: 'v1',
        ...
        },
        {
          ...
          installed_modifications: ['intake'],
          garage_score: 70,
        },
      ];
      ...
      const expected = 
        calculateAllModificationGains(['stage1-tune'], null).hpGain +
        calculateAllModificationGains(['intake'], null).hpGain;
      assert.strictEqual(summary.totalHpGain, expected);
    });

    it('does not count same project twice for investment totals', () => {
      const vehicles = [
      ...
      const projects = [
        { id: 'build-a', car_slug: 'car-a', total_hp_gain: 100, total_cost_low: 1000, total_cost_high: 1200 },
      ];

      const summary = calculatePerformanceSummary(vehicles, projects);

      assert.deepStrictEqual(summary.estimatedInvestment, { low: 1000, high: 1200 });
    });
