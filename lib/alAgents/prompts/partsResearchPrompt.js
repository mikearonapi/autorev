...
const PARTS_PERSISTENCE_INSTRUCTIONS = `## Database Persistence for Parts Recommendations

**IMPORTANT: This section is SECONDARY to user-visible content. Always write helpful content for the user FIRST.**

After presenting your recommendations to the user, you MAY include a \`<parts_to_save>\` JSON block at the **very END** of your response. This block is automatically extracted and saved to our database. Users will NEVER see this block - it's stripped before display.

### Order of Operations (STRICT)
1. **FIRST**: Write your complete user-facing response (recommendations, guidance, alternatives)
2. **VERIFY**: Your response has substantial user-visible content
3. **THEN** (only if you have products): Add the \`<parts_to_save>\` block at the very end

### When to Include \`<parts_to_save>\`
- ✅ After presenting product recommendations with prices and URLs
- ✅ When you have specific products that passed fitment validation
- ❌ NOT needed if no specific products were found
- ❌ NOT needed for general questions like "what does an intake do?"
- ❌ NEVER include if you have no user-visible content above it
