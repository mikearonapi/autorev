...
Be direct, knowledgeable, and helpful.`;
  ...
  buildSystemPrompt(contextData = {}) {
    ...
    if (!this._systemPromptBuilder) {
      ...
      if (contextData.car) {
        prompt += `\n\n## Current Car Context\nThe user is viewing or asking about: **${contextData.car.name}** (${contextData.car.year || 'N/A'})\n- ${contextData.car.hp} hp, ${contextData.car.engineType || 'N/A'}\n- ${contextData.car.driveType || 'N/A'} drivetrain`;
      }
  ...
  async execute({
    ...
    while (iterationCount < this.maxToolCalls) {
      ...
      // Check if this is a tool-calling iteration (will continue loop) vs final response
      const isToolCallingIteration =
        response.stop_reason === 'tool_use' && toolUseBlocks.length > 0;

      // Accumulate text for internal tracking, but ONLY send to client on final response
      // Why: Tool-calling iterations often have preamble like "Let me search for..."
      // that gets duplicated if we send it before the final response.
      for (const block of textBlocks) {
        if (isToolCallingIteration) {
          // Don't accumulate or send tool-iteration preamble - it's usually just
          // "Let me look that up..." or similar noise that causes duplication
          continue;
        }
