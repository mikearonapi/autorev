...
4. **Short follow-ups:** "Yes", "OK", "Thanks" → generalist
...
const PARTS_KEYWORDS = [
  // Intake/Exhaust
  'intake', 'cold air intake', 'cai', 'exhaust', 'catback', 'cat-back', 'downpipe', 'dp',
  'header', 'headers', 'manifold', 'muffler', 'resonator', 'midpipe', 'test pipe',
  // Engine/Power
  'tune', 'ecu', 'flash', 'piggyback', 'tuner', 'turbo', 'turbocharger', 'supercharger',
  'blower', 'intercooler', 'fmic', 'bov', 'blow off valve', 'wastegate', 'boost controller',
  'fuel pump', 'injector', 'fuel rail', 'catch can', 'oil catch', 'throttle body',
  // Suspension/Handling
  'coilover', 'coil-over', 'spring', 'lowering spring', 'sway bar', 'strut', 'shock',
  'control arm', 'bushing', 'end link', 'camber', 'toe arm', 'subframe',
  // Brakes
  'brake pad', 'rotor', 'caliper', 'brake line', 'ss lines', 'stainless lines',
  'brake fluid', 'big brake kit', 'bbk', 'master cylinder',
  // Wheels/Tires
  'wheel', 'rim', 'tire', 'spacer', 'lug', 'hub',
  // Drivetrain
  'clutch', 'flywheel', 'short shifter', 'shift knob', 'diff', 'differential', 'lsd',
  'driveshaft', 'axle', 'cv', 'halfshaft',
  // Fluids/Filters
  'oil filter', 'air filter', 'fuel filter', 'transmission fluid', 'gear oil', 'coolant',
  // Other parts
  'spark plug', 'ignition coil', 'battery', 'alternator', 'pulley', 'belt',
  'splitter', 'diffuser', 'wing', 'spoiler', 'lip', 'bumper', 'fender', 'hood',
  'seat', 'harness', 'roll bar', 'cage', 'steering wheel', 'pedal',
];
...
const SHOPPING_ACTIONS = [
  'find', 'search', 'buy', 'purchase', 'shop', 'looking for', 'need a', 'need an',
  'where to get', 'where to buy', 'where can i get', 'where can i buy',
  'best', 'top', 'recommend', 'recommendation', 'suggestions', 'options',
  'cheapest', 'affordable', 'budget', 'premium', 'quality',
];
 ...
 * 2. Short follow-ups (Generalist)
 * 3. Platform questions (Generalist)
 * 4. Jokes/chitchat (Generalist)
 * 5. Parts Research (BEFORE Build Planning - catches "stage X tune")
 * 6. Build Planning (strategy, not specific products)
 * 7. Community Events
 * 8. Knowledge (educational)
 * 9. Performance Data
 * 10. Car Discovery (broad catch-all for car questions)
 */
function quickClassify(text, hasImage) {
  ...
  if (lower.length < 15) {
    ...
    if (/^(yes|no|ok|okay|sure|thanks|thank you|yep|nope|go on|continue|more|please|got it|perfect|great|cool|nice|awesome)\.?$/i.test(lower)) {
      return { intent: INTENT_TYPES.GENERALIST, confidence: 0.95, reasoning: 'Short follow-up' };
    }
    // Questions that are too short to classify
    if (lower.length < 8 && !/\?/.test(lower)) {
      return { intent: INTENT_TYPES.GENERALIST, confidence: 0.7, reasoning: 'Too short to classify' };
    }
  }
  ...
  // 3. PLATFORM QUESTIONS → Generalist
  // =========================================================================
  if (/how (do i|does|to) use autorev|autorev (help|guide|tutorial)|garage score|earn points|my points|subscription|tier|plan/i.test(lower)) {
  ...
  // 4. JOKES/CHITCHAT → Generalist
  // =========================================================================
  if (/\b(joke|funny|laugh|kidding|humor)\b|tell me a (joke|story|fun)|make me laugh/i.test(lower)) {
  ...
  if (/\b(how much (hp|horsepower|power|torque|whp|wtq)|power gains?|hp gains?|torque gains?)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.PERFORMANCE_DATA, confidence: 0.88, reasoning: 'Power calculation' };
  }
  
  // "What power gains can I expect" - expectation queries
  if (/\b(what|how much).*\b(gain|gains|expect)\b.*\b(from|with)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.PERFORMANCE_DATA, confidence: 0.85, reasoning: 'Power expectation query' };
  }

  if (/\b(0-60|0 to 60|zero to sixty|quarter mile|1\/4 mile|trap speed)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.PERFORMANCE_DATA, confidence: 0.88, reasoning: 'Acceleration data' };
  }
  ...
  if (/research_parts_live|USE THE research_parts_live/i.test(lower)) {
    return { intent: INTENT_TYPES.PARTS_RESEARCH, confidence: 0.98, reasoning: 'Explicit parts research tool' };
  }

  // Pattern 2: "Stage X tune/ECU" - looking for a specific product
  if (/stage\s*[123]\s*(tune|ecu|flash|software|remap)/i.test(lower)) {
    return { intent: INTENT_TYPES.PARTS_RESEARCH, confidence: 0.9, reasoning: 'Stage tune product search' };
  }
  ...
  if (/(?:top|best)\s*\d+\s+\w+/i.test(lower)) {
    ...
    if (hasPartKeywordInTopN) {
      return { intent: INTENT_TYPES.PARTS_RESEARCH, confidence: 0.9, reasoning: 'Top N parts list' };
    }
  }
  ...
  if (hasShoppingAction && hasPartKeyword) {
    return { intent: INTENT_TYPES.PARTS_RESEARCH, confidence: 0.88, reasoning: 'Shopping for specific part' };
  }
  ...
  if (/\b(ready for|prepared for|set up for|can handle)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.BUILD_PLANNING, confidence: 0.88, reasoning: 'Build readiness check' };
  }
  ...
  if (hasPartKeyword && /\b(for my|for the|for a|for track|for street|for daily)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.PARTS_RESEARCH, confidence: 0.85, reasoning: 'Part fitment query' };
  }

  // Pattern 6: Brand + part mentions (e.g., "APR tune", "Akrapovic exhaust")
  const brandPatterns = /\b(apr|034|ecs|ie|integrated engineering|unitronic|cobb|awe|akrapovic|borla|magnaflow|k&n|aem|bilstein|kw|ohlins|brembo|stoptech|bc racing)\b/i;
  if (brandPatterns.test(lower) && hasPartKeyword) {
    return { intent: INTENT_TYPES.PARTS_RESEARCH, confidence: 0.85, reasoning: 'Brand + part query' };
  }

  // Pattern 6b: Brand + specific product name (even if not in parts list)
  // Catches "Cobb Accessport cost", "APR Plus pricing", etc.
  const specificProducts = /\b(accessport|jb4|mhd|bm3|ecutek|hp tuners|diablo|sct|procede|burger jb|piggyback)\b/i;
  if (brandPatterns.test(lower) || specificProducts.test(lower)) {
    ...
    if (/\b(price|cost|how much|pricing|msrp)\b/i.test(lower)) {
      return { intent: INTENT_TYPES.PARTS_RESEARCH, confidence: 0.85, reasoning: 'Product pricing query' };
    }
  }

  // Pattern 7: Price-focused part queries (but NOT "how much HP" which is performance)
  if (!(/how much (hp|horsepower|power)/i.test(lower)) && (
    ...
    return { intent: INTENT_TYPES.PARTS_RESEARCH, confidence: 0.85, reasoning: 'Part pricing query' };
  }
  ...
  if (/\b(my build|review my|what mods|mod order|install order|which mods|mod plan)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.BUILD_PLANNING, confidence: 0.88, reasoning: 'Build planning query' };
  }
  ...
  if (/\b(what order|which order)\b.*\b(install|do)\b.*\b(mod|mods|parts)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.BUILD_PLANNING, confidence: 0.88, reasoning: 'Install order query' };
  }
  ...
  if (/what stage (should|am|do|is)|which stage|ready for stage/i.test(lower)) {
    return { intent: INTENT_TYPES.BUILD_PLANNING, confidence: 0.85, reasoning: 'Build stage strategy' };
  }
  ...
  if (/\b(supporting mods?|what mods? (do i |does it )?need|mods? needed)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.BUILD_PLANNING, confidence: 0.88, reasoning: 'Supporting mods query' };
  }

  // Compatibility checks
  if (/\b(compatible|compatibility|work with|fit with|pair with|stack with)\b/i.test(lower) && 
      /\b(mod|mods|parts|tune|intake|exhaust)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.BUILD_PLANNING, confidence: 0.85, reasoning: 'Compatibility check' };
  }

  // Build progression
  if (/\b(next upgrade|next mod|what next|what's next|upgrade path|build path)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.BUILD_PLANNING, confidence: 0.85, reasoning: 'Build progression' };
  }
  ...
  if (/\b(track days?|cars and coffee|car shows?|autocross|autox|hpde|rally|cruise|meetups?|meet ups?|car meets?)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.COMMUNITY_EVENTS, confidence: 0.9, reasoning: 'Car event query' };
  }

  // Location-based event searches
  if (/\b(event|meet|show)\b.*\b(near|in|around|close to)\b/i.test(lower) ||
      /\b(near|in|around)\b.*\b(event|meet|show)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.COMMUNITY_EVENTS, confidence: 0.85, reasoning: 'Event location search' };
  }

  // Forum/community insights
  if (/\b(what do owners|owner experience|forum|community says|real world)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.COMMUNITY_EVENTS, confidence: 0.8, reasoning: 'Community insights' };
  }
  ...
  if (/\b(reliable|reliability|common issues?|known issues?|problems with|failures?|recalls?)\b/i.test(lower)) {
  ...
  if (/\b(compare|vs\.?|versus)\b/i.test(lower) && 
      /\b(911|gt3|cayman|boxster|m3|m4|c63|rs5|rs6|mustang|camaro|corvette|supra|z4|miata|wrx|sti|civic|type r|gtr|r35|s2000|nsx)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.CAR_DISCOVERY, confidence: 0.85, reasoning: 'Car comparison' };
  }

  // Car specs/info
  if (/\b(tell me about|specs|specifications|horsepower|weight|msrp|price range)\b.*\b(car|911|gt3|m3|rs5|mustang|corvette)\b/i.test(lower)) {
  ...
  // Buying advice
  if (/\b(should i buy|worth buying|good daily|daily driver|weekend car|buy a|buying a|looking at a)\b/i.test(lower)) {
  ...
  if (/\b(best|good)\s+(car|sports car|daily|track car|weekend car)\b/i.test(lower)) {
    return { intent: INTENT_TYPES.CAR_DISCOVERY, confidence: 0.82, reasoning: 'Car recommendation' };
  }
  ...
  if (/^(what is\s|what are\s|how does\s|how do\s|explain\s|teach me about\s|learn about\s)/i.test(lower) &&
      !/what is the best|what is my|what are the best|what are my|common issues|known issues/i.test(lower)) {
    return { intent: INTENT_TYPES.KNOWLEDGE, confidence: 0.85, reasoning: 'Educational question' };
  }

  // "Difference between X and Y"
  if (/difference between|versus|vs\.?\s+/i.test(lower) && 
      /\b(turbo|supercharger|coilover|spring|fwd|rwd|awd|lsd|open diff|dct|manual|auto)\b/i.test(lower)) {
  ...
  if (/\b(how (does|do)|why (does|do)|what (causes|makes))\b.*\b(boost|compression|timing|valve|cam|piston|crank|torque curve)\b/i.test(lower)) {
...
async function classifyWithLLM(text, conversationHistory = [], observability = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[Orchestrator:${correlationId}] LLM API error: ${response.status} (${durationMs}ms)`);
    ...
    console.warn(`[Orchestrator:${correlationId}] LLM response not valid JSON: "${responseText.substring(0, 100)}"`);
  } catch (e) {
