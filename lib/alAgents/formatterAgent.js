...
function normalizeCanonicalFormat(text) {
  ...
  // Standardize "Quick Buying Guide" → "Quick Picks" for consistency
  normalized = normalized.replace(/###?\s*Quick Buying Guide\s*$/gm, '### Quick Picks');
  normalized = normalized.replace(/^\*\*Quick Buying Guide:?\*\*\s*$/gm, '### Quick Picks');
  ...
  // ==========================================================================
  // CURRENCY NORMALIZATION (Standardize to USD-first display)
  // ==========================================================================

  // Convert "€XXX (~$YYY)" → "$YYY (€XXX)" - USD first
  normalized = normalized.replace(/€(\d+(?:,\d{3})?)\s*\(~?\$(\d+(?:,\d{3})?)\)/g, '$$$2 (€$1)');

  // Convert "£XXX (~$YYY)" → "$YYY (£XXX)" - USD first
  normalized = normalized.replace(/£(\d+(?:,\d{3})?)\s*\(~?\$(\d+(?:,\d{3})?)\)/g, '$$$2 (£$1)');

  // Convert standalone "€XXX" with approximate USD (rough 1.08 rate)
  // Only for prices that appear in price contexts (after "Price:" or in parentheses)
  normalized = normalized.replace(/\*\*Price:\*\*\s*€(\d+)/g, (match, euros) => {
    const usd = Math.round(parseInt(euros) * 1.08);
    return `**Price:** $${usd} (€${euros})`;
  });

  // Convert standalone "£XXX" with approximate USD (rough 1.27 rate)
  normalized = normalized.replace(/\*\*Price:\*\*\s*£(\d+)/g, (match, pounds) => {
    const usd = Math.round(parseInt(pounds) * 1.27);
    return `**Price:** $${usd} (£${pounds})`;
  });

  // ==========================================================================
  // RECOMMENDED TAG NORMALIZATION
  // ==========================================================================

  // Ensure first item in a Top N list has RECOMMENDED tag if missing
  // Pattern: **1) Brand Product** without RECOMMENDED → **1) Brand Product** - RECOMMENDED
  // Only add if there's no RECOMMENDED anywhere in the line
  normalized = normalized.replace(
    /^(\*\*1\)\s+[^*\n]+\*\*)(?!\s*-\s*RECOMMENDED)(\s*)$/gm,
    '$1 - RECOMMENDED$2'
  );
