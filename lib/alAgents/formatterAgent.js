...
const FALLBACK_MESSAGES = {
  ...
  parts_research: `## Parts Research

I searched for parts but couldn't find specific products matching your criteria. Here's how to proceed:

**Recommended Next Steps:**
1. **Try different search terms** - Use exact part names or numbers
2. **Check manufacturer sites** - Many have fitment tools
3. **Provide more details** - Year, make, model, and trim help narrow results

**Popular Parts Vendors:**
- **034 Motorsport** - European performance parts
- **ECS Tuning** - Wide selection
- **FCP Euro** - OEM and performance with warranty

Would you like me to search for something more specific?`,

  // Fallback for general queries
  default: `I apologize, but I wasn't able to generate a complete response. This might be because:
 ...
 */
function getContextualFallback(rawContent) {
  // Check if this was likely a parts query based on internal blocks
  if (rawContent?.includes('<parts_to_save>') || rawContent?.includes('upgrade_key')) {
...
export function createStreamFormatter({ correlationId, userId, onText }) {
  ...
  return {
    ...
    async processContent(content) {
      ...
      const cleanedContent = stripInternalBlocks(content);
...
function startsWithContent(text) {
  ...
  const contentPatterns = [
    ...
    // Structured response markers
    /^RESPONSE_TYPE:/i, // Structured format
    /^ITEM\s+\d+:/i, // Item list
    /^TITLE:/i, // Titled content
