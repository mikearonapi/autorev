...
export const AL_TOOLS = [
  ...
  },
  {
    ...
    description: 'Search the AutoRev encyclopedia for information about modifications, car systems, components, or build guides. Use this when users ask how things work or about specific upgrades.',
    input_schema: {
      type: 'object',
      properties: {
        query: {
        ...
        category: {
          type: 'string',
          enum: ['modifications', 'systems', 'components', 'build_guides', 'all'],
...
- **Car Database**: ${Object.keys(context.carCount || 150)}+ sports cars with detailed specs, scores, and ownership data
...
2. **Car Questions**: Use get_car_details for specs, get_expert_reviews for what experts say, get_known_issues for problems
...
- **Be Concise**: ${AL_IDENTITY.voice.length}
...
export function getAvailableToolsForQuery(planId, query) {
  ...
  if (domains.includes('comparison')) {
    priorityTools.push('compare_cars', 'search_cars', 'get_car_details');
  }
  if (domains.includes('modifications')) {
    priorityTools.push('get_upgrade_info', 'search_encyclopedia', 'recommend_build');
  }
  if (domains.includes('reliability')) {
    priorityTools.push('get_known_issues', 'get_expert_reviews');
  }
  if (domains.includes('maintenance')) {
    priorityTools.push('get_maintenance_schedule', 'get_car_details');
  }
  if (domains.includes('buying')) {
    priorityTools.push('get_car_details', 'get_known_issues', 'search_forums', 'get_expert_reviews');
  }
  if (domains.includes('track')) {
    priorityTools.push('recommend_build', 'get_expert_reviews', 'get_car_details');
  }
