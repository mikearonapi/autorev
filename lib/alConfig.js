...
export const AL_TOOLS = [
    ...
    input_schema: {
      ...
      properties: {
        ...
        upgrade_type: {
          type: 'string',
          description:
            'Upgrade category for recommendations: intake, exhaust, downpipe, coilovers, brakes, tune, turbo. Requires car_slug.',
        },
        budget: {
          type: 'string',
    ...
    input_schema: {
      ...
      properties: {
        ...
        car_slug: {
          type: 'string',
          description:
            'Optional: AutoRev car slug if known (e.g., "ram-trx-2022"). If provided, year/make/model can be omitted.',
        },
        upgrade_type: {
          type: 'string',
          description:
  ...
  },
  {
    name: 'search_events',
    description:
      'Search for car events like track days, Cars & Coffee, car shows, autocross, and meetups. Can filter by location, event type, date range, and car brand/model. Use this when users ask about finding events near them or for specific car brands.',
    input_schema: {
      type: 'object',
      properties: {
        location: {
          type: 'string',
          description:
            'Location to search near. Can be ZIP code (5 digits), city name, city/state (e.g., "Fairfax, VA"), or state name.',
        },
        radius: {
          type: 'number',
    ...
    input_schema: {
      ...
      properties: {
        ...
        car_slug: {
          type: 'string',
          description:
            'Optional car slug for context if the user is asking about a specific vehicle.',
        },
      },
    },
  },
...
3. Include \`<parts_to_save>\` JSON at the END of your response (see analysis_instructions in tool output)
