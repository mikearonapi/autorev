...
export const MODEL_TIERS = {
  ...
  standard: {
    ...
    domains: ['specifications', 'troubleshooting', 'comparison', 'maintenance', 'modifications', 'parts'],
...
export function selectModelForQuery(query, detectedDomains = []) {
  ...
  if (detectedDomains.length > 0) {
    ...
    const hasComplexDomain = detectedDomains.some(d => 
      MODEL_TIERS.complex.domains.includes(d)
    );
    if (hasComplexDomain) {
      return MODEL_TIERS.complex;
    }
...
`);
  ...
  if (drivingFocus.length > 0) {
    const focusMap = {
      power: 'power/acceleration', handling: 'handling/cornering', daily: 'daily comfort',
      track: 'track performance', show: 'aesthetics',
    };
  ...
  if (commute) {
    const commuteMap = {
      short_city: 'short city commutes', long_city: 'longer city driving',
      highway: 'mostly highway miles', mixed: 'mixed city/highway',
  ...
  if (primaryGoals.length > 0) {
    const goalMap = {
      more_power: 'more power', better_handling: 'better handling',
      reliability: 'reliability', sound: 'better sound', aesthetics: 'aesthetics',
    };
  ...
  if (wantToLearn.length > 0) {
    const learnMap = {
      power: 'making power', handling: 'suspension/handling', braking: 'brakes',
      aero: 'aerodynamics', tuning: 'ECU tuning', maintenance: 'maintenance/DIY',
    };
...
export function getAvailableToolsForQuery(planId, query) {
  ...
  const availableTools = plan.toolAccess === 'all' 
