...
function computeSummary(data) {
  ...
  if (data.safety.iihs) {
    ...
    summary.safety.topSafetyPick =
      data.safety.iihs.topSafetyPick || data.safety.iihs.topSafetyPickPlus;
  }
  ...
  if (priceSources.length > 0) {
    ...
    summary.pricing.consensusPrice = Math.round(prices.reduce((a, b) => a + b, 0) / prices.length);
  ...
  const ratings = [summary.reviews.carAndDriverRating, summary.reviews.motorTrendRating].filter(
    Boolean
  );

  if (ratings.length > 0) {
    summary.reviews.averageRating =
...
export async function batchAggregateCarData(cars, options = {}) {
  ...
  for (let i = 0; i < cars.length; i += concurrency) {
    ...
    const batchResults = await Promise.all(batch.map((car) => aggregateCarData(car, options)));
