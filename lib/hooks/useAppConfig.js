...
import { 
  ...
  preloadConfigs,
} from '@/lib/configClient.js';

// All config keys we need to preload
const CONFIG_KEYS = [
  'tierConfig',
  'categories', 
  'recommendationTypes',
  ...
  useEffect(() => {
    ...
    async function loadConfigs() {
      try {
        ...
        if (mounted) {
          setConfig({
            tierConfig: tierConfig || DEFAULT_CONFIG.tierConfig,
            categories: categories || [],
            recommendationTypes: recommendationTypes || [],
            priorityDescriptors: priorityDescriptors || {},
            performanceCategories: performanceCategories || [],
            upgradeTiers: upgradeTiers || {},
            upgradeModuleCategories: upgradeModuleCategories || [],
            isLoading: false,
            error: null,
          });
        }
