...
export function generateWelcomeEmailHtml(vars, baseUrl) {
  ...
  const carCount = vars.car_count || 310;
          ...
          <tr>
            <td class="content" style="padding: 32px 40px 40px 40px;">
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                  <td align="center">
                    <a href="${confirmationUrl}" target="_blank" style="display: inline-block; background-color: #111827; color: #ffffff; font-size: 16px; font-weight: 600; text-decoration: none; padding: 16px 48px; border-radius: 8px; border: 1px solid #111827;">
                      Confirm My Email →
                    </a>
                  </td>
                </tr>
...
Click the link below to verify and start exploring sports car specs, reliability data, and a community that celebrates the passion—not the price tag.
    ...
    <tr>
      <td align="center" class="wrapper" style="padding: 40px 20px;">
        ...
        <table role="presentation" class="container" cellspacing="0" cellpadding="0" border="0" width="600" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
          ...
          <tr>
            <td align="center" style="padding: 40px 0 32px 0; border-bottom: 1px solid #f3f4f6;">
              <img src="${EMAIL_CONFIG.baseUrl}/images/autorev-email-logo.png" alt="AutoRev" width="60" height="60" style="display: block;">
            </td>
          </tr>
    ...
    <tr>
      <td align="center" class="wrapper" style="padding: 40px 20px;">
        ...
        <table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #ffffff; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);">
          ...
          <tr>
            <td align="center" style="padding: 40px 0 32px 0; border-bottom: 1px solid #f3f4f6;">
              <img src="${EMAIL_CONFIG.baseUrl}/images/autorev-email-logo.png" alt="AutoRev" width="60" height="60" style="display: block; margin-bottom: 16px;">
...
© ${new Date().getFullYear()} AutoRev · Built for enthusiasts`,
  ...
  'feedback-response': {
    ...
    render: (vars) => {
      ...
      // Feedback type determines content
      const feedbackType = vars.feedback_type || 'general'; // 'car_request' | 'general' | 'feature_request'
      const carName = vars.car_name || null; // e.g., "2024 BMW M3 CS"
      const carSlug = vars.car_slug || null; // e.g., "bmw-m3-cs-2024"
      const carImageUrl = vars.car_image_url || null; // Hero image URL from database
      const carDetailUrl = carSlug ? `${baseUrl}/browse-cars/${carSlug}` : null;
      const originalFeedback = vars.original_feedback || null;
      
      // Dynamic content based on feedback type
      let preheaderText, headlineText, bodyText, ctaText, ctaUrl;
      
      if (feedbackType === 'car_request' && carName && carDetailUrl) {
        preheaderText = `${carName} is now in our database — check it out!`;
        headlineText = `${carName} is live! 🚗`;
        bodyText = `You asked, we delivered. The <strong>${carName}</strong> has been added to AutoRev with full specs, reliability data, and everything you need to know.`;
        ctaText = `View ${carName} →`;
        ctaUrl = carDetailUrl;
      } else if (feedbackType === 'feature_request') {
        preheaderText = `We've addressed your feature request — thank you!`;
        headlineText = `We heard you! 🛠️`;
        bodyText = `Thanks for taking the time to share your thoughts. We've reviewed your feedback and made improvements based on what you told us.`;
        ctaText = `See What's New →`;
        ctaUrl = `${baseUrl}/changelog`;
      } else {
        preheaderText = `We've addressed your feedback — thank you for helping us improve!`;
        headlineText = `Thanks for speaking up! 💬`;
        bodyText = `Your feedback matters. We've reviewed what you shared and taken action to make AutoRev better for everyone.`;
      ...
      const feedbackQuoteHtml = originalFeedback ? `
                ...
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f8fafc; border-radius: 8px; border-left: 4px solid #D4AF37; margin-bottom: 24px;">
                  <tr>
...
` : '';
      ...
      // Build car preview section for car requests
      const carPreviewHtml = (feedbackType === 'car_request' && carName && carDetailUrl) ? `
                <!-- Car Preview Card -->
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #f8fafc; border-radius: 12px; margin-bottom: 24px; overflow: hidden;">
                  ${carImageUrl ? `
                  <tr>
                    <td>
                      <a href="${carDetailUrl}" target="_blank">
                        <img src="${carImageUrl}" alt="${carName}" width="520" style="display: block; width: 100%; max-width: 520px; height: auto; border-radius: 12px 12px 0 0;">
                      </a>
                    </td>
                  </tr>
                  ` : ''}
                  <tr>
                    <td style="padding: 20px 24px;">
                      <h3 style="margin: 0 0 8px 0; font-size: 20px; font-weight: 700; color: #111827;">${carName}</h3>
                      <p style="margin: 0 0 16px 0; font-size: 15px; line-height: 24px; color: #6b7280;">Now available with full specs, reliability info, and more.</p>
                      <a href="${carDetailUrl}" style="display: inline-block; font-size: 15px; font-weight: 600; color: #1a4d6e; text-decoration: none;">
                        View Full Details →
                      </a>
                    </td>
                  </tr>
      ...
      const carPreviewText = (feedbackType === 'car_request' && carName && carDetailUrl) ? `
🏎️ ${carName}
Now available with full specs, reliability info, and more.

View Full Details: ${carDetailUrl}
            ...
            <td class="content" style="padding: 32px 40px 40px 40px;">
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color: #fffbeb; border-radius: 8px; border: 1px solid #fcd34d; margin-bottom: 24px;">
                <tr>
                  <td style="padding: 16px;">
                    <p style="margin: 0; font-size: 15px; line-height: 24px; color: #92400e; text-align: center; font-weight: 500;">
                      Brotherhood over gatekeeping. Thanks for being part of the AutoRev community.
                    </p>
                  </td>
                </tr>
              </table>
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                  <td align="center">
                    <a href="${ctaUrl}" target="_blank" style="display: inline-block; background-color: #111827; color: #ffffff; font-size: 16px; font-weight: 600; text-decoration: none; padding: 16px 48px; border-radius: 8px; border: 1px solid #111827;">
                      ${ctaText}
                    </a>
                  </td>
                </tr>
              ...
              <!-- Secondary Links -->
              <p style="margin: 24px 0 0 0; font-size: 14px; line-height: 22px; color: #9ca3af; text-align: center;">
                Have more feedback? <a href="${baseUrl}/contact" style="color: #4b5563; text-decoration: underline;">We'd love to hear it</a>
              </p>
 ...
 * @param {string} options.feedbackType - Type of feedback: 'car_request' | 'general' | 'feature_request'
 * @param {string} options.carName - Car name for car_request type (e.g., "2024 BMW M3 CS")
 * @param {string} options.carSlug - Car slug for detail page link (e.g., "bmw-m3-cs-2024")
 * @param {string} options.carImageUrl - Hero image URL for the car (optional)
 * @param {string} options.originalFeedback - The original feedback text (optional)
...
export async function sendFeedbackResponseEmail({ 
  ...
  // Validate car slug exists and fetch image URL if needed (for car_request type)
  let validatedSlug = carSlug;
  let validatedImageUrl = carImageUrl;
  let validatedCarName = carName;
  
  if (feedbackType === 'car_request' && carSlug) {
    try {
      const { data: car, error } = await supabaseAdmin
        .from('cars')
        .select('slug, name, image_hero_url')
        .eq('slug', carSlug)
        .single();
      
      if (error || !car) {
        // Slug doesn't exist - try to find a similar one
        const { data: similarCars } = await supabaseAdmin
          .from('cars')
          .select('slug, name, image_hero_url')
          .or(`name.ilike.%${carName?.split(' ').slice(-1)[0] || ''}%,slug.ilike.%${carSlug?.replace(/-/g, '%')}%`)
          .limit(1);
        
        if (similarCars && similarCars.length > 0) {
          console.warn(`[Email] Car slug "${carSlug}" not found, using similar: "${similarCars[0].slug}"`);
          validatedSlug = similarCars[0].slug;
          validatedCarName = similarCars[0].name;
          validatedImageUrl = validatedImageUrl || similarCars[0].image_hero_url;
        } else {
          console.error(`[Email] Car slug "${carSlug}" not found and no similar cars found. Email may have broken link.`);
        }
      } else {
        // Use verified data from database
        validatedSlug = car.slug;
        validatedCarName = validatedCarName || car.name;
        validatedImageUrl = validatedImageUrl || car.image_hero_url;
      }
    } catch (err) {
      console.error(`[Email] Error validating car slug: ${err.message}`);
    }
  }
  ...
  return sendTemplateEmail({
    ...
    variables: {
      ...
      car_name: validatedCarName,
      car_slug: validatedSlug,
      car_image_url: validatedImageUrl,
...
export async function processEmailQueue(limit = 50) {
  ...
  const QUEUE_COLS = 'id, user_id, email_type, to_email, subject, html_content, text_content, metadata, status, priority, scheduled_for, attempts, last_error, sent_at, created_at';
    ...
    <tr>
      <td align="center">
        <table width="100%" style="max-width: 600px; background-color: #ffffff; border-radius: 12px; overflow: hidden;">
          <!-- Header -->
          <tr>
            <td style="background: linear-gradient(135deg, #0d1b2a 0%, #1b263b 100%); padding: 32px; text-align: center;">
              <h1 style="margin: 0; color: #ffffff; font-size: 24px;">
                <span style="color: #ffffff;">AUTO</span><span style="color: #d4ff00;">REV</span>
              </h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 32px;">
              <h2 style="margin: 0 0 16px; color: #0d1b2a; font-size: 24px;">
                Hey ${firstName}, your trial ends ${daysRemaining === 1 ? 'tomorrow' : `in ${daysRemaining} days`}
              </h2>
              
              <p style="margin: 0 0 24px; color: #64748b; font-size: 16px; line-height: 1.6;">
                Your ${tierName} trial will end on <strong>${trialEndDate}</strong>. After that, you'll be moved to the Free tier unless you subscribe.
              </p>
              ...
              <div style="background-color: #f8fafc; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                ...
                <ul style="margin: 0; padding-left: 20px; color: #64748b;">
                  ${tier === 'tuner' ? `
                    <li>Unlimited AL (AI Mechanic) questions</li>
                    <li>Build planning and parts shopping</li>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="${EMAIL_CONFIG.baseUrl}/pricing" style="display: inline-block; padding: 14px 32px; background-color: #d4ff00; color: #0d1b2a; text-decoration: none; font-weight: 600; border-radius: 8px; font-size: 16px;">
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <p style="margin: 0 0 24px; color: #475569; font-size: 16px; line-height: 1.6;">
                Your 7-day ${tierName} trial has come to an end. We hope you enjoyed exploring all the premium features!
              </p>
        ...
        <table width="100%" style="max-width: 600px; background-color: #ffffff; border-radius: 12px; overflow: hidden;">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #dc2626 0%, #b91c1c 100%); padding: 32px; text-align: center;">
              <h1 style="margin: 0; color: #ffffff; font-size: 24px;">
                <span style="color: #ffffff;">AUTO</span><span style="color: #fef08a;">REV</span>
              </h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding: 32px;">
              ...
              <div style="text-align: center; margin-bottom: 24px;">
                <div style="display: inline-block; width: 64px; height: 64px; background-color: #fef2f2; border-radius: 50%; line-height: 64px; font-size: 32px;">
                  ⚠️
                </div>
              </div>
              
              <h2 style="margin: 0 0 16px; color: #0d1b2a; font-size: 24px; text-align: center;">
                Payment Failed
              </h2>
              
              <p style="margin: 0 0 24px; color: #64748b; font-size: 16px; line-height: 1.6; text-align: center;">
                Hey ${firstName}, we couldn't process your payment of ${amountFormatted} for your ${tierName} subscription.
              </p>
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    <a href="${updatePaymentUrl}" style="display: inline-block; padding: 14px 32px; background-color: #dc2626; color: #ffffff; text-decoration: none; font-weight: 600; border-radius: 8px; font-size: 16px;">
