...
export async function signInWithGoogle(redirectTo) {
  ...
  // Use NEXT_PUBLIC_SITE_URL if set (for local dev), otherwise use current origin
 ...
 * Redirects to Facebook for authentication, then back to the app
 * 
 * @param {string} redirectTo - URL to redirect to after sign in (optional)
 * @returns {Promise<{error: Error|null}>}
 */
...
export async function getUserProfile() {
  ...
  if (!isSupabaseConfigured || !supabase) {
    console.warn('[auth] getUserProfile - Supabase not configured');
    return { data: null, error: new Error('Supabase not configured') };
  }
  ...
  try {
    ...
    const PROFILE_COLUMNS = 'id, display_name, avatar_url, bio, subscription_tier, subscription_ends_at, stripe_customer_id, stripe_subscription_id, stripe_subscription_status, referral_code, referred_by_user_id, onboarding_completed_at, onboarding_step, referral_source, referral_source_other, user_intent, email_opt_in_features, email_opt_in_events, location_zip, location_city, location_state, preferred_units, email_notifications, selected_title, unlocked_titles, total_points, public_slug, is_garage_public, created_at, updated_at';
    
    const { data, error } = await supabase
      .from('user_profiles')
      .select(PROFILE_COLUMNS)
      .eq('id', user.id)
      .maybeSingle(); // tolerate missing row; will be created on demand in AuthProvider

    const duration = Date.now() - startTime;
    console.log('[auth] getUserProfile query result:', {
      hasData: !!data,
    ...
    const duration = Date.now() - startTime;
    console.error('[auth] getUserProfile exception:', {
      error: err.message,
      durationMs: duration,
    });
