 ...
 * Uses 'global' scope to ensure server session is invalidated.
 * Also clears cookies manually as fallback.
 * 
 * @param {Object} options - Sign out options
 * @param {'global'|'local'|'others'} options.scope - Scope of sign out (default: 'global')
 * @returns {Promise<{error: Error|null}>}
 */
export async function signOut({ scope = 'global' } = {}) {
  ...
  try {
    ...
    // Using 'global' to invalidate the session on the server
    const { error } = await supabase.auth.signOut({ scope });
    
    // Even if sign out returns an error (e.g., stale session), 
    // we still want to clear local state
    if (error) {
      console.warn('[Auth] Sign out returned error:', error.message);
    } else {
      console.log('[Auth] Supabase signOut succeeded');
    }
    
    // Force clear any remaining auth tokens from localStorage
    // This ensures clean state even if server signOut failed
    if (typeof window !== 'undefined') {
      ...
      // Also clear ALL Supabase auth cookies manually
      // @supabase/ssr stores auth in cookies, and sometimes they don't get cleared
      ...
      // Nuclear option: clear ALL cookies that start with 'sb-'
      const allCookies = document.cookie.split(';');
      allCookies.forEach(cookie => {
        const cookieName = cookie.split('=')[0].trim();
        if (cookieName.startsWith('sb-')) {
          console.log('[Auth] Clearing Supabase cookie:', cookieName);
    ...
    return { error: null }; // Return success even if server had issues - local state is cleared
  } catch (err) {
    console.error('[Auth] Sign out exception:', err);
    
    // Even on exception, try to clear local storage and cookies
    if (typeof window !== 'undefined') {
      const keysToRemove = [];
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
