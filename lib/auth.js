...
export async function getUserProfile() {
  ...
  try {
    console.log('[auth] getUserProfile - getting current user...');
    const { data: { user } } = await supabase.auth.getUser();
    console.log('[auth] getUserProfile - getUser completed:', { hasUser: !!user, userId: user?.id?.slice(0, 8) });
    
    if (!user) {
      return { data: null, error: new Error('Not authenticated') };
    }

    console.log('[auth] getUserProfile - fetching profile from database...');
