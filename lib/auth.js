 ...
 * 3. Google redirects to Supabase's callback
 * 4. Supabase redirects to our /auth/callback with code
 * 5. Our callback exchanges code for session and redirects to `next`
 * 
 * @param {string} next - Path to redirect to AFTER successful auth (default: '/')
 *                        This is passed to /auth/callback?next=<path>
...
export async function signInWithGoogle(next = '/') {
  ...
  // Always redirect to our callback route first
  // The callback will then redirect to the `next` path after exchanging the code
  const callbackUrl = new URL('/auth/callback', window.location.origin);
  
  // Encode the final destination - callback route will redirect here after auth
  if (next && next !== '/') {
    callbackUrl.searchParams.set('next', next);
  }
  
  console.log('[Auth] Initiating Google OAuth, callback:', callbackUrl.toString());
