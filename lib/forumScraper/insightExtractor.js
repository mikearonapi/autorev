...
export class InsightExtractor {
   ...
   * @param {Object} [options.supabaseClient] - Supabase client to use (for scripts)
   */
  constructor(options = {}) {
  ...
  async extractFromThread(thread) {
    ...
    // Pre-filter posts for quality before sending to AI
    const filteredPosts = this._filterPostsForQuality(posts);
    
    if (filteredPosts.length === 0) {
      console.log(`[InsightExtractor] No quality posts found in thread: ${thread.thread_url}`);
      return [];
    }

    console.log(`[InsightExtractor] Processing ${filteredPosts.length}/${posts.length} quality posts from: ${thread.thread_title}`);

    // Prepare thread content for Claude with filtered posts
    const threadWithFilteredPosts = { ...thread, posts: filteredPosts };
    const threadContent = this._formatThreadForClaude(threadWithFilteredPosts);
  ...
  /**
   * Filter posts for quality before sending to AI
   * Removes trolls, spam, and low-value content to save tokens and improve results
   * @param {Array} posts - Raw posts from thread
   * @returns {Array} Filtered high-quality posts
   */
  _filterPostsForQuality(posts) {
    ...
    for (let i = 0; i < posts.length; i++) {
      ...
      // Always keep the original post (provides context)
      if (i === 0 || post.is_op) {
        filtered.push(post);
        continue;
      }
      
      // Skip very short posts (usually low value)
      if (contentLen < 50) continue;
      
      // Skip obvious troll/spam patterns
      const trollPatterns = [
        /lol{3,}|lmao{2,}|rofl+/i,
        /you('re| are) (wrong|stupid|idiot|dumb)/i,
        /this thread (sucks|is garbage)/i,
        /(gtfo|stfu)/i,
        /^\s*(same|agreed?|this|me too|\+1|x2|bump|subscribe|following)\s*$/i,
      ];
      ...
      // Skip sales posts
      const salesPatterns = [
        /pm\s*(sent|me)|selling|for sale|\bfs\b|wtb|wts/i,
        /\$\d+.*obo|best offer|price drop/i,
      ];
      ...
      // Boost: prioritize posts with technical content
      const hasTechnicalContent = 
        /\d+\s*(nm|ft-lb|psi|hp|kw)/i.test(content) ||  // Measurements
        /part\s*#?\s*\d+/i.test(content) ||              // Part numbers
        /diy|how[\s-]to|step[\s-]\d/i.test(content) ||  // Instructions
        /torque.*spec|service\s*interval/i.test(content);
      
      // Boost: prioritize experienced users
      const postCount = parseInt(post.author_post_count) || 0;
      const isExperiencedUser = postCount > 500;
      
      // Boost: ownership experience content
      const hasExperience = /i('ve| have) owned|my experience|after \d+ (years?|months?)|in my \d+k? miles/i.test(content);
      
      // Include post if it has positive signals or is from experienced user
      if (hasTechnicalContent || isExperiencedUser || hasExperience || contentLen > 300) {
        filtered.push(post);
      }
    }
    
    // Limit to most relevant posts to save tokens (keep first 20 quality posts)
...
Return your response as a JSON array of insights. Focus on extracting actionable, specific information that would be valuable to enthusiasts. If the thread contains no extractable insights, return [].`;
  ...
  async saveInsight(insight, thread) {
    ...
    const primaryCarSlug = carSlugs[0] || null;

    // Resolve car_slug to car_id (table uses car_id UUID, not car_slug text)
    let carId = null;
    if (primaryCarSlug) {
      const { data: car } = await client
        .from('cars')
        .select('id')
        .eq('slug', primaryCarSlug)
        .single();
      carId = car?.id || null;
    }
    ...
    const { data: savedInsight, error } = await client
      ...
      .insert({
        car_id: carId,  // Use car_id (UUID) not car_slug
        ...
        details: {
          ...
          all_car_slugs: carSlugs  // Store slugs in details for reference
        },
