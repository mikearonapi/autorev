...
export class ForumScraperService {
  ...
  async scrape(forumSlug, options = {}) {
    ...
    const { data: forumSource, error: sourceError } = await client
      .from('forum_sources')
      .select('*')
      .eq('slug', forumSlug)
      .single();

    if (sourceError || !forumSource) {
      throw new Error(`Forum source not found: ${forumSlug}`);
    }
    ...
    if (!adapter) {
      ...
      adapter = this.platformAdapters.get(forumSource.platform);
    }
    if (!adapter) {
      throw new Error(`No adapter registered for: ${forumSlug} (platform: ${forumSource.platform})`);
    }
