 ...
 * @property {string} carSlug - Car identifier
...
// In-memory cache for fitment data
const fitmentCache = new Map();
const CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

/**
 * Fetch fitment options for a car
 * @param {string} carSlug - Car slug identifier
 ...
 */
export async function fetchCarFitments(carSlug, { skipCache = false } = {}) {
  if (!carSlug) {
    return { data: null, error: new Error('Car slug is required') };
  }

  // Check cache
  if (!skipCache) {
    const cached = fitmentCache.get(carSlug);
    if (cached && Date.now() - cached.timestamp < CACHE_TTL_MS) {
      console.log('[fitmentService] Cache hit for:', carSlug);
      return { data: cached.data, error: null };
    }
  }

  try {
    console.log('[fitmentService] Fetching fitments for:', carSlug);
 ...
 * @param {string} [carSlug] - Optional car slug. If not provided, clears all cache.
 */
export function clearFitmentCache(carSlug = null) {
  if (carSlug) {
    fitmentCache.delete(carSlug);
  } else {
    fitmentCache.clear();
  }
}
...
export function formatWheelSpecs(option) {
  ...
  if (option.wheelWidthFront === option.wheelWidthRear && 
