...
// Server-side limit - files should be pre-compressed on client to ~1.5MB
// This is a safety check; client resize should handle large files before upload
const MAX_FILE_SIZE = 4 * 1024 * 1024; // 4MB (after client resize)
...
export async function uploadImage(file, options) {
  ...
  if (!ALLOWED_TYPES.includes(file.type)) {
    return {
      data: null,
      error: new Error(`Invalid file type. Allowed: ${ALLOWED_TYPES.join(', ')}`),
    };
  }

  // Validate file size (should be pre-compressed on client)
  if (file.size > MAX_FILE_SIZE) {
    return {
      data: null,
      error: new Error('Image too large after compression. Please try a smaller image.'),
    };
  }
    ...
    if (isCompressible(file.type)) {
      try {
        ...
        if (compressed) {
          ...
          console.log(
            `[ImageUpload] Compressed: ${(originalFileSize / 1024).toFixed(0)}KB → ${(compressed.compressedSize / 1024).toFixed(0)}KB (-${(compressed.savings * 100).toFixed(1)}%)`
          );
        }
      } catch (compressError) {
        // Log but don't fail - upload original if compression fails
        console.warn(
          '[ImageUpload] Compression failed, uploading original:',
...
export async function fetchImages(params) {
  ...
  const IMG_COLS =
    'id, user_id, user_vehicle_id, user_build_id, community_post_id, blob_url, thumbnail_url, original_filename, file_size, width, height, media_type, caption, is_primary, display_order, created_at';
