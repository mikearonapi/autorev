...
import { getLapTimePercent, normalizeTireKey } from './tireConfig.js';
...
const MOD_IMPACT = {
  ...
  // Tire compound improvements - now uses unified config from lib/tireConfig.js
  // See TIRE_COMPOUNDS in tireConfig.js for the single source of truth
  // These values are kept for backward compatibility with tests and external consumers
  // Internal calculations use getLapTimePercent() from tireConfig.js
  tires: {
    'all-season': getLapTimePercent('touring-all-season'),
    summer: getLapTimePercent('max-summer'),
    'max-performance': getLapTimePercent('extreme-summer'),
    'r-comp': getLapTimePercent('r-compound'),
    slick: getLapTimePercent('slicks'),
    // New canonical keys
    'touring-all-season': getLapTimePercent('touring-all-season'),
    'uhp-all-season': getLapTimePercent('uhp-all-season'),
    'max-summer': getLapTimePercent('max-summer'),
    'extreme-summer': getLapTimePercent('extreme-summer'),
    'track-200tw': getLapTimePercent('track-200tw'),
    'r-compound': getLapTimePercent('r-compound'),
    slicks: getLapTimePercent('slicks'),
  },
...
function calculateModImprovement(baseTime, mods) {
  ...
  // Tire improvement
  // Use unified tire config for lap time improvement
  const normalizedTire = normalizeTireKey(mods.tireCompound);
  const tirePercent = getLapTimePercent(normalizedTire);
