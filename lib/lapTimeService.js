 ...
 * Removes extreme times (race cars at the fast end, damaged cars at slow end)
 *
 * @param {number[]} sortedTimes - Array of lap times in seconds, already sorted
 * @param {number} multiplier - IQR multiplier (default 1.5 is standard, 2.0 is more lenient)
 * @returns {number[]} Filtered array without outliers
 */
function filterOutliers(sortedTimes, multiplier = 1.5) {
  if (sortedTimes.length < 4) return sortedTimes; // Need at least 4 data points for IQR

  const q1 = percentile(sortedTimes, 0.25);
  const q3 = percentile(sortedTimes, 0.75);
  const iqr = q3 - q1;

  // If IQR is very small (uniform data), don't filter
 ...
 *
 * Uses IQR outlier filtering to exclude race car times and other anomalies
 * that would skew the percentile-based skill level estimates.
 */
function computeTrackStats(lapData) {
  if (!lapData || lapData.length === 0) return null;

  const rawTimes = lapData.map((l) => l.lap_time_ms / 1000).sort((a, b) => a - b);
  ...
  // Uses 1.5x IQR which is standard for outlier detection
  // This removes race car times and damaged car/crash times
  const filteredTimes = filterOutliers(rawTimes, 1.5);

  // Use filtered times for percentiles if we have enough data, otherwise fall back to raw
  const timesForPercentiles = filteredTimes.length >= 5 ? filteredTimes : rawTimes;

  // Build HP to lap time correlation (using raw data for better correlation)
  const hpTimePairs = lapData
    .filter((l) => l.car?.hp)
    .map((l) => ({ hp: l.car.hp, time: l.lap_time_ms / 1000 }));

  return {
    count: rawTimes.length,
    filteredCount: filteredTimes.length,
    stockCount: stockTimes.length,

    // Overall statistics (from raw data)
    fastest: rawTimes[0],
    slowest: rawTimes[rawTimes.length - 1],
    median: percentile(rawTimes, 0.5),
    mean: rawTimes.reduce((a, b) => a + b, 0) / rawTimes.length,
    stdDev: standardDeviation(rawTimes),
 ...
 */
export async function getTrackBaseline(trackSlug, _carSpecs = {}) {
...
export async function estimateLapTime({
  ...
  if (baseline.source === 'real_data') {
    ...
    switch (driverSkill) {
      case 'professional':
        stockTime = baseline.proTime;
        break;
      case 'advanced':
        stockTime = baseline.advancedTime;
        break;
      case 'intermediate':
        stockTime = baseline.intermediateTime;
        break;
      case 'beginner':
        stockTime = baseline.beginnerTime;
        break;
      default:
        stockTime = baseline.intermediateTime;
    }
 ...
 */
export async function findSimilarCars(trackSlug, { hp, _weight }) {
  const client = getSupabaseClient();
  if (!client) return [];
