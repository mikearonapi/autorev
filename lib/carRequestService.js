...
export async function submitCarRequest(request, userId = null) {
  ...
  try {
    ...
    // Submit via /api/feedback to trigger Discord notifications and other side effects
    ...
    if (!response.ok || !result.success) {
      console.error('[carRequestService] API error:', result.error);
      return { data: null, error: new Error(result.error || 'Failed to submit request') };
    }

    console.log('[carRequestService] Car request submitted:', result.data?.id, carDescription);
