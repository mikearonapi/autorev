...
export const carKeys = {
  ...
  /** List helpers for building hierarchical keys */
  lists: () => [...carKeys.all, 'list'],
  
  /** List of all cars with optional filters (browse page) */
  list: (filters) => [...carKeys.lists(), filters],

  /** Details base key */
  details: () => [...carKeys.all, 'detail'],
  
  /** Single car by slug (detail page) */
  detail: (slug) => [...carKeys.details(), slug],
  
  /** Alias for detail (backwards compatibility) */
  bySlug: (slug) => [...carKeys.all, 'detail', slug],

  /** Enriched car data bundle (includes tuning, consensus, etc.) */
  ...
  /** Alias for expertConsensus (backwards compatibility) */
...
export const partsKeys = {
  ...
  /** Turbo options list (no car filter) */
  turbos: () => [...partsKeys.all, 'turbos'],
  
  /** Turbo options for a specific car */
  turbosByCarSlug: (carSlug) => [...partsKeys.all, 'turbos', carSlug],

  /** AL recommendations for a car (all upgrade types) */
  recommendations: (carSlug) => [...partsKeys.all, 'recommendations', carSlug],

  /** AL recommendations for specific upgrade types */
  recommendationsByUpgrade: (carSlug, upgradeKeys) => [
    ...partsKeys.all,
    'recommendations',
    carSlug,
    upgradeKeys?.sort().join(',') || 'all',
  ],
};
...
export default queryKeysModule;
