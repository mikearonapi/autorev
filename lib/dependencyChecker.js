/**
 * SuperNatural Motorsports - Dependency Checker
 * 
...
export function validateUpgradeSelection(selectedUpgradeKeys, car, options = {}) {
  ...
  const selectedUpgradeObjects = selectedUpgradeKeys
    ...
    .filter(Boolean);
  const totalHpGain = car && selectedUpgradeObjects.length > 0
  ...
  const filteredResults = filterResultsByContext(ruleResults, engineType, usageProfile, totalHpGain);
...
function filterResultsByContext(results, engineType, usageProfile, totalHpGain) {
  return results.filter(result => {
    ...
    if (result.ruleId === 'power-clutch' || result.ruleId === 'power-cooling' || result.ruleId === 'power-bottom-end') {
...
function checkPositiveSynergies(selectedUpgradeKeys) {
  ...
  const hasTune = selectedUpgradeKeys.some(k =>
    k === 'tune-street' || k === 'tune-track' || k === 'stage1-tune' || k === 'stage2-tune' || k === 'stage3-tune'
  );
  ...
  // Track tire + brake package synergy
  const hasTrackTires = selectedUpgradeKeys.some(k => k === 'tires-track' || k === 'tires-slicks');
  ...
  if (hasTrackTires && hasTrackBrakes) {
    synergies.push({
      ...
      upgrades: ['tires-track', 'brake-pads-track', 'brake-fluid-lines'],
    });
  }
  
  // Suspension + alignment synergy
  const hasSuspension = selectedUpgradeKeys.some(k => 
    k === 'coilovers-street' || k === 'coilovers-track' || k === 'lowering-springs'
  );
 ...
 * These MUST be added for the build to function properly
 * 
 * @param {string[]} selectedUpgradeKeys - Currently selected upgrades
 * @param {Object} car - Car object
        ...
        if (!alreadySelected.has(reqKey)) {
          ...
          if (upgrade && !required.find(r => r.upgradeKey === reqKey)) {
            required.push({
              ...
              reason: `Required for ${parentUpgrade?.name || key} to function properly`,
            });
          }
        }
      }
...
export function getSystemImpactOverview(selectedUpgradeKeys) {
  ...
  return Object.values(systemImpacts).map(impact => ({
    ...impact,
    affectedNodes: Array.from(impact.affectedNodes),
    totalImpacts: impact.improves + impact.modifies + impact.stresses + 
                  impact.invalidates + impact.compromises,
