...
function checkPositiveSynergies(selectedUpgradeKeys) {
  ...
  const hasTune = selectedUpgradeKeys.some(
    (k) =>
      k === 'stage1-tune' || k === 'stage2-tune' || k === 'stage3-tune' || k === 'piggyback-tuner'
  );
  ...
  if (hasIntake && hasExhaust && hasTune) {
    synergies.push({
      ...
      upgrades: ['intake', 'exhaust-catback', 'stage1-tune'],
    });
  }
...
const EQUIVALENT_UPGRADES = {
  'ecu-tune': ['stage1-tune', 'stage2-tune', 'stage3-tune', 'dct-tune', 'piggyback-tuner'],
};
 ...
 */
export function getRequiredUpgrades(selectedUpgradeKeys, _car) {
  const required = [];
  ...
  for (const key of selectedUpgradeKeys) {
    ...
    if (deps?.requires) {
      for (const reqKey of deps.requires) {
        // Use equivalent check - e.g., stage1-tune satisfies ecu-tune requirement
 ...
 */
export function analyzeScenario(scenarioType, _options = {}) {
  const scenarios = {
    boost_increase: {
      ...
      recommendedUpgrades: ['intercooler', 'oil-cooler', 'clutch-upgrade'],
    },
