...
export async function searchArticles(query, options = {}) {
  const { limit = 10, type: _type = 'all' } = options;
    ...
    for (const pattern of articlePatterns) {
      ...
      while ((match = pattern.exec(html)) !== null) {
        const url = match[1].startsWith('http')
          ? match[1]
          : `https://www.motortrend.com${match[1]}`;

        if (!results.some((r) => r.url === url)) {
          results.push({
            url,
            type: url.includes('comparison')
              ? 'comparison'
              : url.includes('first-drive')
                ? 'first_drive'
                : 'review',
          });
        }
...
export async function getReview(url) {
  try {
    ...
    const _jsonLd = extractJsonLd(html);
    ...
    const reviewType = url.includes('first-drive')
      ? 'first_drive'
      : url.includes('comparison')
        ? 'comparison'
        : url.includes('long-term')
          ? 'long_term'
          : 'review';
...
function parseVehicleFromTitle(title) {
  ...
  const match = cleaned.match(
    /^(\d{4})\s+([A-Za-z-]+)\s+(.+?)(?:\s+(?:Review|First|Test|vs\.|:)|$)/i
  );
...
function extractProsCons(html, type) {
  ...
  const sectionPattern = new RegExp(
    `(?:${type}|${type === 'pros' ? 'likes|good' : 'dislikes|bad'}).*?<ul[^>]*>(.*?)<\\/ul>`,
    'is'
  );
...
export async function getCarOfTheYear(year) {
  try {
    const url = year ? `/news/car-of-the-year/${year}/` : '/news/car-of-the-year/';
