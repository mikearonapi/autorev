...
function percentile(sortedPrices, p) {
  ...
  const idx = Math.max(
    0,
    Math.min(sortedPrices.length - 1, Math.floor((sortedPrices.length - 1) * p))
  );
...
export async function getYearlyMarketData(make, model, options = {}) {
  ...
  for (let y = yMin; y <= yMax; y++) {
    ...
    let listings = filterListingsByKeywords(rawListings, includeKeywords, excludeKeywords).filter(
      (l) => l?.price && l?.year === y
    );
    ...
    if (detailValidation && listings.length < minSample && rawListings.length > 0) {
      ...
      for (let i = 0; i < maxToCheck; i++) {
        ...
        try {
          ...
          const passes = filterListingsByKeywords(
            [merged],
            includeKeywords,
            excludeKeywords
  ...
  const overallAverage = Math.round(
    years
      .map((y) => y.averagePrice)
      .filter(Boolean)
      .reduce((a, b) => a + b, 0) / years.length
  );
...
function parseSearchResults(html) {
  ...
  for (let i = 0; i < startMatches.length; i++) {
    ...
    const endIdx =
      i + 1 < startMatches.length ? (startMatches[i + 1].index ?? html.length) : html.length;
...
function calculatePriceByMileage(listings) {
  ...
  for (const listing of listings) {
    ...
    for (const [_key, band] of Object.entries(bands)) {
...
export async function matchCarToMarketData(car) {
  ...
  if (car.year) {
    yearMin = car.year;
    yearMax = car.year;
  } else if (car.yearStart || car.yearEnd) {
    yearMin = car.yearStart || car.yearEnd;
    yearMax = car.yearEnd || car.yearStart;
  } else if (car.years) {
    const yearMatch = car.years?.match(/(\d{4})(?:-(\d{4}))?/);
    yearMin = yearMatch ? parseInt(yearMatch[1]) : null;
    yearMax = yearMatch && yearMatch[2] ? parseInt(yearMatch[2]) : yearMin;
  }
      ...
      position:
        priceDiffPercent > 10
          ...
          : priceDiffPercent < -10
            ...
            : 'at market',
      inventoryLevel:
        marketData.listingCount > 50 ? 'high' : marketData.listingCount > 20 ? 'moderate' : 'low',
    },
  };
}

const carsComScraper = {
  searchListings,
  getMarketData,
  getListingDetails,
