 ...
 *
 * FALLBACK CHAIN:
 * 1. HTTP scraper (fast, may be blocked)
 * 2. Browser scraper (slower, handles some anti-bot)
 * 3. Apify scraper (paid, most reliable)
 *
...
import { scrapeBringATrailer as apifyBaTScraper, isApifyConfigured } from '../apifyClient.js';
 ...
 * Uses a 3-tier fallback: HTTP → Browser → Apify
 *
 ...
 * @param {boolean} [options.useApifyOnly=false] - Skip custom scrapers, go straight to Apify
 * @returns {Promise<BaTAuction[]>}
 */
export async function searchCompletedAuctions(query, options = {}) {
  const { limit = 20, soldOnly = true, minYear, maxYear, useApifyOnly = false } = options;

  // Option to skip custom scrapers and go straight to Apify
  if (useApifyOnly && isApifyConfigured()) {
    console.log('[BaT] Using Apify scraper directly (useApifyOnly=true)');
    return searchWithApify(query, { limit, soldOnly, minYear, maxYear });
  }
  ...
  try {
    ...
    // Check if browser also got blocked
    if (isBlockedHtml(html)) {
      console.warn('[BaT] Browser also blocked; falling back to Apify...');
      return searchWithApify(query, { limit, soldOnly, minYear, maxYear });
    }

    const auctions = parseAuctionListing(html);

    // If no results, try Apify as fallback
    if (auctions.length === 0 && isApifyConfigured()) {
      console.warn('[BaT] No results from custom scraper; trying Apify...');
      return searchWithApify(query, { limit, soldOnly, minYear, maxYear });
    }
    ...
    // Fallback to Apify on any error
    if (isApifyConfigured()) {
      console.log('[BaT] Falling back to Apify due to error...');
      return searchWithApify(query, { limit, soldOnly, minYear, maxYear });
    }
...
/**
 * Search BaT using Apify scraper as fallback
...
async function searchWithApify(query, options = {}) {
  ...
  if (!isApifyConfigured()) {
    console.warn('[BaT] Apify not configured, cannot fallback');
    return [];
  }

  try {
    const apifyResults = await apifyBaTScraper(query, { maxItems: limit * 2 });
    ...
    console.log(`[BaT/Apify] Returning ${filtered.length} results`);
    return filtered.slice(0, limit);
  } catch (err) {
    console.error(`[BaT/Apify] Error:`, err.message);
    return [];
  }
}
...
function parseAuctionListing(html) {
  ...
  const listingPattern =
    /<a[^>]*class="[^"]*listing-card[^"]*"[^>]*href="([^"]+)"[^>]*>[\s\S]*?<\/a>/gi;
  const cardPattern = /<article[^>]*class="[^"]*listing-card[^"]*"[^>]*>[\s\S]*?<\/article>/gi;

  // Try both patterns
  const matches = html.match(cardPattern) || html.match(listingPattern) || [];

  // Fallback: try to find auction data in JSON
  const jsonMatch = html.match(
    /<script[^>]*type="application\/json"[^>]*id="[^"]*auctions[^"]*"[^>]*>([\s\S]*?)<\/script>/i
  );
  if (jsonMatch) {
    try {
...
function parseAuctionCard(cardHtml) {
  try {
    ...
    const priceMatch =
      cardHtml.match(/\$([0-9,]+)/) || cardHtml.match(/class="[^"]*price[^"]*"[^>]*>([^<]+)</i);
    const price = priceMatch ? parsePrice(priceMatch[1]) : null;

    // Check if sold vs reserve not met
    const sold =
      cardHtml.toLowerCase().includes('sold') ||
      cardHtml.includes('Sold for') ||
      (price && !cardHtml.toLowerCase().includes('reserve not met'));
    const reserveNotMet =
      cardHtml.toLowerCase().includes('reserve not met') ||
...
export async function getMarketData(query, options = {}) {
  ...
  return {
    ...
    sellThroughRate:
      allCompleted.length > 0 ? Math.round((sold.length / allCompleted.length) * 100) : null,
    averageMileage,
...
export async function getAuctionDetails(urlOrId) {
  try {
    const url = urlOrId.startsWith('http') ? urlOrId : `/listing/${urlOrId}`;
...
export async function matchCarToBaTData(car) {
  ...
  // Handle new schema: year (integer), with fallback to years string
  let year = car.year;
  if (!year && car.years) {
...
export default bringATrailerScraper;
