  ...
  try {
    ...
    while ((match = linkPattern.exec(html)) !== null) {
      const url = match[1].startsWith('http')
        ? match[1]
        : `https://www.caranddriver.com${match[1]}`;
...
function parseVehicleFromTitle(title) {
  ...
  const match = cleaned.match(
    /^(\d{4})\s+([A-Za-z-]+)\s+(.+?)(?:\s+(?:Review|Tested|First|Comparison|vs\.|:)|$)/i
  );
...
function extractHighsLows(html, type) {
  ...
  const sectionPattern = new RegExp(
    `(?:${type}|${type === 'highs' ? 'pros' : 'cons'}).*?<ul[^>]*>(.*?)<\\/ul>`,
    'is'
  );
...
function extractTestData(html) {
  ...
  // Quarter mile
  const quarterMile = html.match(
    /(?:1\/4|quarter)[-\s]*mile[^0-9]*(\d+\.?\d*)\s*sec[^0-9]*@?\s*(\d+\.?\d*)?\s*(?:mph)?/i
  );
  if (quarterMile) {
...
export async function matchCarToCaDReviews(car) {
  ...
  const reviews = year
    ? await getVehicleReviews(year, car.make || query.split(' ')[0], query)
...
export default carAndDriverScraper;
