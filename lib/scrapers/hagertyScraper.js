...
import { createScraper, extractJsonLd, stripTags, parsePrice } from './scraperFramework.js';
...
export async function matchCarToHagertyValuation(car) {
  ...
  // Handle new schema: year (integer) or yearStart/yearEnd, with fallback to years string
  let year = null;
  if (car.year) {
    year = car.year;
...
export async function compareToHagerty(car) {
  ...
  return {
    ...
    comparison: {
      ...
      assessment:
        priceDiffPercent > 15 ? 'overpriced' : priceDiffPercent < -15 ? 'underpriced' : 'fair',
    },
  };
}

const hagertyScraper = {
  searchVehicles,
  getValuation,
  getPriceHistory,
