...
import { createScraper, extractBetween, stripTags, extractJsonLd } from './scraperFramework.js';
 ...
 * @returns {string}
 */
function _buildSearchUrl(year, make, model) {
...
function parseRatingFromHtml(html, testName) {
  ...
  const patterns = [
    ...
    new RegExp(
      `<[^>]*data-test="${testName}"[^>]*>.*?<[^>]*class="[^"]*rating[^"]*"[^>]*>([^<]+)`,
      'is'
    ),
...
export async function matchCarToIIHS(car) {
  ...
  // Extract make from car name or make field
