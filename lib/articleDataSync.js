 ...
 */
const ARTICLE_DATA_REQUIREMENTS = {
  ...
  budget: {
    ...
    limit: 10,
    sortBy: 'score',
    priceField: 'price_avg', // Use market/used prices, not MSRP
  },
...
export async function getTopCarsUnderPrice(maxPrice, options = {}) {
  ...
  const CAR_COLS = 'id, slug, name, year_start, year_end, make, model, country, hp, hp_range, torque, acceleration_0_60, top_speed, engine_layout, drivetrain, transmission, price_avg, price_low, price_high, track, sound, reliability, value, aftermarket, daily, tuning_potential, fun, image_url, created_at';
  
  // Query cars within price range
  const { data: cars, error } = await supabase
    .from('cars')
    .select(CAR_COLS)
    .gte('price_avg', minPrice)
  ...
  if (includeScores) {
    return topCars.map(car => ({
      ...
      maxScore: Object.values(weights).reduce((sum, w) => sum + (10 * w), 0).toFixed(1),
    }));
  }
...
export function analyzeArticleDataNeeds(article) {
  ...
  if (threeWayMatch) {
    ...
    result.filters = {
      cars: [
        threeWayMatch[1].trim(),
        threeWayMatch[2].trim(),
        threeWayMatch[3].trim(),
      ],
    };
    return result;
  }
...
export async function auditArticleAlignment(article) {
  ...
  if (needs.dataType === 'budget') {
    ...
    const dbCarNames = dbTopCars.map(c => c.name.toLowerCase());
...
export async function auditAllArticles() {
  ...
  const ARTICLE_COLS = 'id, slug, title, car_id, car_slug, topic, content_markdown, excerpt, seo_title, seo_description, featured_image_url, is_published, published_at, view_count, created_at';
...
async function generateComparisonArticleData(params) {
  ...
  return {
    ...
    categoryComparisons: [
      'sound', 'interior', 'track', 'reliability', 'value', 'driverFun', 'aftermarket'
    ].map(cat => ({
      category: cat,
