...
export async function detectFatigue(userId) {
  ...
  try {
    ...
    const dismissRate = (viewedCount + dismissedCount) > 0 
    ...
    const { data: recentPrefsChanges, error: prefsError } = await client
    ...
    return {
      data: {
        ...
        recommendedAction: isFatigued 
          ? getRecommendedAction(severity, reasons)
          : null,
      },
      error: null,
    };
 ...
 */
function getRecommendedAction(severity, reasons) {
  switch (severity) {
    case 'severe':
      return {
    ...
    case 'mild':
      return {
        ...
        actions: [
          ...
          'Increase time between non-urgent notifications',
        ],
      };
    default:
      return null;
...
export async function getFatigueMetrics() {
  ...
  try {
    ...
    const dismissedCount = interactionStats.filter(i => i.interaction_type === 'dismissed').length;
    ...
    const overallDismissRate = (viewedCount + dismissedCount) > 0
      ? (dismissedCount / (viewedCount + dismissedCount)) * 100
      : 0;
    ...
    return {
      data: {
        ...
        health: {
          status: overallDismissRate < 50 && overallClickRate > 10 ? 'healthy' :
                  overallDismissRate < 70 && overallClickRate > 5 ? 'warning' : 'critical',
        },
      },
      error: null,
    };
