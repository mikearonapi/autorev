...
/**
 * Resolve car_id from car_slug
 * Used as fallback when car.id is not available
 * @param {string} carSlug 
 * @returns {Promise<string|null>} - car UUID or null
 */
...
export async function updateUserProject(userId, projectId, updates) {
  ...
  const selectedUpgradesData = updates.selectedUpgrades || updates.upgrades 
    ? {
        ...
        factoryConfig: updates.factoryConfig || null,
        wheelFitment: updates.wheelFitment || null,
        sizeSelections: updates.sizeSelections || null,
        heroSource: updates.heroSource || 'stock',
        heroImageId: updates.heroImageId || null,
        // Tuner mode: 'basic' or 'advanced' - determines which HP calculation was used
        tunerMode: updates.tunerMode || 'basic',
        // Advanced specs if user was in advanced mode (for recalculation reference)
        advancedSpecs: updates.advancedSpecs || null,
      }
    : undefined;
  ...
  const { data, error } = await supabase
    ...
    .update({
      ...
      // Performance metrics snapshot (for consistent community display)
      stock_hp: updates.stockHp,
      stock_zero_to_sixty: updates.stockZeroToSixty,
      stock_braking_60_0: updates.stockBraking60To0,
      stock_lateral_g: updates.stockLateralG,
      final_zero_to_sixty: updates.finalZeroToSixty,
      final_braking_60_0: updates.finalBraking60To0,
      final_lateral_g: updates.finalLateralG,
      zero_to_sixty_improvement: updates.zeroToSixtyImprovement,
      braking_improvement: updates.brakingImprovement,
      lateral_g_improvement: updates.lateralGImprovement,
    })
    .eq('user_id', userId)
    .eq('id', projectId)
    .select()
