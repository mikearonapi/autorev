...
function normalizeSelectedPartsForDb(selectedParts, projectId) {
  ...
  return selectedParts
    .map((p) => {
      // For user-entered parts (from PartsSelector), generate a UUID if not present
      // Temp IDs (temp_xxx) will get new UUIDs generated
      let partId = p?.part_id || p?.partId;
      if (!partId || (typeof partId === 'string' && partId.startsWith('temp_'))) {
        // Generate a UUID for user-entered parts
