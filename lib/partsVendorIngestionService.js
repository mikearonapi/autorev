 ...
 * This skips products that do NOT match any `fitment_tag_mappings` rows for the vendor.
 */
export async function ingestShopifyVendor(options) {
  ...
  while (page <= maxPages && processed < maxProducts) {
    ...
    for (const product of products) {
      ...
      const tags = splitShopifyTags(product?.tags);
      const mappings = await getFitmentMappingsForTags({ client, vendorKey, tags });
      if (!mappings || mappings.length === 0) continue; // skip unrelated products

      const category = categorize({ title: product?.title, productType: product?.product_type, tags });
      const { partId, productUrl } = await upsertPart({
        client,
        brandId,
        vendorKey,
...
export const SHOPIFY_VENDOR_PRESETS = {
  eqtuning: { vendorKey: 'eqtuning', vendorName: 'EQTuning', vendorUrl: 'https://eqtuning.com' },
  bmptuning: { vendorKey: 'bmptuning', vendorName: 'BMP Tuning', vendorUrl: 'https://www.bmptuning.com' },
  performancebyie: { vendorKey: 'performancebyie', vendorName: 'Integrated Engineering', vendorUrl: 'https://performancebyie.com' },
};
...
export async function runPartsVendorIngestJob(job) {
  ...
  const preset = vendorKey ? SHOPIFY_VENDOR_PRESETS[vendorKey] : null;
