...
- Year: ${context.car.year || 'N/A'}
- Power: ${context.car.hp || 'N/A'} hp
- Engine: ${context.car.engineType || 'N/A'}
- 0-60: ${context.car.zeroToSixty || 'N/A'}s
- Weight: ${context.car.curbWeight || 'N/A'} lbs
- MSRP: ${context.car.msrp ? `$${context.car.msrp.toLocaleString()}` : 'N/A'}`;
  }
...
describe('AI Mechanic Context Priority Logic', () => {
  test('context priority: includes owned vehicles when user has them', () => {
    ...
    assert.ok(
      result.includes("User's Garage (Owned Vehicles)"),
      'Should contain owned vehicles section'
    );
  ...
  test('context priority: shows favorites when no owned vehicles', () => {
    const context = {
      ...
      userFavorites: [
        {
          car_name: 'Porsche 987.2 Cayman S',
          car_slug: 'porsche-cayman-987-2',
          notes: 'Dream car',
        },
    ...
    assert.ok(
      result.includes("User's Favorites (No Owned Vehicles)"),
  ...
  test('context priority: asks for car when no vehicles and no favorites', () => {
    ...
    assert.ok(
      result.includes('user has NO owned vehicles and NO favorites'),
      'Should state no context'
    );
  ...
  test('context priority: prioritizes owned vehicles over favorites', () => {
    const context = {
      ...
      userFavorites: [{ car_name: 'Porsche 911 GT3', car_slug: 'porsche-911-gt3' }],
      hasOwnedVehicles: true,
      hasFavorites: true,
      hasNoCarContext: false,
    };
    ...
    assert.ok(
      !result.includes("User's Favorites (No Owned Vehicles)"),
      'Should not show favorites fallback'
    );
  });
