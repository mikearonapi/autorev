...
export async function processWithMultiAgent({
  ...
  if (
    ...
    console.log(`[AL Multi-Agent:${correlationId}] research_parts_live was used - attempting parts extraction`);
    ...
    console.warn(`[AL Multi-Agent:${correlationId}] research_parts_live used but missing car context (${context.car?.id}) or response`);
  }
...
export async function streamWithMultiAgent({
  ...
  try {
    ...
    if (
      ...
      console.log(`[AL Multi-Agent:${correlationId}] research_parts_live was used (streaming) - attempting parts extraction`);
      extractAndSavePartsFromResponse(
      ...
      console.warn(`[AL Multi-Agent:${correlationId}] research_parts_live used (streaming) but missing car context (${context.car?.id}) or response`);
    }
