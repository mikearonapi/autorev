...
export async function processWithMultiAgent({
  ...
  if (
    ...
    console.warn(
      `[AL Multi-Agent:${correlationId}] research_parts_live used but missing car context (${context.car?.id}) or response`
    );
  }
...
export async function streamWithMultiAgent({
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `[AL Multi-Agent:${correlationId}] research_parts_live used (streaming) but missing car context (${context.car?.id}) or response`
      );
    }
