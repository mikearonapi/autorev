...
export async function fetchAllSafetyData(vehicle) {
  ...
  try {
    // Fetch recalls - try VIN first, fall back to make/model/year if VIN returns empty
    // VIN-based lookup only returns recalls specific to that VIN's configuration,
    // while make/model/year returns all recalls for the model
    let recallsResult;
    if (vin && vin.length === 17) {
      recallsResult = await fetchRecallsByVIN(vin);
      // If VIN lookup returned empty, fall back to make/model/year
      // This ensures users see all potential recalls for their vehicle
      if (!recallsResult.data || recallsResult.data.length === 0) {
        recallsResult = await fetchRecallsByVehicle(year, make, model);
      }
    } else {
      recallsResult = await fetchRecallsByVehicle(year, make, model);
    }

    // Fetch other data in parallel (these don't support VIN lookup)
    const [complaintsResult, investigationsResult, ratingsResult] = await Promise.all([
...
export async function fetchComprehensiveSafetyData(vehicle) {
  ...
  try {
    ...
    // Fetch other data in parallel including TSBs
    const [complaintsResult, investigationsResult, ratingsResult, tsbsResult] = await Promise.all([
      fetchComplaints(year, make, model),
      fetchInvestigations(year, make, model),
      fetchSafetyRatings(year, make, model),
