 ...
 * (Synced with upgradeCalculator.js STAGE_TUNE_INCLUDED_MODS)
 */
...
export async function calculateBuildPerformance(car, selectedUpgradeKeys, options = {}) {
  ...
  for (const upgradeKey of selectedUpgradeKeys) {
    ...
    let adjustmentSource = gain.source;
    const category = getUpgradeCategory(upgradeKey);
    
    // Check if this mod is "expected" by a stage tune (avoid double-counting)
    // e.g., Stage 3 tune is calibrated assuming turbo upgrade is installed
    const isExpectedByTune = activeTune && 
      STAGE_TUNE_INCLUDED_MODS[activeTune]?.includes(upgradeKey);
    
    if (isExpectedByTune) {
      // Reduce gain by 50% since tune already accounts for this hardware
      const reducedGain = Math.round(appliedGain * 0.5);
      result.adjustmentAmount += (appliedGain - reducedGain);
      appliedGain = reducedGain;
      adjustmentSource = `${gain.source} (50% - included in ${activeTune} tune)`;
    }
    
    if (categoryGains[category] > 0 && PHYSICS_CONSTANTS.DIMINISHING_RETURNS[category]) {
      const diminishedGain = Math.round(appliedGain * PHYSICS_CONSTANTS.DIMINISHING_RETURNS[category]);
      result.adjustmentAmount += (appliedGain - diminishedGain);
...
export default performanceCalculatorV2;
