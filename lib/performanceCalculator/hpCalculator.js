...
export function calculateSmartHpGain(car, selectedKeys, options = {}) {
  ...
  const conflicts = includeConflicts 
    ? detectUpgradeConflictsInternal(selectedKeys, car) 
    : [];
  ...
  for (const { key, upgrade } of upgrades) {
    ...
    // Physics-based: percentage of stock HP (already has aspiration baked in)
    const physicsPercent = getPhysicsBasedGainPercent(key, aspiration);
    const physicsBasedGain = Math.round(stockHp * physicsPercent);
    ...
    const ruleBasedGain = Math.round(baseHpGain * multiplier);
...
export function calculateAllModificationGains(installedMods, car = null) {
  ...
  if (car) {
    ...
    const smartResult = calculateSmartHpGain(car, installedMods, { includeConflicts: false });
    ...
    installedMods.forEach(modKey => {
      ...
      const overlapModifier = isModExpectedByTune(modKey, activeTune) 
        ? TUNE_OVERLAP_MODIFIER 
        : 1.0;
  ...
  // ==========================================================================
  // TORQUE, BRAKING, LATERAL G - Simple summation with overlap handling
  // (calculateSmartHpGain doesn't handle these, so we calculate separately)
  // ==========================================================================
  ...
  installedMods.forEach(modKey => {
    ...
    // Check if this mod is expected by the active tune (for torque overlap)
    const overlapModifier = isModExpectedByTune(modKey, activeTune) 
      ? TUNE_OVERLAP_MODIFIER 
      : 1.0;
    
    // Torque gain - apply same overlap logic as HP
    ...
    // Other metrics - direct summation (no overlap concerns)
 ...
 * @private
 */
function detectUpgradeConflictsInternal(selectedKeys, car) {
  const conflicts = [];
...
export function calculateHandlingScore(installedMods) {
  ...
  if (installedMods.includes('brake-pads-track')) {
    breakdown.brakes += 4;
  } else if (installedMods.includes('brake-pads-street') || installedMods.includes('brake-pads-performance')) {
    breakdown.brakes += 2;
  }
  ...
  if (installedMods.includes('braided-brake-lines') || installedMods.includes('brake-fluid-lines')) {
    breakdown.brakes += 2;
  }
  ...
  if (installedMods.includes('tires-track') || installedMods.includes('tires-r-compound')) {
    breakdown.tires += 8;
  } else if (installedMods.includes('tires-performance') || installedMods.includes('tires-summer')) {
    breakdown.tires += 5;
  }
...
export function calculateReliabilityScore(installedMods, hpGainPercent = 0) {
  ...
  // Fuel system
  if (installedMods.includes('fuel-system-upgrade') || installedMods.includes('hpfp-upgrade')) {
    supportBonus += 6;
  }
  ...
  if (hpGainPercent > 0.40 && !installedMods.includes('fuel-system-upgrade') && !installedMods.includes('hpfp-upgrade')) {
    warnings.push('40%+ HP gain may benefit from fuel system upgrades');
  }
  
  if (hpGainPercent > 0.25 && !installedMods.includes('oil-cooler') && !installedMods.includes('intercooler')) {
