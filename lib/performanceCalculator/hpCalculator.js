...
export function calculateSmartHpGain(car, selectedKeys, options = {}) {
  ...
  for (const { key, upgrade } of upgrades) {
    ...
    // Handle exhaust mods
      ...
      // Check if this mod is expected by a stage tune (engine-type aware)
      if (isModExpectedByTune(key, activeTune, engineCategory)) {
...
export function calculateHandlingScore(installedMods) {
  ...
  // Supports both legacy keys and new canonical keys from lib/tireConfig.js
  // ==========================================================================
  // R-compound / Track tires (max grip)
  if (
    installedMods.includes('tires-track') ||
    installedMods.includes('tires-r-compound') ||
    installedMods.includes('r-compound') ||
    installedMods.includes('track-200tw') ||
    installedMods.includes('slicks') ||
    installedMods.includes('competition-tires')
  ) {
    breakdown.tires += 8;
  }
  // Performance / Extreme summer tires
  else if (
    installedMods.includes('tires-performance') ||
    installedMods.includes('tires-summer') ||
    installedMods.includes('max-summer') ||
    installedMods.includes('extreme-summer') ||
    installedMods.includes('performance-tires')
  ) {
    breakdown.tires += 5;
  }
  // UHP all-season (slight improvement)
  else if (installedMods.includes('uhp-all-season')) {
    breakdown.tires += 2;
  }

  if (
    installedMods.includes('wheels-lightweight') ||
    installedMods.includes('forged-wheels') ||
    installedMods.includes('lightweight-wheels')
  ) {
    breakdown.tires += 3;
  }
