...
import { getGripCoefficient, getBaselineTireCompound } from '../tireConfig.js';
...
// Now uses unified tire config from lib/tireConfig.js
// See TIRE_COMPOUNDS in tireConfig.js for the single source of truth
// =============================================================================
const BASELINE_GRIP = getBaselineTireCompound().gripCoefficient;
...
/**
 * Get the best tire grip coefficient from installed mods or tire compound
 *
 * Uses unified tire config from lib/tireConfig.js
 *
 * @param {string[]} upgradeKeys - Array of upgrade keys
 * @param {string} [tireCompound] - Optional explicit tire compound (from WheelTireConfigurator)
 * @returns {number} - Grip coefficient (higher = more grip)
 */
function getTireGripCoefficient(upgradeKeys, tireCompound = null) {
  // If explicit tire compound provided (from WheelTireConfigurator), use it
  if (tireCompound) {
    return getGripCoefficient(tireCompound);
  }

  if (!upgradeKeys || upgradeKeys.length === 0) return BASELINE_GRIP;

  // Find any tire-related upgrade and get its grip coefficient
  // These are legacy keys that might exist in upgrade data
  const tireRelatedKeys = [
    'slicks',
    'r-compound',
    'tires-r-compound',
    'track-200tw',
  ...
  // Find the first tire upgrade and use its grip coefficient
  for (const key of tireRelatedKeys) {
    if (upgradeKeys.includes(key)) {
      return getGripCoefficient(key);
    }
  }

  return BASELINE_GRIP;
}
...
export function calculateUpgradedMetrics(car, selectedUpgrades) {
  ...
  // ==========================================================================
  const newTrapSpeed = estimateTrapSpeedImproved(car.hp || 400, newHp, newWeight, car.trapSpeed, {
    drivetrain: car.driveType || 'RWD',
  ...
  // ==========================================================================
  let newLateralG = estimateLateralG(car.lateralG || 0.95, upgradeKeys, tireGrip);

  // Add explicit improvements from mods (e.g., sway bars have lateralGImprovement)
  if (lateralGImprovement > 0) {
    newLateralG = Math.min(1.5, newLateralG + lateralGImprovement);
  }

  // ==========================================================================
  // POWER TO WEIGHT - Now accounts for weight changes
  // ==========================================================================
  const powerToWeight = newWeight && newHp ? (newHp / (newWeight / 1000)).toFixed(1) : null;
...
function detectTransmissionType(car) {
  ...
  if (
    ...
    trans.includes('mt') ||
    (trans.includes('6-speed') && !trans.includes('auto'))
  ) {
    return 'manual';
  }
...
export function estimateZeroToSixty(hp, weight, options = {}) {
  ...
  const shiftPenalty =
    (ACCELERATION_PHYSICS.SHIFT_TIME[transmission] || 0.2) * ACCELERATION_PHYSICS.GEARS_TO_60;
 ...
 */
export function estimateQuarterMileImproved(
  stockHp,
  upgradedHp,
  weight,
  stockQuarterMile,
 ...
 */
export function estimateTrapSpeedImproved(
  stockHp,
  upgradedHp,
  weight,
  stockTrapSpeed,
...
export function estimateBrakingDistance(
  weight,
  brakeUpgradeBonus = 0,
  tireGrip = BASELINE_GRIP,
  stockBraking = null
) {
  // Stock grip baseline from unified config - anything at or below this doesn't penalize
  const STOCK_GRIP_BASELINE = BASELINE_GRIP;
 ...
 */
export function estimateLateralG(baseLateralG, upgradeKeys = [], tireGrip = BASELINE_GRIP) {
  if (!baseLateralG) baseLateralG = 0.9; // Default for average sports car
  ...
  if (tireGrip > STOCK_GRIP_BASELINE) {
    const gripImprovement = tireGrip / STOCK_GRIP_BASELINE - 1;
    multiplier += gripImprovement * 0.8; // 80% of grip improvement transfers to lateral G
  }
  ...
  return Math.round(Math.max(0.75, Math.min(1.5, result)) * 100) / 100;
}
