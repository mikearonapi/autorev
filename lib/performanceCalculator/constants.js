...
export const TUNE_HIERARCHY = {
  ...
  // Stage tunes for turbo/SC cars only - NA cars use bolt-ons
...
export function getPhysicsBasedGainPercent(upgradeKey, aspiration) {
  ...
  // Stage tunes - major gains on forced induction (turbo/SC cars only)
  ...
  if (key.includes('supercharger-roots')) return 0.5;
  if (key.includes('supercharger-centrifugal')) return 0.4;
  if (key.includes('turbo-kit-twin')) return 0.65;
  if (key.includes('turbo-kit-single')) return 0.5;
