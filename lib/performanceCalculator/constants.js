...
/**
 * Mutually exclusive upgrade groups - only ONE item from each group can be selected.
 * When selecting an item, any other items from the same group are automatically deselected.
 *
 * Based on UPGRADE-CATEGORY-LOGIC.md:
 * - Suspension: Springs vs Coilovers (you can't have both)
 * - Brake Pads: Street vs Track compound (same slot)
 * - Camshafts: Mild vs Aggressive (same engine component)
 * - Forced Induction Kits: Only one FI kit for NA cars
 * - Rear Exhaust: Cat-back replaces rear section, axle-back is subset
 */
export const MUTUALLY_EXCLUSIVE_GROUPS = {
  // Suspension - only one spring/damper setup
  suspension: ['lowering-springs', 'coilovers-street', 'coilovers-track'],

  // Brake pads - street or track compound, not both
  brakePads: ['brake-pads-street', 'brake-pads-track'],

  // Camshafts - mild or aggressive profile, not both
  camshafts: ['camshaft-mild', 'camshaft-aggressive'],

  // Forced induction kits - only one FI kit for NA cars
  // (supercharger OR turbo kit, not multiple)
  forcedInductionKit: [
    'supercharger-centrifugal',
    'supercharger-roots',
    'turbo-kit-single',
  ...
  // Rear exhaust section - cat-back and axle-back both replace the rear portion
  rearExhaust: ['exhaust-catback', 'exhaust-axleback'],
};

/**
 * Get the mutually exclusive group for an upgrade, if any
 * @param {string} upgradeKey - The upgrade key to check
 * @returns {{groupName: string, members: string[]}|null} - Group info or null
 */
export function getMutuallyExclusiveGroup(upgradeKey) {
  for (const [groupName, members] of Object.entries(MUTUALLY_EXCLUSIVE_GROUPS)) {
    if (members.includes(upgradeKey)) {
...
/**
 * Get conflicting upgrades from mutually exclusive groups
 ...
 */
export function getExclusiveConflicts(upgradeKey, selectedKeys) {
  const group = getMutuallyExclusiveGroup(upgradeKey);
  if (!group) return [];

  // Return any selected upgrades that are in the same exclusive group
  return selectedKeys.filter((key) => key !== upgradeKey && group.members.includes(key));
}
