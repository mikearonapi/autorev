...
/**
 * Get the low price from car data
 * @param {Object} car
 * @returns {number}
 */
function getPriceLow(car) {
  // Use msrp directly (new schema)
  return car.msrp || 0;
}
...
export function checkCarAgainstPreferences(car, preferences) {
  ...
  if (carPrice > preferences.budgetMax) {
    mismatches.push(
      `Over budget ($${(carPrice / 1000).toFixed(0)}K vs $${(preferences.budgetMax / 1000).toFixed(0)}K max)`
    );
  }
  ...
  // Check drivetrain
  if (preferences.drivetrainFilter !== 'all' && car.driveType !== preferences.drivetrainFilter) {
    mismatches.push(
      `${car.driveType || 'Unknown'} drivetrain (wanted ${preferences.drivetrainFilter})`
    );
  }
  ...
  if (preferences.engineLayoutFilter !== 'all' && car.category !== preferences.engineLayoutFilter) {
    mismatches.push(
      `${car.category || 'Unknown'} layout (wanted ${preferences.engineLayoutFilter})`
    );
  }
  ...
  if (preferences.tierFilter !== 'all' && car.tier !== preferences.tierFilter) {
    ...
    mismatches.push(
      `${tierLabels[car.tier] || car.tier} tier (wanted ${tierLabels[preferences.tierFilter]})`
    );
  }
