...
const STORAGE_KEY = 'compare'; // Storage abstraction adds 'autorev_' prefix
...
export function loadCompare() {
  ...
  if (stored && Array.isArray(stored.cars)) {
    return { cars: stored.cars };
  }
  return defaultState;
}
...
export function addToCompare(state, car) {
  ...
  if (state.cars.some((c) => (c.carId && c.carId === carId) || (!c.carId && c.slug === car.slug))) {
    return state;
  }
 ...
 * @returns {CompareState}
 */
export function removeFromCompare(state, carId) {
  const newState = {
    // Filter by carId, with fallback to slug for backward compat
    cars: state.cars.filter((c) => c.carId !== carId && c.slug !== carId),
  };
...
export function toggleCompare(state, car) {
  const carId = car.id || car.carId;
  if (state.cars.some((c) => (c.carId && c.carId === carId) || (!c.carId && c.slug === car.slug))) {
    return removeFromCompare(state, carId);
  }
 ...
 * @returns {boolean}
 */
export function isInCompare(state, carId) {
  // Check by carId, with fallback to slug for backward compat
