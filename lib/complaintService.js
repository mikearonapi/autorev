 ...
 * @param {string} [carSlug] - Optional car slug for backward compatibility
 * @returns {object|null}
 */
export function mapNhtsaComplaintToCarIssuesRow(carId, complaint, modelYear, carSlug = null) {
 ...
 * @returns {Promise<{car_id:string, rows: object[], errors: string[], totalFetched: number}>}
 */
export async function fetchComplaintRowsForCar(params) {
  ...
  if (error) return { car_id: carId, rows: [], errors: [error], totalFetched: 0 };
  if (!carId) return { car_id: '', rows: [], errors: ['car.id is required'], totalFetched: 0 };
  ...
  for (const id of identifiers) {
    const url = buildComplaintsByVehicleUrl({
      make: id.make,
      model: id.model,
      modelYear: id.modelYear,
    });
    ...
    for (const r of results) {
      const row = mapNhtsaComplaintToCarIssuesRow(carId, r, id.modelYear, carSlug);
  ...
  for (const row of allRows) {
    ...
    const key = `${carId}:${odiNumber}`;
    if (!dedup.has(key)) dedup.set(key, row);
  }

  return { car_id: carId, rows: [...dedup.values()], errors, totalFetched };
}

/**
 * Upsert complaint rows into `car_issues`.
 * Uses ODI number in metadata for deduplication via a conflict check.
 * Updated 2026-01-11: Uses car_id for queries (car_slug column removed from car_issues)
 *
 * @param {object} params
...
export async function upsertComplaintRows({ client, rows }) {
  ...
  // Build a cache of car_slug -> car_id resolutions
  const carIdCache = new Map();
  const uniqueSlugs = [...new Set(rows.map((r) => r.car_slug).filter(Boolean))];
  for (const slug of uniqueSlugs) {
    const carId = await resolveCarId(slug);
    if (carId) carIdCache.set(slug, carId);
  }
  ...
  try {
    ...
    for (const row of rows) {
      ...
      // Resolve car_id from slug (car_slug column no longer exists on car_issues)
      const carId = carIdCache.get(row.car_slug);
      if (!carId) {
        console.warn(
          `[complaintService] Skipping row: could not resolve car_id for slug ${row.car_slug}`
        );
        continue;
      }
      ...
      // Ensure row has car_id for insert/update
      const rowWithCarId = { ...row, car_id: carId };
      ...
      if (existing) {
        ...
        // Insert new record
        const { error: insertErr } = await client.from('car_issues').insert(rowWithCarId);
 ...
 * @param {string} params.carId - UUID from cars table
 * @param {object[]} params.rows
 * @returns {Promise<{success:boolean, deleted:number, inserted:number, error?:string}>}
 */
export async function replaceComplaintsForCar({ client, carId, rows }) {
  if (!client) return { success: false, deleted: 0, inserted: 0, error: 'client is required' };
  if (!carId) return { success: false, deleted: 0, inserted: 0, error: 'carId is required' };

  try {
    // Delete existing complaints for this car using car_id
    ...
    // Ensure all rows have car_id set for insert
    const rowsWithCarId = rows.map((row) => ({ ...row, car_id: carId }));

    // Insert all new rows (with car_id)
    const { error: insertErr } = await client.from('car_issues').insert(rowsWithCarId);

    if (insertErr) throw insertErr;

    return { success: true, deleted: deletedCount, inserted: rowsWithCarId.length };
  } catch (err) {
