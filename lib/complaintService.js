...
export async function upsertComplaintRows({ client, rows }) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (!carId) {
        console.warn(`[complaintService] Skipping row: could not resolve car_id for slug ${row.car_slug}`);
        continue;
      }
...
export async function replaceComplaintsForCar({ client, carSlug, rows }) {
  ...
  // Resolve car_id once (car_slug column no longer exists on car_issues)
  const carId = await resolveCarId(carSlug);
  if (!carId) {
    return { success: false, deleted: 0, inserted: 0, error: `Could not resolve car_id for slug: ${carSlug}` };
  }
