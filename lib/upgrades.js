...
const moduleToEducationKeyMap = {
  ...
  'downpipe': 'downpipe',
  'charge-pipe-upgrade': 'charge-pipe-upgrade',
  'hpfp-upgrade': 'hpfp-upgrade',
  'flex-fuel-e85': 'fuel-system-upgrade',
  'methanol-injection': 'fuel-system-upgrade',
  'dct-tune': 'ecu-tune',
  'camshafts': 'camshafts',
  'ported-heads': 'ported-heads',
  'forged-internals': 'forged-internals',
  'stroker-kit': 'stroker-kit',
  ...
  'heat-exchanger-sc': 'heat-exchanger-sc',
 ...
 * Used by garage to display modified performance metrics
...
export function calculateAllModificationGains(installedMods, car = null) {
  ...
  const engineType = car ? getEngineType(car) : null;
  ...
  installedMods.forEach(modKey => {
    ...
    if (changes.hpGain) {
      if (car) {
        ...
        totalHpGain += Math.round(changes.hpGain * multiplier);
      } else {
        totalHpGain += changes.hpGain;
      }
    }
    ...
    if (changes.torqueGain) {
      if (car) {
        const multiplier = getHpGainMultiplier(car, upgrade);
        totalTorqueGain += Math.round(changes.torqueGain * multiplier);
      } else {
        totalTorqueGain += changes.torqueGain;
      }
    }
    
    // Other metrics - direct summation
