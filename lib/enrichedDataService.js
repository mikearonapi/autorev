...
// ============================================================================
// CAR ID RESOLUTION HELPER
// ============================================================================

/**
 * Resolve car_slug to car_id for efficient subsequent queries.
 * Caches the resolution during a single request lifecycle.
 * @param {string} carSlug
 * @returns {Promise<string|null>} car_id (UUID) or null if not found
 */
export async function getCarIdFromSlug(carSlug) {
  if (!isSupabaseConfigured || !carSlug) return null;
  ...
  try {
    ...
    if (error && error.code !== 'PGRST116') {
      console.error('[EnrichedData] Error resolving car_id:', error);
      return null;
    }
    
    return data?.id || null;
  } catch (err) {
    console.error('[EnrichedData] Unexpected error resolving car_id:', err);
    return null;
  }
}
 ...
 * @param {string} carSlug 
 * @param {string} [carId] - Optional car_id for optimized query (skips slug lookup)
 * @returns {Promise<Object|null>}
 */
export async function getFuelEconomy(carSlug, carId = null) {
  if (!isSupabaseConfigured) return null;
  
  try {
    // Resolve car_id if not provided (car_slug column no longer exists on car_fuel_economy)
    const resolvedCarId = carId || await getCarIdFromSlug(carSlug);
    if (!resolvedCarId) return null;
    
    const FUEL_COLUMNS = 'id, car_id, city_mpg, highway_mpg, combined_mpg, fuel_type, tank_size_gallons, annual_fuel_cost, epa_score, source, fetched_at, created_at';
 ...
 * @param {string} carSlug 
 * @param {string} [carId] - Optional car_id for optimized query
 * @returns {Promise<Object|null>}
 */
export async function getSafetyData(carSlug, carId = null) {
  if (!isSupabaseConfigured) return null;
  
  try {
    // Resolve car_id if not provided (car_slug column no longer exists on car_safety_data)
    const resolvedCarId = carId || await getCarIdFromSlug(carSlug);
    if (!resolvedCarId) return null;
    
    const SAFETY_COLUMNS = 'id, car_id, nhtsa_overall, nhtsa_frontal_driver, nhtsa_frontal_passenger, nhtsa_side_driver, nhtsa_side_passenger, nhtsa_rollover, iihs_overall, iihs_frontal, iihs_side, iihs_roof, iihs_headlights, source, fetched_at, created_at';
 ...
 * @returns {Promise<Object|null>}
 */
export async function getMarketPricing(carSlug, carId = null) {
  if (!isSupabaseConfigured) return null;
  
  try {
    // Resolve car_id if not provided (car_slug column no longer exists on car_market_pricing)
    const resolvedCarId = carId || await getCarIdFromSlug(carSlug);
    if (!resolvedCarId) return null;
    
    const PRICING_COLUMNS = 'id, car_id, avg_price, price_low, price_high, price_trend, sample_count, condition_avg, mileage_avg, sources, fetched_at, created_at';
 ...
 */
export async function getPriceHistory(carSlug, days = 365, carId = null) {
  if (!isSupabaseConfigured) return [];
  
  try {
    // Resolve car_id if not provided (car_slug column no longer exists on car_price_history)
    const resolvedCarId = carId || await getCarIdFromSlug(carSlug);
    ...
    const HISTORY_COLUMNS = 'id, car_id, avg_price, price_low, price_high, sample_count, recorded_at, created_at';
 ...
 * @returns {Promise<Array>}
 */
export async function getExpertReviews(carSlug, carId = null) {
  if (!isSupabaseConfigured) return [];
  
  try {
    // Resolve car_id if not provided (car_slug column no longer exists on car_expert_reviews)
    const resolvedCarId = carId || await getCarIdFromSlug(carSlug);
    if (!resolvedCarId) return [];
    
    const EXPERT_REVIEW_COLS = 'id, car_id, source, author, review_date, overall_rating, pros, cons, verdict, excerpt, url, created_at';
    
    const { data, error } = await supabase
      .from('car_expert_reviews')
      .select(EXPERT_REVIEW_COLS)
      .eq('car_id', resolvedCarId)
 ...
 * @returns {Promise<Object>}
 */
export async function checkDataFreshness(carSlug, carId = null) {
  if (!isSupabaseConfigured) {
  ...
  try {
    // Check fuel economy (uses car_id if provided)
    const fuelData = await getFuelEconomy(carSlug, carId);
    ...
    // Check safety data (uses car_id if provided)
    const safetyData = await getSafetyData(carSlug, carId);
    ...
    // Check pricing data (uses car_id if provided)
    const pricingData = await getMarketPricing(carSlug, carId);
 ...
 * Optimized: Resolves car_id once, then uses it for all sub-queries
 * @param {string} carSlug 
 * @returns {Promise<Object>}
 */
export async function getAllEnrichedData(carSlug) {
  // Resolve car_id once for efficient subsequent queries
  const carId = await getCarIdFromSlug(carSlug);
  
  // Use car_id for all sub-queries (falls back to car_slug if car_id not found)
  const [fuelEconomy, safetyData, marketPricing, expertReviews, priceHistory, freshness] = await Promise.all([
    getFuelEconomy(carSlug, carId),
    getSafetyData(carSlug, carId),
    getMarketPricing(carSlug, carId),
    getExpertReviews(carSlug, carId),
    getPriceHistory(carSlug, 365, carId),
    checkDataFreshness(carSlug, carId),
  ]);
 ...
 * @returns {Promise<Array>}
 */
export async function getManualData(carSlug, dataType = null, carId = null) {
  if (!isSupabaseConfigured) return [];
  
  try {
    // Resolve car_id if not provided (car_slug column no longer exists on car_manual_data)
    const resolvedCarId = carId || await getCarIdFromSlug(carSlug);
    if (!resolvedCarId) return [];
    
    const MANUAL_COLUMNS = 'id, car_id, data_type, data_value, source, notes, created_by, created_at, updated_at';
