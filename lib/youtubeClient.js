...
import { resolveCarId, resolveCarIds } from './carResolver.js';
...
export async function fetchChannels({ tier, activeOnly = true } = {}) {
  ...
  const CHANNEL_COLUMNS = 'id, channel_id, channel_name, channel_url, subscriber_count, video_count, credibility_tier, is_active, specialties, notes, created_at, updated_at';
...
export async function getVideoById(videoId) {
  ...
  const VIDEO_COLUMNS = 'id, video_id, channel_id, title, description, thumbnail_url, duration_seconds, published_at, view_count, like_count, comment_count, processing_status, transcript, summary, key_points, car_mentions, created_at, updated_at';
 ...
 * @param {string} [options.carId] - Optional pre-resolved car_id (avoids extra lookup)
 * @returns {Promise<Array>} Array of videos with link metadata
 */
export async function fetchVideosForCar(carSlug, { limit = 5, role, editorsPickOnly = false, carId = null } = {}) {
  if (!isSupabaseConfigured) return [];

  // Resolve car_id if not provided (car_slug column no longer exists on youtube_video_car_links)
  const resolvedCarId = carId || await resolveCarId(carSlug);
  if (!resolvedCarId) return []; // Cannot query without car_id
  
  // Query the join table with video details using car_id
  let query = supabase
...
export async function getComparisonVideos(carSlugs) {
  ...
  // Resolve all car_ids in a single batch query (car_slug column no longer exists)
  const carIdMap = await resolveCarIds(carSlugs);
  const carId1 = carIdMap.get(carSlugs[0]);
  const carId2 = carIdMap.get(carSlugs[1]);
  
  // Cannot query without car_ids
  if (!carId1 || !carId2) return [];
 ...
 * @param {string} carSlug - Car slug
 * @param {string} [carId] - Optional pre-resolved car_id
 * @returns {Promise<Object>} Consensus data
 */
export async function calculateCarConsensus(carSlug, carId = null) {
  if (!isSupabaseConfigured) return null;
  ...
  // Fetch all links for this car with video quality info using car_id
...
export async function getNextQueueItems(limit = 10) {
  ...
  const QUEUE_COLUMNS = 'id, video_id, channel_id, status, priority, discovered_at, processed_at, error_message, created_at';
