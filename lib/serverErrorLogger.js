...
export async function logServerError(error, request = null, context = {}) {
  ...
  try {
    ...
    let pageUrl = null;
    let requestId = context.requestId || generateRequestId();
    ...
    // Use the upsert_error function for deduplication
    const { data, error: dbError } = await supabase.rpc('upsert_error', {
    ...
    if (dbError) {
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          feedback_type: 'bug',
          category: 'auto-error',
          message: `[${errorSource.toUpperCase()}] ${error?.message || 'Unknown server error'}`,
          severity,
          ...
          status: 'new',
          priority: severity === 'blocking' ? 'urgent' : severity === 'major' ? 'high' : 'normal',
        });
      
      if (insertError) {
        console.error('[ServerErrorLogger] Fallback insert also failed:', insertError);
      }
    }
  ...
  try {
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        feedback_type: 'bug',
        category: 'auto-error',
        message: `[SLOW] ${apiRoute} took ${(duration / 1000).toFixed(1)}s`,
        severity: duration > 15000 ? 'major' : 'minor',
        error_source: 'api',
        feature_context: extractFeatureFromRoute(apiRoute),
        error_metadata: {
    ...
    if (insertError) {
      console.error('[ServerErrorLogger] Failed to log slow request:', insertError);
    }
  } catch (err) {
