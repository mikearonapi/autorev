...
export function createVehicleState(car, physicsParams = {}, mods = []) {
  ...
  const state = {
    ...
    // Drivetrain
    drivetrain: physicsParams.drivetrain || car.driveType || 'RWD',
...
function applyModifications(state, mods) {
  ...
  if (tireMods.length > 0) {
    const result = tires.applyTireMods(
      { compound: tires.TIRE_COMPOUNDS[state.tireCompound] },
      tireMods
    );
...
export function analyzePerformance(vehicleState, options = {}) {
  const { ambientTempC = 25, altitudeFt = 0, surfaceGrip = 1.0 } = options;
  ...
  // Tire grip analysis
  const compound =
    tires.TIRE_COMPOUNDS[vehicleState.tireCompound] || tires.TIRE_COMPOUNDS['summer-sport'];
  results.tireGrip = {
  ...
  const frontRideFreq = suspension.calculateRideFrequency(
    frontWheelRate,
    (vehicleState.weight * vehicleState.frontWeightPercent) / 100 / 2
  );
  const rearRideFreq = suspension.calculateRideFrequency(
    rearWheelRate,
    (vehicleState.weight * (100 - vehicleState.frontWeightPercent)) / 100 / 2
  );

  const frontRollStiffness = suspension.calculateRollStiffness(
    frontWheelRate,
    vehicleState.trackWidthFront
  );
  const rearRollStiffness = suspension.calculateRollStiffness(
    rearWheelRate,
    vehicleState.trackWidthRear
  );
  ...
  const radiator =
    cooling.RADIATOR_SIZES[vehicleState.radiatorType] || cooling.RADIATOR_SIZES['stock-medium'];

  results.thermal = {
    heatGenerated: heatRejection,
    radiatorCapacity: cooling.analyzeRadiatorCapacity(
      radiator,
      heatRejection.coolantHeatWatts,
      ambientTempC,
      60
    ),
    oilTemp: vehicleState.hasOilCooler
      ? cooling.calculateOilTemperature(
          heatRejection.oilHeatWatts,
          cooling.OIL_COOLER_TYPES['air-medium'],
          ambientTempC
        )
      : cooling.calculateOilTemperature(
          heatRejection.oilHeatWatts,
          cooling.OIL_COOLER_TYPES['none'],
          ambientTempC
        ),
  };
  ...
  return {
    ...
    differences: {
      zeroTo60: {
        ...
        percentChange:
          ((modAnalysis.summary.zeroTo60 - baseAnalysis.summary.zeroTo60) /
            baseAnalysis.summary.zeroTo60) *
          100,
      ...
      quarterMile: {
        ...
        percentChange:
          ((modAnalysis.summary.quarterMile - baseAnalysis.summary.quarterMile) /
            baseAnalysis.summary.quarterMile) *
          100,
      ...
      topSpeed: {
        ...
        percentChange:
          ((modAnalysis.summary.topSpeed - baseAnalysis.summary.topSpeed) /
            baseAnalysis.summary.topSpeed) *
          100,
      ...
      braking: {
        ...
        percentChange:
          ((modAnalysis.summary.brakingFrom60 - baseAnalysis.summary.brakingFrom60) /
            baseAnalysis.summary.brakingFrom60) *
          100,
      ...
      lateralG: {
        ...
        percentChange:
          ((modAnalysis.summary.maxLateralG - baseAnalysis.summary.maxLateralG) /
            baseAnalysis.summary.maxLateralG) *
          100,
