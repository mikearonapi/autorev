...
const _GRAVITY = 32.174; // ft/s²
const LB_TO_KG = 0.453592;
const IN_TO_FT = 1/12;
const _HP_TO_KW = 0.7457;
 ...
 */
export function calculateYawMoi(mass, wheelbase, trackWidth, _frontPercent = 50) {
  // Convert to metric
...
export function calculatePowerToWeight(hp, weightLbs, torque = null) {
  ...
  const _hpPerKg = hp / (weightLbs * LB_TO_KG);
...
export function calculateWeightChangeEffect(originalWeight, weightChange, hp, original060) {
  ...
  const _lateralGripChange = -weightChangePercent * 0.3; // Partial correlation
...
export function applyWeightMods(baseWeight, baseFrontPercent, mods) {
  ...
  const _newRearWeight = baseRearWeight + rearWeightChange;
