 ...
 *
 * NOTE: braking_60_0 and lateral_g are needed for Performance HUB calculations
 * in Tuning Shop's UpgradeCenter component. Without these, performance scores
 * may be incomplete.
 */
...
function normalizeCarFromSupabase(car) {
  ...
  return {
    ...
    // Year/Make/Model/Trim (for cascading selector)
    model: car.model,
    trim: car.trim,
    ...
    try {
      const { data, error: _error } = await withRetry(async () => {
        ...
        const result = await withTimeout(
          supabase.from('cars').select(LIST_VIEW_COLUMNS).order('price_avg', { ascending: true })
        );
