...
export const ENGINE_PLATFORMS = {
  ...
  'S58': {
    ...
    patterns: [
      /S58/i,
      /3\.0L.*Twin.*Turbo.*I6.*\(S58\)/i,
      /3\.0L.*TT.*Inline-6.*\(S58\)/i,
    ],
    manufacturers: ['bmw'],
...
export function identifyEnginePlatform(car) {
  ...
  // Parse car year from years field (e.g., "2018-2024" → 2018)
  let carYear = null;
  if (car.years) {
  ...
  for (const [platformKey, platform] of Object.entries(ENGINE_PLATFORMS)) {
    ...
    if (platform.manufacturers && platform.manufacturers.length > 0) {
      const mfrMatch = platform.manufacturers.some(mfr => 
        manufacturer.includes(mfr) || carSlug.includes(mfr)
      );
    ...
    if (platform.modelHints) {
      const modelMatch = platform.modelHints.some(hint => 
        carName.includes(hint.toLowerCase()) || carSlug.includes(hint.toLowerCase().replace(/\s+/g, '-'))
      );
...
export function getPlatformsForManufacturer(manufacturer) {
  ...
  return Object.values(ENGINE_PLATFORMS).filter(p => 
    p.manufacturers?.includes(mfr)
  );
}
