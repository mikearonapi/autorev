...
const CONFIG = {
  ...
  weights: {
    ...
    engagement: 0.25,
    relevance: 0.2,
    learned: 0.2, // New: learned preferences from interaction history
    random: 0.1,
  },
...
function calculateEngagementScore(post) {
  const { like_count = 0, comment_count = 0, view_count = 0, share_count = 0 } = post;
...
function calculateLearnedScore(post, learnedPreferences) {
  ...
  const makePref = preferredMakes.find(
    (p) => p.make?.toLowerCase() === post.car_make?.toLowerCase()
  );
  if (makePref) {
  ...
  const makeDislike = dislikedMakes.find(
    (p) => p.make?.toLowerCase() === post.car_make?.toLowerCase()
  );
  if (makeDislike) {
...
function scorePost(post, options = {}) {
  const { userContext = null, learnedPreferences = null, sessionSeed = 'default' } = options;
...
export function rankFeed(posts, options = {}) {
  ...
  const diversifiedPosts = skipDiversity ? scoredPosts : applyDiversityRules(scoredPosts);
...
export function buildUserContext(userData) {
  ...
  const { ownedVehicles = [], favorites = [], recentViews = [], projects = [] } = userData;
...
export async function trackFeedImpression(userId, posts, sessionId) {
  ...
  try {
    ...
    const interactions = posts.map((post, index) => ({
      ...
      post_id: post.id,
      car_id: post.car_id || null, // car_id is the single source of truth
      car_make: post.car_make || null,
      action: 'impression',
      feed_position: index,
    }));

    const { error } = await supabase.from('feed_interactions').insert(interactions);
...
export async function trackEngagement(params) {
  const {
    ...
    sessionId,
    postId,
    carSlug: _carSlug, // Deprecated: use carId instead
    carMake,
    carId,
    action,
    feedPosition,
...
export async function getLearnedPreferences(userId) {
  ...
  try {
    ...
    const PREF_COLS =
      'id, user_id, preferred_categories, blocked_users, content_freshness_weight, engagement_weight, diversity_weight, created_at, updated_at';
...
export async function recalculateUserPreferences(userId) {
  ...
  try {
    ...
    const confidence = Math.min(
      totalInteractions / CONFIG.learning.minInteractionsForConfidence,
      1
    );
    ...
    console.log(
      `[FeedAlgorithm] Updated preferences for user ${userId}: ${totalInteractions} interactions, ${confidence.toFixed(2)} confidence`
    );
