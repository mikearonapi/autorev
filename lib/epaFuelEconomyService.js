...
export async function searchVehicles(year, make, model) {
  ...
  const vehicles = await Promise.all(options.map((opt) => getVehicleById(opt.id)));
...
export async function getFuelEconomy(year, make, model) {
  ...
  const performanceVariant = vehicles.find(
    (v) =>
...
export function calculateAnnualFuelCost(vehicle, annualMiles = 12000, fuelPrices = null) {
  ...
  const pricePerGallon = fuelPrices?.premium || fuelPrices?.regular || 4.0;
...
export async function matchCarToEpaData(car) {
  ...
  // Handle new schema: year (integer), with fallback to old years string
  let year = car.year;
  if (!year && car.years) {
...
export async function getComprehensiveFuelData(car) {
  ...
  return {
    ...
    comparison: userMpg
      ? {
          epaVsReal:
            epaData.combinedMpg && userMpg.averageMpg
...
export default epaFuelEconomyService;
