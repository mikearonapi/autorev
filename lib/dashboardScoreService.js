...
export async function getDailyActivityBreakdown(userId) {
  ...
  try {
    ...
    return dayDates.map(date => ({
      ...
      data: 0, // Track sessions would need separate tracking
    }));
  } catch (error) {
...
export async function getMonthlyActivityBreakdown(userId) {
  ...
  try {
    ...
    // Build daily activity array for the month
...
export async function getYearlyActivityBreakdown(userId) {
  ...
  try {
    ...
    // Build monthly activity array for the year
...
function getEmptyDailyActivity() {
  return Array(7).fill(null).map(() => ({ al: 0, community: 0, data: 0 }));
}
...
function getEmptyYearlyActivity() {
  return Array(12).fill(null).map((_, idx) => ({ month: idx + 1, al: 0, community: 0, data: 0 }));
}
