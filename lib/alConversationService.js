 ...
 */
function generateConversationTitle(message, _carSlug = null) {
...
export async function createConversation(
  userId,
  { carSlug = null, carId = null, page = null, title = null, firstMessage = null } = {}
) {
  ...
  try {
    ...
    // Resolve car_id from slug if not provided (car_id is single source of truth)
    ...
    const { data, error } = await client
      ...
      .insert({
        user_id: userId,
        initial_car_id: resolvedCarId, // car_id is the single source of truth
        initial_page: page,
...
export async function getConversation(conversationId) {
  ...
  // Uses JOIN to cars table to get slug from car_id (normalized data model)
  const CONVERSATION_COLUMNS =
    'id, user_id, title, summary, initial_car_id, initial_page, message_count, total_credits_used, is_archived, is_pinned, last_message_at, created_at, updated_at, cars:initial_car_id (slug, name)';
  const MESSAGE_COLUMNS =
    'id, conversation_id, role, content, tool_calls, credits_used, response_tokens, car_context_slug, car_context_name, data_sources, sequence_number, created_at';

  try {
    // Get conversation (JOIN to cars for slug)
    ...
    const conversation = conversationData
      ? {
          ...conversationData,
          initial_car_slug: conversationData.cars?.slug || null,
        }
      : null;

    if (convError) {
      console.error(
        '[AL Conversations] getConversation query error:',
        convError.message,
        convError.code,
        { conversationId }
      );
...
export async function getUserConversations(
  userId,
  { limit = 20, offset = 0, includeArchived = false, carSlug = null } = {}
) {
  const client = getServerClient();
  if (!client || !userId) {
  ...
  try {
    ...
    const conversationsWithPreviews = (data || []).map((conv) => ({
      ...conv,
      preview:
        conv.preview && conv.preview.length === 120
          ? conv.preview.trim() + '...'
 ...
 */
async function getUserConversationsLegacy(
  userId,
  { limit = 20, offset = 0, includeArchived = false, carSlug = null } = {}
) {
  const client = getServerClient();
        ...
        try {
          ...
          const preview = previewMessage?.content
            ? previewMessage.content.substring(0, 120).replace(/\n/g, ' ').trim() +
              (previewMessage.content.length > 120 ? '...' : '')
            : null;
 ...
 *
 * Includes idempotency check to prevent duplicate consecutive messages
 * (e.g., from double-clicks, retries, or race conditions).
 *
 * @param {string} conversationId - Conversation ID
...
export async function addMessage(
  ...
  try {
    // =============================================================================
    // IDEMPOTENCY CHECK: Prevent duplicate consecutive messages
    // Checks if same role + content was stored in the last 30 seconds
    // =============================================================================
    ...
    // Normalize content for comparison (trim, lowercase, collapse whitespace)
    const normalizeContent = (str) => str?.trim().toLowerCase().replace(/\s+/g, ' ') || '';

    if (
      recentDuplicate &&
      normalizeContent(recentDuplicate.content) === normalizeContent(content)
    ) {
      console.warn(
        `[AL Conversations] Duplicate message detected (same ${role} message within 30s), skipping insert`
      );
      // Return the existing message as if it was just created
      return { message: recentDuplicate, success: true, wasDuplicate: true };
    }
    ...
    if (rpcError) {
      ...
      console.warn(
        '[AL Conversations] add_al_message RPC failed, falling back to direct insert:',
        rpcError
      );
...
export async function deleteConversation(conversationId) {
  ...
  try {
    ...
    const { error } = await client.from('al_conversations').delete().eq('id', conversationId);
