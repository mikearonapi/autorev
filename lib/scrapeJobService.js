...
export async function getPendingJobs(limit = 10) {
  ...
  try {
    ...
    const JOB_COLS = 'id, car_id, car_slug, job_type, status, priority, scheduled_for, started_at, completed_at, result, error_message, attempts, created_at';
 ...
 * @param {string} jobType 
 */
async function persistJobResults(carSlug, result, _jobType) {
  // Save fuel economy
...
/**
 * Clean up stuck "running" jobs that have been running longer than maxHours
 * These are zombie jobs that never completed (process crash, timeout, etc.)
 * @param {number} maxHours - Jobs running longer than this are considered stuck (default: 2)
 * @returns {Promise<Object>}
 */
export async function cleanupStuckJobs(maxHours = 2) {
  if (!isSupabaseConfigured) {
    return { success: false, error: 'Supabase not configured' };
  }
  ...
  try {
    ...
    const { data, error } = await client
      ...
      .update({
        status: 'failed',
        completed_at: new Date().toISOString(),
        error_message: `Job timed out after ${maxHours}+ hours - marked stale by cleanup`,
      })
      .eq('status', 'running')
      .lt('started_at', cutoffTime.toISOString())
      .select('id, job_type, source_key, started_at');
    
    if (error) throw error;
    
    const cleanedCount = data?.length || 0;
    if (cleanedCount > 0) {
      console.log(`[ScrapeJob] Cleaned up ${cleanedCount} stuck jobs`);
    }
    
    return { success: true, cleanedCount, jobs: data || [] };
  } catch (err) {
    console.error('[ScrapeJob] Error cleaning up stuck jobs:', err);
    return { success: false, error: err.message };
  }
}

const scrapeJobService = {
  JOB_TYPES,
  createJob,
  createBatchJobs,
  getPendingJobs,
