...
export const DEFAULT_DESCRIPTION =
  'Find your perfect sports car, plan performance builds with purpose, and join a community that values mastery over materialism.';
...
export function generateCarMetadata(car) {
  ...
  const yearDisplay = car.year ? String(car.year) : '';
  const msrpDisplay = car.msrp ? `$${car.msrp.toLocaleString()}` : '';
  const hp = car.hp ? `${car.hp} HP` : '';

  // Combine make + name for full display name (name no longer includes make prefix)
  const fullName = car.make ? `${car.make} ${car.name}` : car.name;

  const title = `${fullName} - Specs, Reviews & Pricing`;
  const description = `Complete guide to the ${fullName}${yearDisplay ? ` (${yearDisplay})` : ''}. ${hp ? `${hp}. ` : ''}${msrpDisplay ? `MSRP: ${msrpDisplay}. ` : ''}Performance scores, ownership costs, known issues, and expert reviews.`;
...
export function generateOrganizationSchema() {
  return {
    ...
    description:
      'Sports car research, performance builds, and an enthusiast community built on excellence over ego.',
...
export function generateVehicleSchema(car, reviewData = null) {
  ...
  // Get model year from new year field
  const modelYear = car.year ? String(car.year) : undefined;

  // Use msrp for price
  const priceValue = car.msrp ? String(car.msrp) : undefined;
  ...
  if (car.hp) {
    ...
    if (car.engineType) {
      schema.vehicleEngine.name = car.engineType;
    }
    if (car.torque) {
  ...
  if (car.driveType) {
    schema.driveWheelConfiguration = car.driveType;
  }
...
export function generateEventSchema(event) {
  ...
  // Determine event status based on status field or if past
  const getEventStatus = () => {
    // Check for explicit status from database
    ...
    if (eventDate) {
      const endDate = new Date(eventDate + 'T23:59:59');
      if (endDate < new Date()) {
        // Past events are still "Scheduled" in schema.org terms
        // but we could use EventMovedOnline if it was virtual
 ...
 * Helps with local SEO and provides contact information in search results.
 *
 * @param {Object} options
 * @param {string} options.path - URL path (e.g., '/contact')
 * @param {string} [options.email] - Contact email address
 * @param {string} [options.telephone] - Contact phone number
...
export function generateContactPageSchema({
  ...
  email = 'contact@autorev.app',
  telephone,
  description = 'Contact AutoRev for questions about sports cars, performance upgrades, or general inquiries.',
}) {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'ContactPage',
    name: 'Contact AutoRev',
    description,
    url: getCanonicalUrl(path),
    mainEntity: {
      '@type': 'Organization',
  ...
  if (telephone) {
    schema.mainEntity.telephone = telephone;
    schema.mainEntity.contactPoint.telephone = telephone;
  }
