...
export function generateCarMetadata(car) {
  ...
  // Combine brand + name for full display name (name no longer includes brand prefix)
  const fullName = car.brand ? `${car.brand} ${car.name}` : car.name;

  const title = `${fullName} - Specs, Reviews & Pricing`;
  const description = `Complete guide to the ${fullName}${yearRange ? ` (${yearRange})` : ''}. ${hp ? `${hp}. ` : ''}${priceRange ? `Price: ${priceRange}. ` : ''}Performance scores, ownership costs, known issues, and expert reviews.`;
