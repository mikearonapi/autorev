...
export async function fetchComparisonPage(slug) {
  ...
  try {
    const COMPARISON_COLS =
      'id, slug, title, description, car_slugs, winner_slug, content, seo_title, seo_description, is_published, view_count, created_at, updated_at';

    // Fetch comparison page
    const { data: comparison, error: comparisonError } = await supabase
      .from('comparison_pages')
      .select(COMPARISON_COLS)
      .eq('slug', slug)
    ...
    supabase.rpc('increment_comparison_views', { p_slug: slug }).then(({ error }) => {
      // eslint-disable-next-line no-console -- non-critical background operation
 ...
 *
 * @param {number} carId - Car ID to find comparisons for
 * @param {number} limit - Max results
 * @returns {Promise<{data: Array|null, error: Error|null}>}
 */
export async function fetchComparisonsForCar(carId, limit = 5) {
  if (!isSupabaseConfigured || !supabase) {
    return { data: null, error: new Error('Supabase not configured') };
  }
...
export function generateSpecComparison(cars) {
  ...
  const specs = [
    { key: 'engine_type', label: 'Engine', format: (v) => v || '—' },
    ...
    { key: 'drive_type', label: 'Drivetrain', format: (v) => v || '—' },
    {
      key: 'curb_weight',
      label: 'Curb Weight',
    ...
    { key: 'msrp', label: 'MSRP', format: (v) => (v ? `$${v.toLocaleString()}` : '—') },
    {
      key: 'msrp_new_low',
      label: 'MSRP (New)',
      format: (v, car) => {
...
function determineWinner(cars, key) {
  ...
  const lowerIsBetter = ['curb_weight', 'zero_to_sixty', 'quarter_mile', 'msrp'];
