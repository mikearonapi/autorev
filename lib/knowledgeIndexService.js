...
import { resolveCarId as resolveCarIdFromResolver } from './carResolver.js';
import { 
  chunkText, 
...
// Wrapper to maintain backwards compatibility with (client, carSlug) signature
async function resolveCarId(client, carSlug) {
  return resolveCarIdFromResolver(carSlug, { client, skipCache: true });
}
...
function requireOpenAI() {
  if (!isEmbeddingConfigured()) throw new Error('OPENAI_API_KEY not configured');
}

/**
 * Wrapper for embedding generation that throws on error (legacy behavior).
 * @param {string} text
...
// Re-export OPENAI_EMBEDDING_MODEL for compatibility
...
export async function ingestKnowledgeText({
  ...
  text,
  metadata = {},
  confidenceLevel = 0.7, // Default: curated content (0.0-1.0 scale)
}) {
  ...
  if (!documentId) {
    const { data: inserted, error: insErr } = await client
      ...
      .insert({
        ...
        raw_text: text,
        raw_json: null,
        source_confidence: Math.max(0, Math.min(1, confidenceLevel)), // Store default confidence for source
        metadata: {
          ...(metadata || {}),
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const { error } = await client
      ...
      .insert({
        ...
        embedding: pgVec,
        confidence_level: Math.max(0, Math.min(1, confidenceLevel)), // Clamp to 0-1
