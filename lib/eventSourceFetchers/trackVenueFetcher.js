...
function parseJsonLdEvent(jsonLd, track) {
  ...
  try {
    ...
    // Silently ignore invalid date format - startDate remains null
  }
  ...
  if (jsonLd.endDate) {
    try {
      ...
      // Silently ignore invalid end date format - endDate remains null
    }
  }
  ...
  for (const pattern of datePatterns) {
    ...
    if (match && match[1]) {
      try {
        ...
        // Silently ignore invalid date format from regex match
      }
    }
  }
