...
export const fuelSystemTopics = [
  {
    ...
    definition:
      'The low-pressure fuel pump (also called the lift pump or in-tank pump) moves fuel from the tank to the engine bay at relatively low pressure, typically 40-70 PSI. In direct injection systems, this pump feeds the high-pressure pump; in port injection systems, it delivers fuel directly to the injectors.',

    howItWorks:
      'Most modern vehicles use an electric fuel pump submerged in the fuel tank. The pump motor spins an impeller or gerotor that pressurizes fuel and pushes it through the fuel lines. Being submerged in fuel keeps the pump cool and prevents vapor lock. A fuel pressure regulator maintains consistent pressure, returning excess fuel to the tank. The fuel pump is controlled by the ECU and typically primes when the key is turned on (you can hear it whir for 2-3 seconds).',

    whyItMatters:
      'The fuel pump must supply enough volume to feed the engine at maximum power. Stock pumps are sized for stock power with some margin—adding significant power through turbocharging or other means often requires a higher-flow pump. An undersized pump causes fuel starvation at high RPM/load, leading to lean conditions that can destroy an engine.',
    ...
    modPotential: {
      summary:
        'Fuel pump upgrades are essential for any significant power increase, especially forced induction builds.',
      gains:
        'Higher-flow pumps support 30-100%+ more power depending on setup. E85-compatible pumps enable flex-fuel setups.',
      considerations:
        'Must match pump to injector/fuel system capacity. Wiring upgrades often needed for high-draw pumps. Return-style conversions may be needed. $150-600 for quality pumps.',
    },
...

  {
    ...
    definition:
      "The high-pressure fuel pump (HPFP) is used in direct injection systems to boost fuel pressure from the low-pressure pump's 40-70 PSI to the 500-3,000+ PSI required for direct injection. It's typically mechanically driven by the camshaft and is a critical component in modern turbocharged engines.",

    howItWorks:
      "The HPFP is a cam-driven piston pump mounted on the cylinder head. A lobe on the camshaft actuates the pump's piston, compressing fuel to extreme pressures. A solenoid controls how much fuel is pressurized each stroke by varying the effective piston travel. The ECU modulates this solenoid to maintain target rail pressure based on engine load and RPM. Higher pressures enable better fuel atomization and more precise injection timing.",

    whyItMatters:
      "The HPFP is often the first fuel system component to reach its limit on tuned direct injection engines. When the pump can't maintain pressure at high loads, the engine runs lean—a dangerous condition. HPFP upgrades are common on popular platforms like the BMW N54/N55, VW/Audi EA888, and Ford EcoBoost engines.",
    ...
    modPotential: {
      summary:
        'HPFP upgrades are essential for tuned DI engines, often supporting 30-100% more power.',
      gains:
        'Upgraded internals can double or triple fuel flow. Enables higher boost and more aggressive timing.',
      considerations:
        'Must match LPFP capacity. May require upgraded cam follower. Some platforms have multiple aftermarket options. $400-1,200 for quality upgrades.',
    },
...

  {
    ...
    definition:
      'The fuel rail is a pipe or manifold that distributes pressurized fuel to all injectors in the engine. It maintains consistent fuel pressure across all cylinders and provides a mounting point for the fuel injectors. In direct injection systems, the fuel rail operates at extremely high pressures.',

    howItWorks:
      "Fuel enters the rail from the fuel pump and fills the entire rail volume. Each injector is sealed into the rail and draws fuel when opened by the ECU. The rail's internal volume acts as a pressure accumulator, dampening pressure fluctuations as injectors fire. A fuel pressure sensor monitors rail pressure, and in port injection systems, a regulator maintains pressure by returning excess fuel to the tank.",

    whyItMatters:
      'The fuel rail must handle the pressure and flow demands of all injectors simultaneously. Stock rails are sized for stock injectors—upgrading to larger injectors may require a rail with larger internal volume or better flow characteristics to prevent pressure drops when multiple injectors fire together.',
    ...
    modPotential: {
      summary:
        'Aftermarket fuel rails improve flow and accommodate larger injectors for high-power builds.',
      gains:
        'Better pressure stability under high demand. Accommodate larger injectors. Often include -AN fittings for braided lines.',
      considerations:
        'Must match injector size and spacing. May require custom fuel lines. DI fuel rails are expensive due to high-pressure requirements. $150-800 depending on application.',
    },
...

  {
    ...
    definition:
      'Electronic fuel injection (EFI) is a system that uses electronically controlled injectors to precisely meter fuel delivery based on sensor inputs and ECU calculations. EFI replaced carburetors in the 1980s and 1990s due to its superior precision, efficiency, and emissions control capabilities.',
    ...
    modPotential: {
      summary:
        'EFI tuning unlocks significant power potential through optimized fuel maps and often accompanies other modifications.',
      gains:
        'Proper tuning ensures mods work together safely. Custom tuning can add 5-15% power over stock maps.',
      considerations:
        'Professional tuning required for best results. Datalogging essential for monitoring. Wideband O2 recommended for tuning.',
    },
...

  {
    ...
    modPotential: {
      summary:
        'DI system upgrades focus on HPFP and injectors for high-power builds, plus carbon cleaning maintenance.',
      gains:
        'Upgraded HPFP and injectors support major power increases. Port injection supplements can add 50%+ fueling capacity.',
      considerations:
        'Carbon buildup requires walnut blasting or chemical cleaning every 40-60k miles. Port injection kits eliminate carbon issue. DI injectors are expensive ($100-300 each).',
    },
...

  {
    ...
    modPotential: {
      summary:
        'Port injection upgrades center on larger injectors to support more power, or adding port injection to DI engines.',
      gains:
        'Larger injectors support any power level needed. Supplemental PI on DI engines adds fueling capacity and eliminates carbon buildup.',
      considerations:
        'Must size injectors for target power. Large injectors may idle poorly without proper tuning. Injector dynamics (latency) must be calibrated in tune.',
    },
...

  {
    ...
    modPotential: {
      summary:
        'Monitoring pulse width/duty cycle reveals when larger injectors are needed for increased power.',
      gains:
        'Larger injectors reduce duty cycle, providing headroom for more power. Proper injector sizing ensures safe, full-power operation.',
      considerations:
        'Larger injectors require ECU retuning. Injector dead time must be properly calibrated. Oversized injectors can cause poor idle quality.',
    },
...

  {
    ...
    definition:
      'Fuel atomization is the process of breaking liquid fuel into tiny droplets that can mix thoroughly with air and burn completely. Better atomization means more surface area for evaporation and combustion, resulting in more power, cleaner emissions, and improved fuel efficiency.',

    howItWorks:
      "Fuel injectors are designed to spray fuel in specific patterns with controlled droplet sizes, typically 10-100 microns in diameter. Smaller droplets evaporate faster and mix more completely with air. High-pressure direct injection produces finer atomization due to the greater pressure differential. Injector design (pintle, disc, multi-hole), fuel pressure, and spray pattern all affect atomization quality. Poor atomization leaves some fuel as liquid droplets that don't burn completely.",

    whyItMatters:
      'Complete combustion depends on thorough fuel-air mixing, which starts with good atomization. Poor atomization causes incomplete combustion, wasted fuel, increased emissions, and carbon deposits. This is why fuel quality matters—detergent additives help keep injectors clean for proper spray patterns, and why direct injection enables higher efficiency.',
...

  {
    ...
    modPotential: {
      summary:
        'Flex-fuel or E85 conversions can add significant power potential due to higher octane.',
      gains:
        'E85 enables 20-30% more power on turbo cars through more boost and timing. Race gas enables maximum timing advance.',
      considerations:
        'E85 requires 30-40% more fuel system capacity. Ethanol content varies seasonally. Fuel system must be E85-compatible. Separate tunes often needed for different fuels.',
    },
...

  {
    ...
    modPotential: {
      summary:
        'Flex fuel conversions unlock significant power potential, especially on forced induction engines.',
      gains:
        'E85 enables 15-30% more power through more boost and timing. Consistent fuel quality for tuning.',
      considerations:
        'Requires fuel pump, injector, and potentially line upgrades. E85 availability varies by region. 25-40% worse fuel economy on E85. Cold starting can be difficult on pure E85 in winter.',
    },
...

  {
    ...
    definition:
      "The fuel pressure regulator (FPR) maintains consistent fuel pressure in the fuel rail by controlling how much fuel returns to the tank. In return-style systems, the regulator is in the engine bay and references manifold vacuum; in returnless systems, it's integrated with the fuel pump module in the tank.",

    howItWorks:
      "The regulator uses a spring-loaded diaphragm with fuel on one side and manifold vacuum (or atmospheric pressure in returnless systems) on the other. When fuel pressure exceeds the spring's setpoint, the diaphragm opens a return valve, sending excess fuel back to the tank. Vacuum-referenced regulators maintain a constant pressure differential across the injectors regardless of manifold pressure—when vacuum is high (light load), fuel pressure drops; when boost is present, fuel pressure rises.",

    whyItMatters:
      'Consistent fuel pressure is essential for accurate fuel delivery. Injector flow varies with pressure—a 10% pressure drop means roughly 5% less fuel flow. Failing regulators cause rich or lean conditions that affect performance and can damage engines. Adjustable regulators are popular for tuning because they allow fine-tuning of fuel delivery.',
    ...
    modPotential: {
      summary:
        'Adjustable fuel pressure regulators enable fine-tuning of fuel delivery for modified engines.',
      gains:
        'Precise fuel pressure control. Useful when fuel system changes require pressure adjustments. Essential for some turbo setups.',
      considerations:
        "Most EFI systems don't require FPR changes unless significantly modified. Adjustable FPRs should be set with fuel pressure gauge. Must maintain proper boost reference for turbo applications.",
    },
