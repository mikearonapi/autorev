...
export const topicsBySlug = Object.fromEntries(allTopics.map((topic) => [topic.slug, topic]));
...
export function getRelatedTopics(slug) {
  ...
  return topic.relatedTopics.map((relatedSlug) => getTopicBySlug(relatedSlug)).filter(Boolean);
}
...
export default encyclopediaTopics;
