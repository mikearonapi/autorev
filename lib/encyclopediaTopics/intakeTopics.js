...
export const intakeTopics = [
  {
    ...
    definition:
      "The intake system encompasses all components that deliver air from outside the vehicle to the engine's throttle body or turbo inlet. The airbox is an enclosed chamber that houses the air filter, designed to provide clean, consistent airflow while reducing intake noise and protecting the filter from heat and water.",

    howItWorks:
      'Air enters through an opening (often in the front grille or fender) and flows into the airbox, where it passes through the filter element. From there, it travels through intake tubing to the throttle body or (in turbo applications) the turbo compressor inlet. Stock airboxes are engineered to dampen intake noise and draw air from cooler locations. The airbox volume also acts as a resonance chamber, tuned to boost airflow at certain RPM ranges.',

    whyItMatters:
      "The intake system is the first bottleneck in the engine's airflow path. A restrictive intake limits power potential. However, stock systems are often well-designed for street use—they balance performance, noise, and heat management. Aftermarket intakes prioritize flow over other factors, which can provide gains but may also increase noise and potentially intake air temperature.",
    ...
    modPotential: {
      summary:
        'Intake upgrades range from drop-in performance filters to complete cold air intake systems.',
      gains:
        'Typical gains of 5-15 HP depending on engine and restrictiveness of stock system. Better throttle response and intake sound.',
      considerations:
        'Some "cold air" intakes actually draw warmer underhood air. Oiled filters can contaminate MAF sensors. May require ECU recalibration for MAF-based systems.',
    },
...

  {
    ...
    definition:
      'High-flow air filters are designed to allow more airflow than stock paper filters while still filtering contaminants. They typically use cotton gauze, foam, or synthetic media and are often reusable—cleaned and re-oiled rather than replaced.',

    howItWorks:
      "Unlike paper filters that trap particles on the surface, high-flow filters use depth filtration through layers of oiled cotton or foam. The oil helps capture small particles. Multi-layer designs increase surface area while maintaining low restriction. Some filters use dry synthetic media that doesn't require oiling. The goal is to reduce the pressure drop across the filter while maintaining adequate filtration.",
    ...
    modPotential: {
      ...
      gains:
        'Typically 1-5 HP, better throttle response, improved intake sound. Main benefit is reusability and long-term cost savings.',
      considerations:
        'Over-oiling damages MAF sensors. Dry filters avoid this issue. Some filters may reduce filtration efficiency. Clean on recommended schedule.',
    },
...

  {
    ...
    definition:
      'A cold air intake (CAI) is an aftermarket intake system designed to draw cooler air from outside the engine bay, typically from the fender well, bumper area, or front grille. Cooler air is denser, containing more oxygen per volume, which allows the engine to make more power.',

    howItWorks:
      'Cold air intakes relocate the filter away from the hot engine bay, often using a heat shield or enclosed box to isolate the filter from engine heat. Tubing routes air from this cool location to the throttle body. Some designs use ram air principles, capturing high-pressure air from vehicle motion. For every 10°F reduction in intake air temperature, engine power increases by approximately 1% due to increased air density.',

    whyItMatters:
      'Intake air temperature directly affects power—engines make less power on hot days because the air is less dense. A true cold air intake can reduce intake temps by 20-50°F compared to stock, translating to meaningful power gains. However, many "cold air intakes" position the filter in the engine bay where it can actually be hotter than stock, negating benefits.',
    ...
    modPotential: {
      summary:
        'A well-designed cold air intake provides modest power gains and improved throttle response.',
      gains:
        'Typical gains of 5-20 HP depending on how restrictive stock system is. Better sound and throttle feel.',
      considerations:
        'True cold air routing is essential—engine bay intakes may hurt performance. Hydrolock risk if filter can get submerged. May require tune for proper MAF scaling.',
    },
...

  {
    ...
    modPotential: {
      summary:
        'Turbo upgrades range from bolt-on larger units to full custom turbo kits on naturally aspirated cars.',
      gains: 'Upgraded turbos can add 50-500+ HP depending on supporting mods and engine strength.',
      considerations:
        'Requires supporting mods: fuel system, intercooler, exhaust, tune. Heat management critical. May require internal engine upgrades at high power levels. Turbo kits for NA cars cost $3,000-10,000+.',
    },
...

  {
    ...
    definition:
      'A supercharger is a belt or gear-driven air compressor that forces more air into the engine, providing boost without the exhaust-driven lag associated with turbochargers. Superchargers provide immediate throttle response since boost is directly proportional to engine RPM.',
    ...
    modPotential: {
      summary:
        'Supercharger kits add reliable, predictable power with immediate throttle response.',
      gains: 'Typical kits add 40-60% power. Larger pulleys can increase boost further.',
      considerations:
        'Parasitic losses reduce efficiency vs turbos. Heat generation requires good intercooling. Belt/drive maintenance required. Kits typically cost $4,000-8,000+.',
    },
...

  {
    ...
    definition:
      'Pulley upgrades change the drive ratio of a supercharger to increase or decrease boost pressure. A smaller supercharger pulley (or larger crank pulley) spins the supercharger faster, creating more boost. This is one of the simplest and most cost-effective ways to increase power on supercharged vehicles.',

    howItWorks:
      "The ratio between the crankshaft pulley and supercharger pulley determines how fast the blower spins relative to engine speed. A 10% reduction in supercharger pulley diameter increases blower speed by roughly 10%, which increases boost pressure. The relationship between pulley size and boost isn't linear—smaller pulleys also increase heat generation and parasitic load. Proper tuning is required after any pulley change.",

    whyItMatters:
      "For factory supercharged cars (Hellcat, ZL1, GT500, etc.), pulley swaps are often the first modification. They're relatively inexpensive, easy to install, and can add significant power. A typical pulley swap on a modern supercharged V8 can add 50-100+ HP with proper supporting mods and tuning.",
    ...
    modPotential: {
      summary:
        'Pulley upgrades are a cost-effective way to increase boost on supercharged applications.',
      gains:
        '2-5 PSI boost increase typical. Power gains of 30-100+ HP depending on engine and supporting mods.',
      considerations:
        'Must tune for increased boost. Heat management becomes critical. May need smaller idler/tensioner pulleys. Belt upgrades often required. $100-500 for pulley kits.',
    },
...

  {
    ...
    definition:
      "A wastegate is a valve that bypasses exhaust gas around the turbo's turbine, controlling boost pressure. When boost reaches the target, the wastegate opens to divert exhaust away from the turbine, preventing over-boost. Wastegates can be internal (built into the turbo housing) or external (separate unit).",
    ...
    modPotential: {
      ...
      gains:
        'Better boost stability. Eliminate boost creep. Enable higher boost targets. External wastegate sound on screamer pipe.',
      considerations:
        'Larger wastegates may reduce spool at lower boost. External setups require exhaust fabrication. Spring pressure must match boost target. $150-600 for external wastegates.',
    },
...

  {
    ...
    howItWorks:
      'When the throttle closes suddenly (lifting off the gas), pressurized air in the intake has nowhere to go. Without a BOV, this air pushes back against the still-spinning compressor, causing it to stall and reverse. The BOV senses the vacuum created when the throttle closes and opens a valve to release the pressure—either to atmosphere (the signature "psssh" sound) or back to the intake before the turbo (recirculating).',

    whyItMatters:
      "Compressor surge causes accelerated bearing and compressor wheel wear. The flutter sound many find appealing is actually the compressor stalling repeatedly. While occasional surge won't immediately destroy a turbo, consistent surge reduces turbo life. Proper BOV operation also maintains spool between shifts for faster boost recovery.",
    ...
    modPotential: {
      summary:
        'Aftermarket BOVs provide better flow capacity and the option for atmospheric venting.',
      gains:
        'Proper surge protection. Better boost recovery. Adjustable response. The characteristic turbo sound.',
      considerations:
        'Atmospheric BOVs can cause rich conditions on MAF cars (metered air is vented). Recirculating works with all setups. Proper spring pressure is critical. $50-300 for quality BOVs.',
    },
...

  {
    ...
    whyItMatters:
      'Twin-charging represents an engineering solution to the inherent trade-offs of each system alone. Turbos are efficient but laggy; superchargers are responsive but parasitic. Together, they can provide lag-free boost with peak efficiency. However, the complexity, cost, and packaging challenges have limited widespread adoption.',
    ...
    modPotential: {
      summary:
        'Twin-charged conversions are rare but offer the best of both forced induction worlds.',
      gains:
        'Combines immediate response with high-RPM efficiency. Eliminates turbo lag completely.',
      considerations:
        'Very complex to implement properly. High cost and packaging challenges. Requires sophisticated engine management. Limited aftermarket support.',
    },
...

  {
    ...
    definition:
      'An intercooler is a heat exchanger that cools compressed air from the turbo or supercharger before it enters the engine. Compressing air heats it significantly—often to 250-350°F. The intercooler reduces this temperature, increasing air density and reducing the risk of detonation.',

    howItWorks:
      'Hot compressed air flows through the intercooler core, which contains many small passages with fins. In air-to-air intercoolers, ambient air passing over the fins absorbs heat. In water-to-air intercoolers, coolant flowing through the core absorbs heat and is cooled by a separate radiator. Temperature reductions of 100-150°F are typical, with each 10°F reduction worth approximately 1% power increase.',

    whyItMatters:
      'Hot intake air reduces power and increases knock risk. Intercooler sizing is often a limiting factor on factory turbo cars pushed beyond stock power levels. Upgrading to a larger intercooler is one of the most effective modifications for any boosted engine, providing consistent power and safety margin for aggressive tuning.',
    ...
    modPotential: {
      summary:
        'Intercooler upgrades are essential for any serious boost increase on turbo or supercharged applications.',
      gains:
        'Lower intake temps allow more boost and timing. Consistent power run after run. Essential for track use.',
      considerations:
        'Larger cores may have more pressure drop (lag). Front mounts may affect other cooling (radiator, AC). Proper piping is important. $300-1,500 for quality intercoolers.',
    },
...

  {
    ...
    definition:
      'Charge pipes are the tubing that carries pressurized air from the turbo/supercharger through the intercooler and to the throttle body. They must handle boost pressure without leaking or blowing off, while flowing air efficiently to minimize pressure loss and turbulence.',

    howItWorks:
      'Pressurized air exits the turbo compressor, travels through the charge pipe to the intercooler, and then through another pipe to the throttle body. Factory charge pipes are often plastic to reduce cost and weight, but can crack or separate under higher boost pressures. Aftermarket pipes are typically aluminum or silicone-reinforced, with better connections and larger diameters for improved flow.',

    whyItMatters:
      "Charge pipe failures are common on tuned turbo cars—the plastic stock pipes simply weren't designed for significantly higher boost. A blown charge pipe causes immediate and complete loss of boost. Upgrading charge pipes before increasing boost is inexpensive insurance. Larger diameter pipes also reduce restriction for marginal flow improvements.",
    ...
    modPotential: {
      ...
      gains:
        'Eliminates common failure point. Slightly improved flow with larger diameter. Peace of mind at higher boost.',
      considerations:
        'Essential before significant boost increases. Match pipe diameter to turbo outlet. Quality couplers and clamps are important. $150-400 for quality kits.',
    },
...

  {
    ...
    modPotential: {
      summary:
        'Boost controllers enable consistent boost levels and unlock additional boost potential.',
      gains:
        'Consistent boost equals consistent power. Higher boost targets when tuned appropriately. Advanced features like gear-based boost.',
      considerations:
        'Must be tuned properly—overboost damages engines. EBC provides better control than MBC. ECU-integrated control is often simplest. $30-300 depending on type.',
    },
