 ...
 *
 * Updated 2026-01-31: Normalized to use carId (UUID) as primary parameter.
 * Functions accept either carId (UUID) or slug (for backward compatibility) and normalize internally.
 */
...
/**
 * Helper to normalize carId/slug input
 * If input is a UUID, returns it directly. Otherwise treats it as a slug and resolves to carId.
 * @param {string} carIdOrSlug - Either a car_id (UUID) or car_slug
 * @returns {Promise<string|null>} Normalized car_id (UUID) or null
 */
async function normalizeCarId(carIdOrSlug) {
  if (!carIdOrSlug) return null;

  // UUID pattern: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
  const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

  // If it's already a UUID, return it
  if (uuidPattern.test(carIdOrSlug)) {
    return carIdOrSlug;
  }

  // Otherwise, treat as slug and resolve
  return await resolveCarId(carIdOrSlug, { skipCache: true });
}

async function getCarAiContextBlob(carId) {
  const client = getServerDbClient();
  if (!carId || !isSupabaseConfigured || !client) return null;

  try {
    // Normalize input (accepts UUID or slug)
    const normalizedCarId = await normalizeCarId(carId);
    if (!normalizedCarId) return null;

    // RPC expects slug, so resolve carId back to slug
    const carSlug = await getSlugFromCarId(normalizedCarId, { client });
...
export async function getCarBySlug(slug) {
  ...
  // getCarAiContextBlob now accepts carId, so resolve slug to ID first
  const carId = await resolveCarId(slug);
  if (carId) {
    const aiContext = await getCarAiContextBlob(carId);
    if (aiContext?.car) {
 ...
 * @param {string} carId - Car ID (UUID) or slug (for backward compatibility)
 * @returns {Object|null} Maintenance specifications
 */
export async function getMaintenanceSpecs(carId) {
  const client = getServerDbClient();
  if (!carId || !isSupabaseConfigured || !client) return null;
  ...
  try {
    ...
    const MAINT_COLS =
      'id, car_id, oil_type, oil_viscosity, oil_spec, oil_capacity_liters, oil_capacity_quarts, oil_change_interval_miles, coolant_type, coolant_spec, brake_fluid_type, brake_fluid_spec, trans_fluid_type, trans_fluid_spec, spark_plug_type, spark_plug_interval_miles, tire_pressure_front, tire_pressure_rear, created_at';
 ...
 */
export async function getUpgradeRecommendations(carId) {
  const client = getServerDbClient();
  if (!carId || !isSupabaseConfigured || !client) return {};
...
export async function getUserVehicle(userId, vehicleId = null) {
  ...
  try {
    let query = client.from('user_vehicles').select(VEHICLE_COLUMNS).eq('user_id', userId);
...
export async function getUserProfile(userId) {
  ...
  try {
    const { data, error } = await client
      .from('user_profiles')
      .select(
        'display_name, location_zip, location_city, location_state, subscription_tier, user_intent'
      )
      .eq('id', userId)
      .maybeSingle();
...
export async function getServiceLogs(vehicleId) {
  ...
  try {
    const { data, error } = await client
      ...
      .select(
        'id, user_vehicle_id, service_date, service_type, description, mileage_at_service, cost_cents, vendor_name, notes, created_at'
      )
 ...
 * @param {string} userId - User ID
 * @param {string} [carId] - Optional car ID (UUID) or slug (for backward compatibility) filter
 * @returns {Array} User projects
 */
export async function getUserProjects(userId, carId = null) {
  const client = getServerDbClient();
  if (!userId || !isSupabaseConfigured || !client) return [];
  ...
  try {
    let query = client
      .from('user_projects')
      .select(
        'id, user_id, car_id, car_slug, car_name, project_name, selected_upgrades, total_hp_gain, total_cost_low, total_cost_high, final_hp, notes, created_at, updated_at'
      )
      .eq('user_id', userId);
    ...
    if (carId) {
      ...
      if (normalizedCarId) {
        query = query.eq('car_id', normalizedCarId);
      } else {
        // Fallback to slug if resolution failed
        query = query.eq('car_slug', carId);
      }
    }
  ...
  return allCars
    .filter(
      (car) =>
        ...
        car.engineType?.toLowerCase().includes(searchLower)
    )
    .slice(0, 10);
}
 ...
 */
export async function findSimilarCars(carId) {
  if (!carId) return [];

  const allCars = await fetchCars();

  // Normalize input and resolve to slug for array lookup
  const normalizedCarId = await normalizeCarId(carId);
  let carSlug = carId; // Default to input (might be slug)

  if (normalizedCarId && normalizedCarId !== carId) {
    // Input was a slug that got resolved, use original input
    carSlug = carId;
  } else if (normalizedCarId) {
    // Input was a UUID, resolve to slug
    const client = getServerDbClient();
    carSlug = (await getSlugFromCarId(normalizedCarId, { client })) || carId;
  }

  const baseCar = allCars.find((c) => c.slug === carSlug);
  if (!baseCar) return [];

  // Find cars with similar MSRP and HP
  const basePrice = baseCar.msrp || 0;
 ...
 * @param {string} [params.carId] - Car ID (UUID) or slug (for backward compatibility)
 * @param {string} [params.carSlug] - Deprecated: use carId instead. Kept for backward compatibility.
...
export async function buildAIContext({
  ...
  // Normalize car identifier (support both carId and deprecated carSlug)
  const carIdentifier = carId || carSlug;
  ...
  if (carIdentifier) {
    // getCarBySlug accepts slug, so if carId is a UUID, resolve to slug first
    let slugForLookup = carIdentifier;
    const normalizedCarId = await normalizeCarId(carIdentifier);
    if (normalizedCarId && normalizedCarId !== carIdentifier) {
      // It was a slug that got resolved, use original slug
      slugForLookup = carIdentifier;
    } else if (normalizedCarId) {
      // It's a UUID, resolve to slug for getCarBySlug
      const client = getServerDbClient();
      slugForLookup = (await getSlugFromCarId(normalizedCarId, { client })) || carIdentifier;
    }

    promises.push(
      getCarBySlug(slugForLookup).then((data) => {
        context.car = data;
      })
    );
  ...
  if (userId) {
    promises.push(
      getUserVehicle(userId, vehicleId).then((data) => {
        context.userVehicle = data;
      }),
      getAllUserVehicles(userId).then((data) => {
        context.allOwnedVehicles = data;
      }),
      getUserFavorites(userId, 5).then((data) => {
        context.userFavorites = data;
  ...
  context.hasNoCarContext = !context.hasOwnedVehicles && !context.hasFavorites && !carIdentifier;
...
export function formatContextForAI(context) {
  ...
  if (context.allOwnedVehicles?.length > 0) {
    ...
    context.allOwnedVehicles.slice(0, 3).forEach((v, _i) => {
    ...
    const primaryVehicle =
      context.allOwnedVehicles.find((v) => v.is_primary) || context.allOwnedVehicles[0];
    ...
    const favNames = context.userFavorites
      .slice(0, 5)
  ...
  if (context.car) {
    ...
    const yearDisplay = context.car.year || '';
    const msrpDisplay = context.car.msrp ? `$${context.car.msrp.toLocaleString()}` : '';
    contextText += `\n${yearDisplay} | ${context.car.hp || '?'} hp | ${context.car.engineType || ''} | ${msrpDisplay}`;
    if (context.car.slug) {
 ...
 */
export async function webSearch(_query) {
