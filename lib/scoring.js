 ...
 * 
 * IMPORTANT: Recommendations are drawn ONLY from the user's Top 10 ranked cars.
 * This ensures recommendations are relevant - a car ranked #50 overall shouldn't
 * be recommended just because it excels in one category.
 * 
 * @param {Object[]} filteredCars - Array of car objects (already filtered)
 * @param {Object} weights - Object with category keys and weight values
 * @param {number} topN - Number of top cars to consider for recommendations (default: 10)
 * @returns {Object} - Object with recommendation keys and car objects
 */
export function getRecommendations(filteredCars, weights = DEFAULT_WEIGHTS, topN = 10) {
  ...
  // Sort by total weighted score and take only the top N
  const sortedByTotal = [...carsWithScores].sort((a, b) => b.total - a.total);
  const topCars = sortedByTotal.slice(0, topN);

  // Build recommendations from ONLY the top N cars
  const recommendations = {
    top: topCars[0],
  };
  
  // Find the best car for each category, but only from the top N
  // This ensures all recommendations are from cars the user would actually consider
  ...
  categoryKeys.forEach(key => {
    ...
    // Sort top cars by category score, with tiebreaker on total weighted score
    const sorted = [...topCars].sort((a, b) => {
