...
export function filterCarsBySearch(cars, query) {
  ...
  return cars.filter(car => 
    ...
    car.years?.toLowerCase().includes(searchTerm)
  );
}
...
export function filterCarsByPrice(cars, minPrice, maxPrice, getPriceFn) {
  const getPrice = getPriceFn || (car => car.price_avg || car.priceAvg || 0);
...
export function filterCarsByDrivetrain(cars, drivetrain) {
  ...
  return cars.filter(car => car.drivetrain === drivetrain);
}
...
export function sortCars(cars, sortBy) {
  ...
  switch (sortBy) {
    ...
    case 'price-high':
      sorted.sort((a, b) => (b.price_avg || b.priceAvg || 0) - (a.price_avg || a.priceAvg || 0));
      break;
    case SORT_MODES.PRICE_LOW:
    case 'price-low':
    case 'price':
      sorted.sort((a, b) => (a.price_avg || a.priceAvg || 0) - (b.price_avg || b.priceAvg || 0));
      break;
    case SORT_MODES.ZERO_TO_SIXTY:
    case '0-60':
      sorted.sort((a, b) => (a.zero_to_sixty || a.zeroToSixty || 999) - (b.zero_to_sixty || b.zeroToSixty || 999));
      break;
