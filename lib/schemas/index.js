...
export const communityPostSchema = z.object({
  ...
  description: z.string().max(5000, 'Description too long').optional().nullable().transform(val => val?.trim()),
...
export const communityPostUpdateSchema = z.object({
  ...
  description: z.string().max(5000).optional().nullable().transform(val => val?.trim()),
});
...
export const dynoResultSchema = z.object({
  ...
  dynoType: z.enum(['inertia', 'load-bearing', 'hub', 'other']).optional().nullable(),
  dynoDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be YYYY-MM-DD').optional().nullable(),
...
export const trackTimeSchema = z.object({
  ...
  lapTimeSeconds: z.number().min(20, 'Lap time must be at least 20 seconds').max(1200, 'Lap time cannot exceed 20 minutes'),
  sessionDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be YYYY-MM-DD').optional().nullable(),
  sessionType: z.enum(['track_day', 'time_attack', 'race', 'practice', 'other']).optional().nullable(),
  conditions: z.enum(['dry', 'wet', 'damp', 'mixed']).optional().nullable(),
  ambientTempF: z.number().min(-50).max(150).optional().nullable(),
  ...
  driverSkillLevel: z.enum(['beginner', 'novice', 'intermediate', 'advanced', 'professional']).optional().nullable(),
  notes: z.string().max(2000).optional().nullable(),
