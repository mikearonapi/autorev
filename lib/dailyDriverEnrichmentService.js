...
export async function getExistingEnrichment(year, make, model) {
  ...
  const ENRICHMENT_COLS = 'id, lookup_key, year, make, model, maintenance_specs, service_intervals, known_issues, image_url, status, error_message, created_at, updated_at';
...
export async function enrichDailyDriver(vehicle, userId) {
  ...
  const db = getSupabase();
  const _lookupKey = generateLookupKey(year, make, model);
