...
async function searchExistingContent(gap) {
  ...
  if (car_slug) {
    // Resolve car_id from slug per Cardinal Rule 1
    const carId = await resolveCarId(car_slug);
    if (!carId) {
      console.warn('[GapResolver] Could not resolve car_id for slug:', car_slug);
      // Continue without car-specific data checks
    } else {
      // Check car_issues
...
export async function processGap(gap) {
  const { id, question_pattern: _question_pattern, car_slug: _car_slug, gap_type, occurrence_count } = gap;
...
export async function notifyContentGaps() {
  ...
  const { stats, highPriorityGaps, byCarSlug: _byCarSlug } = report;
...
export async function getGapsForReview(options = {}) {
  ...
  if (carSlug) {
    ...
    if (carId) {
      ...
      // Fallback to slug if car not found in cars table
...
export default alContentGapResolver;
