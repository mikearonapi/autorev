 ...
 * 
 * Fetches educational content from Supabase with fallback to local data.
 * This ensures the app works both with and without database connectivity.
 * 
 * Data Sources:
 * - Primary: Supabase upgrade_education table
 * - Fallback: Local upgradeEducation.js
 */
...
/**
 * Fetch upgrade education content from Supabase
 * Falls back to local data if database is unavailable
 */
export async function getUpgradeEducation() {
  if (!isSupabaseConfigured || !supabase) {
    console.log('[Education] Using local upgrade education data');
    return { data: getAllUpgradesGrouped(), source: 'local' };
  }
  ...
  try {
    ...
    if (error) {
      console.warn('[Education] Supabase error, falling back to local:', error.message);
      return { data: getAllUpgradesGrouped(), source: 'local' };
    }

    if (!data || data.length === 0) {
      console.log('[Education] No data in database, using local fallback');
      return { data: getAllUpgradesGrouped(), source: 'local' };
    }

    // Transform database data to match local structure
    const grouped = transformDatabaseData(data);
    return { data: grouped, source: 'database' };
  } catch (err) {
    console.error('[Education] Failed to fetch from Supabase:', err);
    return { data: getAllUpgradesGrouped(), source: 'local' };
  }
}

/**
 * Transform database rows to match the grouped structure from local data
 */
function transformDatabaseData(rows) {
  const grouped = {};
  
  // Initialize categories
  Object.keys(upgradeCategories).forEach(key => {
    grouped[key] = {
      ...upgradeCategories[key],
      upgrades: [],
    };
  });

  // Map database category names to local category keys
  const categoryMap = {
    'power-engine': 'power',
    'exhaust-sound': 'exhaust',
    'electronics-tuning': 'power',
  ...
  rows.forEach(row => {
    const localCategory = categoryMap[row.category] || row.category;
    if (grouped[localCategory]) {
      grouped[localCategory].upgrades.push({
        key: row.key,
        name: row.name,
        shortDescription: row.short_description,
        fullDescription: row.full_description,
        howItWorks: row.how_it_works,
        expectedGains: row.expected_gains || {},
        cost: {
          range: row.cost_range,
          low: row.cost_low,
          high: row.cost_high,
        },
        ...
        worksWellWith: row.works_well_with || [],
 ...
 * Checks database first, falls back to local
 */
export async function getUpgradeByKey(key) {
  // Try local first for speed
  if (upgradeDetails[key]) {
    return { data: upgradeDetails[key], source: 'local' };
  }
  ...
  try {
    ...
    if (error || !data) {
      return { data: upgradeDetails[key] || null, source: 'local' };
    }

    return { data: transformSingleUpgrade(data), source: 'database' };
  } catch (err) {
    return { data: upgradeDetails[key] || null, source: 'local' };
  }
}

function transformSingleUpgrade(row) {
  return {
    key: row.key,
    name: row.name,
    shortDescription: row.short_description,
