...
export async function decodeVIN(vin) {
  ...
  try {
    const response = await fetch(`${NHTSA_API_BASE}/DecodeVin/${cleanedVin}?format=json`);
...
function normalizeDriveType(driveType) {
  ...
  if (dt.includes('all') || dt.includes('awd') || dt.includes('4wd') || dt.includes('4x4'))
    return 'AWD';
...
export async function getVINBasicInfo(vin) {
  ...
  try {
    const response = await fetch(`${NHTSA_API_BASE}/DecodeVinValues/${cleanedVin}?format=json`);
...
export function matchVINToCar(decodedVin, cars) {
  ...
  const matches = cars.filter((car) => {
    ...
    const _carMake = carName.split(' ')[0]; // First word is usually the make
    ...
    // Check if year matches - handle new schema (year integer) or old (years string)
    if (year) {
      if (car.year) {
        // New schema: single year integer
        if (year !== car.year) {
          return false;
        }
      } else if (car.yearStart || car.yearEnd) {
        // New schema: year range
