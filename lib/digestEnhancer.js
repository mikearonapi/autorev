...
function detectAlerts(stats, historicalAvg = {}) {
  ...
  // High error rate alert (based on unique errors, not total occurrences)
  if (stats.autoErrors > 5) {
    const totalMsg = stats.totalAutoErrors > stats.autoErrors 
      ? ` (${stats.totalAutoErrors} total occurrences)` 
      : '';
    alerts.push({
      severity: 'critical',
      message: `${stats.autoErrors} unique auto-errors today${totalMsg} (investigate immediately)`,
    });
    ...
    alerts.push({
      severity: 'warning',
      message: `${stats.autoErrors} unique auto-errors today${totalMsg}`,
    });
  }
...
function generateActionItems(stats, alerts) {
  ...
  if (stats.autoErrors > 0) {
    const totalMsg = stats.totalAutoErrors > stats.autoErrors 
      ? ` (${stats.totalAutoErrors} occurrences)` 
      : '';
    actions.push(`Investigate ${stats.autoErrors} unique auto-errors${totalMsg} in #errors channel`);
  }
