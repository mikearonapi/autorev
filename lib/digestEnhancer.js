...
function detectAlerts(stats, historicalAvg = {}) {
  ...
  if (stats.autoErrors > 10) {
    alerts.push({
      severity: 'critical',
      message: `${stats.autoErrors} auto-errors today (investigate immediately)`,
    });
  }
...
function generateActionItems(stats, alerts) {
  ...
  if (stats.autoErrors > 5) {
    actions.push(`Investigate ${stats.autoErrors} auto-errors (check #errors channel)`);
  }
