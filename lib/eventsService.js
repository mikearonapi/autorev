...
import { geocodeLocation, calculateDistanceMiles } from './geocodingService.js';
 ...
 * @param {string} [params.car_id] - Filter by specific car affinity (preferred)
 * @param {string} [params.car_slug] - Filter by specific car affinity (deprecated, kept for backward compatibility)
...
export async function searchEvents(params = {}) {
  ...
  try {
    ...
    if (directLat && directLng && !isNaN(directLat) && !isNaN(directLng) && radius) {
      ...
      console.info('[eventsService] Using direct coordinates:', {
        lat: directLat,
        lng: directLng,
    ...
    // For popularity sort, first get event IDs that have RSVPs
    // This ensures we fetch events with RSVPs regardless of their date
    let popularEventIds = null;
    if (sort === 'popularity') {
      const today = new Date().toISOString().split('T')[0];

      // Get event IDs that have RSVPs and are upcoming/approved
      ...
      if (!rsvpError && rsvpData) {
        // Filter to upcoming events and get unique event IDs
        const upcomingEventIds = new Set();
        for (const rsvp of rsvpData) {
          const event = rsvp.events;
          if (!event) continue;

          // Check if event is upcoming (same logic as main query)
          const isUpcoming =
            (event.end_date && event.end_date >= today) ||
            (!event.end_date && event.start_date >= today);

          if (isUpcoming) {
            upcomingEventIds.add(rsvp.event_id);
          }
        }

        popularEventIds = Array.from(upcomingEventIds);
        console.info('[eventsService] Found events with RSVPs:', popularEventIds.length);

        // If no events have RSVPs, return empty result early
    ...
    // For popularity sort, filter to only events that have RSVPs
    if (sort === 'popularity' && popularEventIds && popularEventIds.length > 0) {
      query = query.in('id', popularEventIds);
    }
    ...
    // Skip pagination for popularity sort - we'll apply it after sorting by RSVP count
    if (sort !== 'popularity') {
    ...
    // If brand or car_id/car_slug filter, we need to filter by affinities
    let filteredEvents = events || [];

    if (brand || car_id || car_slug) {
      ...
      if (eventIds.length > 0) {
        ...
        // Prefer car_id, fall back to resolving car_slug for backward compatibility
        let effectiveCarId = car_id || null;
        if (!effectiveCarId && car_slug) {
          ...
          if (carData) {
            effectiveCarId = carData.id;
          }
        }

        if (effectiveCarId) {
          affinityQuery = affinityQuery.eq('car_id', effectiveCarId);
        }
    ...
    if (searchCenter && effectiveRadius) {
      ...
      if (filteredEvents.length === 0 && beforeRadiusCount > 0) {
        console.warn(
          '[eventsService] Radius search returned 0 results. Possible geocoding mismatch.',
          {
    ...
    if (eventIds.length > 0) {
      ...
      if (!rsvpError && rsvpCounts) {
        ...
        for (const rsvp of rsvpCounts) {
          if (!rsvpCountsMap[rsvp.event_id]) {
            rsvpCountsMap[rsvp.event_id] = { going: 0, interested: 0, total: 0 };
          }
          if (rsvp.status === 'going') {
            rsvpCountsMap[rsvp.event_id].going++;
          } else if (rsvp.status === 'interested') {
            rsvpCountsMap[rsvp.event_id].interested++;
          }
          rsvpCountsMap[rsvp.event_id].total++;
        }
      }
    }

    // Transform events to response shape
    let transformedEvents = filteredEvents.map((event) => {
      const rsvpCounts = rsvpCountsMap[event.id] || { going: 0, interested: 0, total: 0 };
    ...
    // Apply popularity sorting if requested (sort by total RSVP count descending)
    // Also filter to only events with at least 1 RSVP and apply pagination
    if (sort === 'popularity') {
      // Filter to only events with RSVPs (should already be filtered, but double-check)
      transformedEvents = transformedEvents.filter((event) => (event.rsvp_counts?.total || 0) > 0);

      // Sort by total RSVPs descending, then by date ascending for ties
      transformedEvents.sort((a, b) => {
        const totalA = a.rsvp_counts?.total || 0;
        const totalB = b.rsvp_counts?.total || 0;
        if (totalB !== totalA) {
      ...
      // Apply pagination after sorting (we skipped it earlier for popularity sort)
      const totalBeforePagination = transformedEvents.length;
      transformedEvents = transformedEvents.slice(
        effectiveOffset,
        effectiveOffset + effectiveLimit
      );
      console.info(
        '[eventsService] Popularity sort: total with RSVPs:',
