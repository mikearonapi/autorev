...
export async function searchEvents(params = {}) {
  ...
  try {
    ...
    if (sort === 'popularity') {
      ...
      if (!rsvpError && rsvpData) {
        ...
        for (const rsvp of rsvpData) {
          ...
          const isUpcoming = (event.end_date && event.end_date >= today) ||
    ...
    if (sort === 'popularity') {
      ...
      transformedEvents = transformedEvents.slice(effectiveOffset, effectiveOffset + effectiveLimit);
      console.info('[eventsService] Popularity sort: total with RSVPs:', totalBeforePagination, 'after pagination:', transformedEvents.length);
    }
