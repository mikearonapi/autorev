 ...
 *
 * NOTE: Tuning profiles are linked to v1 (legacy) car records. For v2 (year-specific) cars,
 * we resolve to the parent_car_id to find the content.
 */

import { resolveContentCarId, resolveContentCarIdFromId } from './carResolver';
...
export async function getTuningProfile(carSlug, options = {}) {
  ...
  // Resolve to content car ID (v1 parent for v2 cars)
  // Tuning profiles are linked to v1 cars
  let contentCarId = null;

  if (carId) {
    // If carId provided, resolve to content car
    const resolved = await resolveContentCarIdFromId(carId, { client });
    contentCarId = resolved?.contentCarId;
  } else if (carSlug) {
    // If slug provided, resolve to content car
    const resolved = await resolveContentCarId(carSlug, { client });
    contentCarId = resolved?.contentCarId;
  }

  if (!contentCarId) return null;

  const TUNING_COLS =
    'id, car_id, car_variant_id, tuning_focus, platform_strengths, platform_weaknesses, upgrades_by_objective, baseline_metrics, compatibility_notes, created_at, updated_at';

  // Query for profile using content car ID
  let query = client
 ...
 */
export async function getTuningProfileByCarId(
  carId,
  variantId = null,
...
export async function hasTuningProfile(carId) {
  ...
  const { data: _data, count } = await client
    .from('car_tuning_profiles')
...
export function getStageProgressions(profile) {
  ...
  return profile.stage_progressions.map((stage, index) => ({
    ...
    avgHpGain:
      stage.hpGainLow && stage.hpGainHigh
        ? Math.round((stage.hpGainLow + stage.hpGainHigh) / 2)
        : null,
    // Compute average cost
    avgCost:
      stage.costLow && stage.costHigh ? Math.round((stage.costLow + stage.costHigh) / 2) : null,
    // Compute total HP at this stage
    totalHp: profile.stock_whp && stage.hpGainHigh ? profile.stock_whp + stage.hpGainHigh : null,
...
export function getProfileSummary(profile) {
  ...
  return {
    ...
    maxTotalHp:
      profile.stock_whp && maxStage?.hpGainHigh ? profile.stock_whp + maxStage.hpGainHigh : null,
