...
import { getEffectiveTier, IS_BETA } from './tierAccess';
...
export async function getUserBalance(userId) {
  ...
  try {
    const { data: usage, error } = await client
      ...
      .select(
        'id, user_id, subscription_tier, current_credits, purchased_credits, credits_used_this_month, messages_this_month, last_refill_date, created_at, updated_at, balance_cents, purchased_cents, spent_cents_this_month, input_tokens_this_month, output_tokens_this_month, is_unlimited'
      )
      .eq('user_id', userId)
      .single();
    ...
    const percentage = isUnlimited
      ? 100
      : calculateTankPercentage(usage.balance_cents, usage.subscription_tier);
 ...
 * @param {string} [usage.carId] - Car ID (UUID) being discussed (for analytics)
...
export async function deductUsage(
  ...
  try {
    ...
    const { data: current, error: fetchError } = await client
      .from('al_user_credits')
      .select(
        'balance_cents, spent_cents_this_month, input_tokens_this_month, output_tokens_this_month, messages_this_month, is_unlimited'
      )
      .eq('user_id', userId)
      .single();
    ...
    const { error: updateError } = await client
      ...
      .update({
        ...
        credits_used_this_month: Math.round(
          (parseFloat(current.spent_cents_this_month) || 0) + costCents
        ),
      })
      .eq('user_id', userId);
...
export async function deductCredits(
  userId,
  { toolCalls = [], responseTokens = 0, hasContext = false }
) {
...
export async function updateSubscriptionTier(userId, newTier) {
  ...
  try {
    ...
    const isUpgrade =
      ['free', 'collector', 'tuner'].indexOf(newTier) >
 ...
 * @param {number} usage.inputTokens - Number of input tokens used
 * @param {number} usage.outputTokens - Number of output tokens used
 * @param {number} usage.costCents - Cost in cents (0 for unlimited users)
 * @param {string[]} usage.toolCalls - Array of tool names called
 * @param {string} [usage.carId] - Car ID (UUID) being discussed (for analytics)
 * @param {string} [usage.pageContext] - Page context (for analytics)
 * @param {string} [usage.promptVersionId] - Prompt version ID (for A/B testing)
 */
...
export async function getUsageHistory(userId, limit = 50) {
  ...
  const LOG_COLS =
    'id, user_id, conversation_id, message_id, credits_used, model, prompt_tokens, completion_tokens, response_time_ms, car_id, tool_calls, created_at';
...
export async function getUsageStats(userId) {
  ...
  try {
    ...
    const totalInputTokens =
      monthStats?.reduce((sum, log) => sum + (log.input_tokens || 0), 0) || 0;
    const totalOutputTokens =
      monthStats?.reduce((sum, log) => sum + (log.output_tokens || 0), 0) || 0;
    const totalCostCents =
      monthStats?.reduce((sum, log) => sum + (parseFloat(log.cost_cents) || 0), 0) || 0;
    ...
    return {
      thisMonth: {
        ...
        avgCostPerMessage:
          totalMessages > 0 ? formatCentsAsDollars(totalCostCents / totalMessages) : '$0.00',
      },
...
export async function needsMonthlyRefill(userId) {
  ...
  try {
    ...
    return (
      lastRefill.getMonth() !== now.getMonth() || lastRefill.getFullYear() !== now.getFullYear()
    );
  } catch (err) {
...
// IS_BETA imported at top from tierAccess for consistency
