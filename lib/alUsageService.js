 ...
 */
export async function deductUsage(userId, { inputTokens = 0, outputTokens = 0, toolCalls = [], carSlug = null, pageContext = null }) {
  if (!isSupabaseConfigured || !userId) {
  ...
  try {
    ...
    if (current.is_unlimited) {
      ...
      await logUsage(userId, { inputTokens, outputTokens, costCents: 0, toolCalls, carSlug, pageContext });
    ...
    await logUsage(userId, { inputTokens, outputTokens, costCents, toolCalls, carSlug, pageContext });
 ...
 * where cost_cents = 0
 */
async function logUsage(userId, { inputTokens, outputTokens, costCents, toolCalls, carSlug = null, pageContext = null }) {
