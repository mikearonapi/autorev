...
export async function getUserBalance(userId) {
  ...
  try {
    const { data: usage, error } = await client
      ...
      .select('id, user_id, subscription_tier, current_credits, purchased_credits, credits_used_this_month, messages_this_month, last_refill_date, created_at, updated_at, balance_cents, purchased_cents, spent_cents_this_month, input_tokens_this_month, output_tokens_this_month')
      .eq('user_id', userId)
      .single();
    ...
    const percentage = calculateTankPercentage(usage.balance_cents, usage.subscription_tier);

    return {
      balanceCents: usage.balance_cents || 0,
      plan: usage.subscription_tier,
      planName: plan.name,
      monthlyAllocationCents: plan.allocation.monthlyCents,
 ...
 * @param {string} userId - User ID
 * @param {string} planId - Plan ID (default: 'free')
 * @returns {Object} Initial balance info
 */
export async function initializeUserBalance(userId, planId = 'free') {
  ...
  try {
    ...
    const percentage = calculateTankPercentage(initialCents, planId);
 ...
 */
export async function deductUsage(userId, { inputTokens = 0, outputTokens = 0, toolCalls = [] }) {
  if (!isSupabaseConfigured || !userId) {
  ...
  try {
    ...
    const { data: current, error: fetchError } = await client
      .from('al_user_credits')
      .select('balance_cents, spent_cents_this_month, input_tokens_this_month, output_tokens_this_month, messages_this_month')
      .eq('user_id', userId)
      .single();
    ...
    await logUsage(userId, { inputTokens, outputTokens, costCents, toolCalls });
 ...
 */
async function logUsage(userId, { inputTokens, outputTokens, costCents, toolCalls }) {
