#!/usr/bin/env node
/**
 * Seed Expansion Events
 * 
 * Adds comprehensive expansion events: car clubs, concours, track days, import shows
 * 
 * Run: node scripts/events/seeders/seed-expansion.js
 */
...
async function main() {
  console.log('🚀 Seeding Expansion Events\n');
  
  const dataPath = join(__dirname, '..', 'data', 'expansion-2026-events.json');
  ...
  for (const event of events) {
    try {
      ...
      const payload = {
        ...
        event_type_id,
        region,
        country: event.state === 'BC' ? 'Canada' : 'USA',
      };
