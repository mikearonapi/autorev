...
/**
 * Car Detail Client Component
 * 
 * Handles all interactive features for the car detail page.
 * Receives pre-fetched car data from the server component.
 * Uses React Query for enriched data (efficiency, safety, etc.)
 * 
 * @module app/browse-cars/[slug]/CarDetailClient
 */
...
export default function CarDetailClient({ car: rawCar }) {
  ...
  // Popular parts using React Query
  const { 
    data: popularParts = [], 
    isLoading: popularPartsLoading, 
    error: popularPartsError 
  } = useCarPopularParts(car?.slug);
  
  // Car selection integration
  const { selectedCar, selectCar, isHydrated } = useCarSelection();
  const isThisCarSelected = isHydrated && selectedCar?.slug === car?.slug;
  ...
  useEffect(() => {
    if (car) {
      const heroUrl = getCarHeroImage(car);
      if (heroUrl) {
      ...
      <div className={styles.tabContent}>
        ...
        {activeTab === 'buying' && (
          <div className={styles.tabPanel}>
            ...
            {/* Best Years */}
            {Array.isArray(car.bestYearsDetailed) && car.bestYearsDetailed.length > 0 && (
              <section className={styles.contentSection}>
                <div className={styles.sectionHeader}>
                  <Icons.check size={22} />
            ...
            {/* Years to Avoid */}
            {Array.isArray(car.yearsToAvoidDetailed) && car.yearsToAvoidDetailed.length > 0 && (
              <section className={styles.contentSection}>
                <div className={styles.sectionHeader}>
                  <Icons.x size={22} />
            ...
            {/* Must-Have Options */}
            {Array.isArray(car.mustHaveOptions) && car.mustHaveOptions.length > 0 && (
              <section className={styles.contentSection}>
                <div className={styles.sectionHeader}>
                  <Icons.star size={22} />
        ...
        {activeTab === 'ownership' && (
          <div className={styles.tabPanel}>
            ...
            <section className={styles.contentSection}>
              ...
              {popularPartsError && <div className={styles.partsHintMuted}>{popularPartsError.message || 'Failed to load popular parts'}</div>}
