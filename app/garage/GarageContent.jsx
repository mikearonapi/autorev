...
// Accepts initialData from SSR for instant content rendering
function GarageContent({ initialData }) {
  ...
  // 
  // SSR OPTIMIZATION: If we have initialData from server, skip loading state entirely
  // The data is already available and providers will use it via getPrefetchedData()
  const hasSSRData = initialData?.user && !initialData?.hadError;
  const isDataLoading = !hasSSRData && (authLoading || (isAuthenticated && (!isDataFetchReady || favoritesLoading || buildsLoading || vehiclesLoading)));
...
// Export GarageContent as default for use by GarageClient
