...
function generateBuildJsonLd(post, primaryImage, shareUrl, totalModsCount, carData) {
  ...
  const articleSchema = {
    ...
    keywords: post.car_name
      ? `${post.car_name}, car build, modifications, tuning`
      : 'car build, modifications',
...
export default async function BuildDetailPage({ params }) {
  ...
  // Sort by display_order to respect user's custom ordering
  const galleryImages = (post.images || []).slice().sort((a, b) => {
      ...
      <div className={styles.page} data-no-main-offset>
        ...
        {heroImageUrl && (
          <div className={styles.heroImage}>
            <Image src={heroImageUrl} alt={post.title} fill priority className={styles.mainImage} />
            <div className={styles.heroOverlay} />
            {!hasUserHero && <span className={styles.stockImageBadge}>Stock Photo</span>}
        ...
        <div className={styles.content}>
          ...
          <header className={styles.header}>
            ...
            {post.car_name && (
              <Link href={`/browse-cars/${post.car_slug}`} className={styles.carLink}>
                {post.car_name}
              </Link>
            )}
                ...
                <div className={styles.authorDetails}>
                  ...
                  {post.author?.location_city && (
                    <span className={styles.location}>
                      📍 {post.author.location_city}
                      {post.author.location_state && `, ${post.author.location_state}`}
                    </span>
                  )}
                </div>
              </div>
          ...
          {carData && (
            <section className={styles.section}>
              <ExperienceScores carData={carData} buildData={post.buildData} />
            </section>
          )}
                    ...
                    (computedPerformance?.hpGain ?? post.buildData.total_hp_gain) > 0 && (
                      <div className={styles.summaryCard}>
                        <span className={styles.summaryValue}>
                          $
                          {Math.round(
                            (post.buildData.total_cost_low || post.buildData.total_cost_high || 0) /
                              (computedPerformance?.hpGain ?? post.buildData.total_hp_gain)
                          ).toLocaleString()}
                        </span>
          ...
          {(totalModsCount > 0 || (post.parts && post.parts.length > 0)) && (
            <section className={styles.section}>
              <BuildModsList parts={post.parts} buildData={post.buildData} />
            </section>
          )}
          ...
          <section className={styles.shareSection}>
            ...
            <ShareButtons shareUrl={shareUrl} title={post.title} carName={post.car_name} />
          </section>
