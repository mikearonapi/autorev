...
import { TrackPageSkeleton } from '@/components/ui/Skeleton';
...
function TrackPageContent() {
  ...
  const trackTimesWithPB = useMemo(() => {
    ...
    Object.entries(byTrack).forEach(([track, times]) => {
      const sorted = [...times].sort((a, b) => new Date(a.session_date) - new Date(b.session_date));
  ...
  const handleDeleteTrackTime = useCallback(
    async (trackTimeId) => {
  ...
  const handleSaveTrackTime = useCallback(
    async (trackData) => {
      // Throw descriptive errors instead of failing silently
      if (!user?.id) {
        throw new Error('Please sign in to log track times.');
      }
      if (!selectedVehicle) {
      ...
      try {
        ...
        console.log('[TrackPage] Saving track time:', { userId: user.id, payload });
      ...
      {selectedVehicle && !buildsLoading && (
        <div className={styles.estimateExplainer}>
          ...
          <span className={styles.estimateExplainerText}>
            Lap time estimates are <strong>approximations</strong>—actual times depend on driver
            skill, conditions, and car setup.
          </span>
        </div>
      )}
            ...
            <>
              ...
              <div className={styles.dataCard}>
                <LapTimeEstimator
                  ...
                  estimatedHp={estimatedHp}
                  weight={
                    selectedVehicle.matchedCar?.curb_weight ||
              ...
              <div className={styles.logDataSection}>
                <div className={styles.logDataHeader}>
                  ...
                  <p className={styles.logDataSubtitle}>
                    Log your actual track times for deeper insights and to help us improve
                      ...
                      <div className={styles.trackProgressBanner}>
                        ...
                        <div className={styles.trackProgressText}>
                          ...
                          <span className={styles.trackProgressLabel}>
                            improvement across {trackTimesSummary.uniqueTracks} track
                            {trackTimesSummary.uniqueTracks !== 1 ? 's' : ''}
                          </span>
                        </div>
                      </div>
                    )}
                          ...
                          <div className={styles.trackTimeMeta}>
                            <span className={styles.trackTimeDate}>
                              {new Date(time.session_date).toLocaleDateString('en-US', {
                                month: 'short',
                                day: 'numeric',
                                year: time.session_date?.startsWith(
