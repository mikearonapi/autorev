...
export default function CommunityBuildsPage() {
    ...
    async function fetchLikeStatus() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setLikedItems((prev) =>
            new Map(prev).set(currentBuild.id, {
              liked: data.liked,
  ...
  const handleTouchEnd = useCallback(
    (e) => {
      ...
      if (
        ...
        (Math.abs(deltaX) > 50 || Math.abs(deltaX) / deltaTime > 0.5)
      ) {
        ...
        !showDetails &&
        !showComments &&
        (Math.abs(deltaY) > 50 || Math.abs(deltaY) / deltaTime > 0.5)
      ) {
  ...
  const handleCommentAdded = useCallback(() => {
    ...
    const queryKey = communityKeys.buildsList({
      limit: 20,
      sort: 'algorithm',
      seed: sessionSeed,
      infinite: true,
      ...
      {currentBuild && !isLoading && (
        <>
          <div className={styles.feedContent}>
            ...
            >
                ...
                <div className={styles.headerText}>
                  <div className={styles.userLine}>
                    <span className={styles.username}>
                      {currentBuild.author?.display_name || 'Anonymous'}
                    </span>
                    {currentBuild.author?.selected_title &&
                      TITLES[currentBuild.author.selected_title] && (
                        <span
              ...
              {(currentBuild.computedPerformance?.upgraded?.hp ||
                ...
                <div className={styles.statsRow}>
                      ...
                      <span className={styles.statLabel}>
                        ...
                        <DataSourceIndicator
                          source={
                            ...
                            (currentBuild.computedPerformance?.isUserProvided ? 'measured' : null)
                          }
                        />
                      </span>
                    </div>
                  ...
                  {(currentBuild.computedPerformance?.upgraded?.zeroToSixty ||
                    currentBuild.car_specs?.zero_to_sixty) && (
                    <div className={styles.stat}>
                      <span className={styles.statValue}>
                        {(
            ...
            <div
              className={`${styles.imageCard} ${aspectRatio === '3:2' ? styles.imageCardWide : ''}`}
            >
            ...
            <div className={styles.actionsRow}>
              <button
                ...
                aria-label={
                  getLikeData(currentBuild.id).liked ? 'Unlike this build' : 'Like this build'
                }
