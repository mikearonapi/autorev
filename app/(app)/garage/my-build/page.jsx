...
function MyBuildContent() {
  ...
  const {
    selectedFitment,
    selectFitment,
    clearSelection: clearFitmentSelection,
  ...
  // Get user's hero image for this car (available for future use)
  ...
  const handleBuildWizardComplete = useCallback(
    (result) => {
      if (result?.car) {
  ...
  const handleBuildSummaryUpdate = useCallback(
    (summary) => {
      ...
      {buildSummary.upgradeCount > 0 && (
        <div className={styles.continueCtaContainer}>
          <Link
            href={
              currentBuildId
                ? `/garage/my-parts?build=${currentBuildId}`
                : `/garage/my-parts?car=${selectedCar.slug}`
            }
