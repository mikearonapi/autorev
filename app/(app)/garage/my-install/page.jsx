...
function MyInstallContent() {
  ...
  // TODO: This is currently unused but may be needed for future install tracking features
  const _normalizedUpgradeKeys = useMemo(() => {
  ...
  const buildParts = useMemo(() => {
    ...
    return upgradeKeys
      .map((upgrade, index) => {
        ...
        const displayName =
          typeof upgrade === 'object' && upgrade?.name
            ? upgrade.name
  ...
  if (!currentBuild || buildParts.length === 0) {
    return (
      <div className={styles.page}>
        <MyGarageSubNav onBack={handleBack} carSlug={selectedCar.slug} />
        <GarageVehicleSelector selectedCarSlug={selectedCar.slug} />
              ...
              <div className={styles.partsList}>
                ...
                {buildParts.map((part) => (
                  <InstallChecklistItem
                    ...
                    onToggleVideos={() =>
                      setShowVideosFor(showVideosFor === part.id ? null : part.id)
                    }
                    showTools={showToolsFor === part.id}
                    onToggleTools={() => setShowToolsFor(showToolsFor === part.id ? null : part.id)}
                  />
                ))}
              </div>
            </div>
      ...
      {selectedCar && (
        <div className={styles.continueCtaContainer}>
          <Link
            href={
              currentBuildId
                ? `/garage/my-photos?build=${currentBuildId}`
                : `/garage/my-photos?car=${selectedCar.slug}`
            }
