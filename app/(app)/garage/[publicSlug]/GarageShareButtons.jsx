...
/**
 * Garage Share Buttons Component
 * 
 * Client component for social sharing of public garages.
 * Supports Instagram, Facebook, Twitter/X, and native Web Share API.
 */
...
export default function GarageShareButtons({ shareUrl, displayName }) {
    ...
    if (navigator.share) {
      try {
        await navigator.share(getNativeShareData({
          title: `${displayName}'s Garage on AutoRev`,
          text: `Check out ${displayName}'s garage on AutoRev!`,
          url: shareUrl,
        }));
      } catch (err) {
  ...
  return (
    <>
      <div className={styles.shareButtons}>
        ...
        <a 
          href={getTwitterShareUrl(shareUrl, `Check out ${displayName}'s garage on AutoRev!`)}
          target="_blank"
          rel="noopener noreferrer"
          className={styles.shareBtn}
        >
      ...
      {copySuccess && (
        <p className={styles.copySuccessText}>
          Link copied! Share it on Instagram or anywhere else.
        </p>
      )}
    </>
  );
