...
async function handleGet(request, { params }) {
  ...
  const transformedVehicles = (vehicles || []).map((v) => {
    ...
    return {
      ...
      model: v.model,
      trim: v.trim,
      matchedCarId: v.matched_car_id, // car_id is the single source of truth
      matchedCarSlug: v.cars?.slug || null, // Get slug from JOIN
      nickname: v.nickname,
      color: v.color,
      mileage: v.mileage,
  ...
  const avgScore =
    transformedVehicles.length > 0 ? Math.round(totalScore / transformedVehicles.length) : 0;
