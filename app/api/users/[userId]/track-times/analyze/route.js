...
async function handlePost(request, { params }) {
  try {
    ...
    const { carId, trackName } = body;

    const TRACK_TIME_COLS =
      'id, user_id, user_vehicle_id, car_id, track_name, track_config, session_date, lap_time_seconds, conditions, tire_compound, mods_summary, notes, created_at';
...
export const POST = withErrorLogging(handlePost, {
  route: 'users/track-times/analyze',
  feature: 'lap-times',
});
...
function generateTrackTimeAnalysis(trackTimes) {
  ...
  Object.entries(byTrack).forEach(([trackName, times]) => {
    ...
    const sortedTimes = times.sort((a, b) => new Date(a.session_date) - new Date(b.session_date));
  ...
  if (withEstimates.length > 0) {
    const avgDelta =
      withEstimates.reduce((sum, t) => sum + (t.lap_time_seconds - t.estimated_time_seconds), 0) /
    ...
    if (avgDelta > 5) {
      analysis.skillAssessment.estimatedVsActual = {
        avgDelta,
        message:
          'Your actual times are slower than estimates. This is normal - focus on seat time!',
        suggestedSkillLevel:
          avgDelta > 15 ? 'beginner' : avgDelta > 8 ? 'intermediate' : 'advanced',
      };
    } else if (avgDelta < -2) {
      analysis.skillAssessment.estimatedVsActual = {
        avgDelta,
...
function analyzeNotes(notes) {
  ...
  if ((allNotes.match(/brak/g) || []).length >= 2) {
    patterns.push({
      ...
      category: 'brakes',
      message:
        "You've mentioned braking issues multiple times. Consider brake upgrades (pads, fluid, or BBK) or focus on braking technique.",
    });
  }
  ...
  if ((allNotes.match(/understeer|push|tight/g) || []).length >= 2) {
    patterns.push({
      ...
      category: 'handling',
      message:
        'Understeer is a recurring theme. Try adjusting tire pressures (lower front, higher rear) or consider suspension upgrades.',
    });
  }
  ...
  if ((allNotes.match(/oversteer|loose|snap|slide/g) || []).length >= 2) {
    patterns.push({
      ...
      category: 'handling',
      message:
        'Oversteer/loose handling mentioned often. Check rear tire pressures or consider stiffer rear sway bar.',
    });
  }
  ...
  if ((allNotes.match(/tire|grip|traction/g) || []).length >= 2) {
    patterns.push({
      ...
      category: 'tires',
      message:
        'Tire/grip issues come up frequently. Upgrading to a grippier compound could make a significant difference.',
    });
  }
...
function generateRecommendations(analysis, _trackTimes) {
  ...
  if (analysis.summary.totalSessions < 3) {
    recommendations.push({
      ...
      message:
        'With only a few logged sessions, the best investment is more track time. Aim for at least 5-10 sessions before making major modifications.',
    });
  }
  ...
  if (analysis.skillAssessment.suggestedSkillLevel === 'beginner') {
    recommendations.push({
      ...
      message:
        'A half-day with a driving coach can improve your times more than any modification. Look for HPDE schools in your area.',
    });
  }
  ...
  // Progress plateau
  const plateauTracks = analysis.progress.filter(
    (p) => p.sessions >= 4 && Math.abs(p.latestTime - p.bestTime) < 1
  );
