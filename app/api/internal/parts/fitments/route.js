...
function computeFitmentQualityScore(row) {
  ...
  const hasSource =
    ...
    (Array.isArray(row?.parts?.source_urls) && row.parts.source_urls.length > 0);
  ...
  const score =
    (verified ? 0.6 : 0.0) + confidence * 0.3 + (hasSource ? 0.07 : 0.0) + (hasNotes ? 0.03 : 0.0);
  return Math.max(0, Math.min(1, Math.round(score * 100) / 100));
}

/**
 * GET /api/internal/parts/fitments?carId=&verified=&limit=
...
async function handleGet(request) {
  try {
    ...
    let q = supabase
      .from('part_fitments')
      .select(
        'id,car_id,car_variant_id,fitment_notes,requires_tune,install_difficulty,estimated_labor_hours,verified,verified_by,verified_at,confidence,source_url,created_at,updated_at, parts(id,name,brand_name,part_number,category,quality_tier,street_legal,source_urls)'
      )
...
async function handlePatch(request) {
  try {
    ...
    if (fitment_notes !== undefined)
      updates.fitment_notes = fitment_notes === null ? null : String(fitment_notes);
    if (source_url !== undefined)
      updates.source_url = source_url === null ? null : String(source_url);
    ...
    if (estimated_labor_hours !== undefined)
      updates.estimated_labor_hours =
        estimated_labor_hours === null ? null : Number(estimated_labor_hours);
    ...
    const { data, error } = await supabase
      ...
      .in('id', targetIds)
      .select(
        'id,verified,confidence,fitment_notes,source_url,requires_tune,install_difficulty,estimated_labor_hours,verified_by,verified_at,updated_at'
      );
