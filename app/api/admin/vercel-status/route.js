...
async function handleGet(request) {
  ...
  try {
    ...
    const [projectData, deploymentsData, domainsData, envVarsData] = await Promise.all([
      ...
      vercelFetch(`/v6/deployments?projectId=${VERCEL_PROJECT_ID}&limit=20`).catch((e) => ({
        error: e.message,
      })),
      // Get domains
    ...
    const cronJobs =
      projectData.crons?.definitions?.map((cron) => ({
        path: cron.path,
    ...
    const envVars =
      envVarsData.error || envVarsData.notFound
        ? []
        : (envVarsData.envs || []).map((env) => ({
            key: env.key,
    ...
    // Process domains
    const domains =
      domainsData.error || domainsData.notFound
        ? []
    ...
    const deployments = deploymentsData.error
      ? []
    ...
    const isBuilding = deployments.some(
      (d) => d.readyState === 'BUILDING' || d.readyState === 'QUEUED'
    );
    ...
    const stats = {
      ...
      building: deployments.filter((d) => d.readyState === 'BUILDING' || d.readyState === 'QUEUED')
        .length,
    ...
    const buildMetrics =
      buildsWithDuration.length > 0
        ? {
            averageBuildTime: Math.round(
              buildsWithDuration.reduce((acc, d) => acc + d.buildDurationSeconds, 0) /
                buildsWithDuration.length
            ),
