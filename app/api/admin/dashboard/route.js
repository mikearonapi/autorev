...
function estimateVariableCosts(stats) {
  ...
  return {
    anthropic: {
      name: 'Anthropic API',
      amount: Math.round(anthropicCost * 100) / 100,
      category: 'ai',
    },
...
function generateCumulativeData(records, dateField) {
  ...
  // Sort by date
  const sorted = [...records].sort((a, b) => new Date(a[dateField]) - new Date(b[dateField]));
...
async function handleGet(request) {
  ...
  try {
    ...
    const [
      ...
      supabase
        .from('al_credit_balances')
        .select(
          'id, user_id, balance, lifetime_earned, lifetime_spent, last_activity_at, created_at'
        ),
      ...
      supabase.from('user_activity').select('user_id').gte('created_at', weekAgo),
    ]);
    ...
    const wauPercent = totalUsers > 0 ? Math.round((weeklyActiveUsers / totalUsers) * 100) : 0;

    // Conversations per user
    const conversationsPerUser =
      alUsers > 0 ? Math.round((alConversations.length / alUsers) * 10) / 10 : 0;

    // AL engagement change
    const engagementChange =
      previousAlConversations.length > 0
        ? Math.round(
            ((alConversations.length - previousAlConversations.length) /
              previousAlConversations.length) *
              100
          )
        : alConversations.length > 0
          ? 100
          : 0;

    // Generate daily conversation data for sparkline
    const conversationsByDay = generateDailyData(
      alConversations,
      'created_at',
      dateRange.start,
      dateRange.end
    ...
    const cronStatus =
      hoursSinceLastCron < 24 ? 'healthy' : hoursSinceLastCron < 72 ? 'degraded' : 'down';
    ...
    const response = {
      ...
      content: {
        ...
        vehiclesWithImages: carsResult.data?.filter((c) => c.image_url).length || 0,
        ...
        pendingVideos:
          youtubeResult.data?.filter((v) => v.processing_status === 'pending').length || 0,
