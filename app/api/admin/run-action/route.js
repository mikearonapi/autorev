...
const ALLOWED_ACTIONS = {
    ...
    handler: async (supabase) => {
      ...
      const metrics = {
        ...
        upcoming_events: events.filter(
          (e) => e.status === 'approved' && new Date(e.start_date) > now
        ).length,
  ...
  'refresh-events': {
    ...
    handler: async (supabase) => {
      ...
      // Get event stats
      const { data: events, error } = await supabase.from('events').select('status, start_date');
      ...
      const stats = {
        ...
        upcoming:
          events?.filter((e) => e.status === 'approved' && new Date(e.start_date) > now).length ||
          0,
        past:
          events?.filter((e) => e.status === 'approved' && new Date(e.start_date) <= now).length ||
          0,
