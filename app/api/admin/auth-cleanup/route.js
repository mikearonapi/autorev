...
import { getAuthenticatedUserWithProfile } from '@/lib/supabaseServer';
...
async function handlePost(request) {
  try {
    ...
    const { data: _beforeStats, error: _beforeError } = await supabaseServiceRole
    ...
    const { data: _stats, error: _statsError } = await supabaseServiceRole
      .rpc('get_auth_cleanup_stats', {
        cutoff_date: cutoffDate.toISOString(),
      })
      .catch(() => ({ data: null, error: 'Function not available' }));

    // Alternative: Get session info we can access
    const { data: sessionStats, error: _sessionStatsError } = await supabaseServiceRole
      .from('auth.sessions')
