  ...
  try {
    ...
    const [
      ...
      supabase
        .from('al_usage_logs')
        .select(
          'input_tokens, output_tokens, cost_cents, estimated_cost_cents, created_at, purpose'
        )
        .gte('created_at', startDate.toISOString())
      ...
      supabase
        .from('service_rates')
        .select(
          'id, service_name, rate_per_unit, unit_type, is_current, effective_date, notes, created_at'
        )
        .eq('is_current', true),
    ...
    const actualOutputTokens = userUsageLogs.reduce(
      (sum, log) => sum + (log.output_tokens || 0),
      0
    );
    const actualCostCents = userUsageLogs.reduce(
      (sum, log) => sum + parseFloat(log.estimated_cost_cents || log.cost_cents || 0),
      0
    );

    // If we have actual data, use it; otherwise fall back to estimates
    const hasActualData = userUsageLogs.length > 0;
    const inputTokens = hasActualData
      ? actualInputTokens
      : totalMessages * TOKEN_ESTIMATES.AVG_INPUT_TOKENS_PER_MESSAGE;
    const outputTokens = hasActualData
      ? actualOutputTokens
      : totalMessages * TOKEN_ESTIMATES.AVG_OUTPUT_TOKENS_PER_MESSAGE;
    const totalTokens = inputTokens + outputTokens;

    // Cost calculation
    const inputRate =
      rateLookup.ANTHROPIC_API?.PER_1M_INPUT_TOKENS ||
      DEFAULT_RATES.ANTHROPIC_API.PER_1M_INPUT_TOKENS;
    const outputRate =
      rateLookup.ANTHROPIC_API?.PER_1M_OUTPUT_TOKENS ||
      DEFAULT_RATES.ANTHROPIC_API.PER_1M_OUTPUT_TOKENS;
    ...
    const adminOutputTokens = adminUsageLogs.reduce(
      (sum, log) => sum + (log.output_tokens || 0),
      0
    );
    const adminCostCents = adminUsageLogs.reduce(
      (sum, log) => sum + parseFloat(log.estimated_cost_cents || log.cost_cents || 0),
      0
    );
    ...
    const embeddingRate =
      rateLookup.OPENAI_API?.PER_1M_TOKENS || DEFAULT_RATES.OPENAI_API.PER_1M_TOKENS;
    ...
    const { data: _sizeData } = await supabase
      .rpc('pg_database_size', { dbname: 'postgres' })
      .single();
    ...
    const supabaseStorageCents =
      storageOverageGB *
      (rateLookup.SUPABASE?.PER_GB_STORAGE || DEFAULT_RATES.SUPABASE.PER_GB_STORAGE);
    ...
    const projectedMonthlyInputTokens =
      projectedMonthlyMessages * TOKEN_ESTIMATES.AVG_INPUT_TOKENS_PER_MESSAGE;
    const projectedMonthlyOutputTokens =
      projectedMonthlyMessages * TOKEN_ESTIMATES.AVG_OUTPUT_TOKENS_PER_MESSAGE;
    ...
    const response = {
      ...
      totals: {
        currentPeriod: {
          ...
          total:
            (anthropicTotalCents + openaiCostCents + supabaseStorageCents + adminCostCents) / 100,
        },
      },
