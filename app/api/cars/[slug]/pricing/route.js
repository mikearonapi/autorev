...
async function handleGet(request, { params }) {
  ...
  try {
    ...
    if (!source || source === 'hagerty') {
      fetchPromises.push(
        hagertyScraper
          .matchCarToHagertyValuation(car)
          .then((data) => {
            if (data && data.values) {
    ...
    if (prices.length > 0) {
      ...
      pricing.consensus = {
        ...
        comparedToListed: car.msrp
          ? {
              difference: consensusPrice - car.msrp,
              differencePercent: Math.round(((consensusPrice - car.msrp) / car.msrp) * 100),
            }
          : null,
      };
    }
