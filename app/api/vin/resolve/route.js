...
export async function POST(request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Resolver RPC failed' }, { status: 500 });
    }

    const best = Array.isArray(data) ? data[0] : data; // supabase-js sometimes returns array for set-returning functions
    ...
    console.error('[vin/resolve] Error:', err);
