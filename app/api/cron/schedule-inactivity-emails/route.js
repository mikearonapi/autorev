...
const INACTIVITY_30D = 30 * 24 * 60 * 60 * 1000; // 30 days in ms
...
export async function GET(request) {
  try {
    ...
    const thirtyDaysAgo = new Date(now.getTime() - INACTIVITY_30D);
    ...
    for (const user of users || []) {
      ...
      const { data: existingQueue } = await supabaseAdmin
        ...
        .select('id')
        .eq('user_id', user.id)
        .in('template_slug', ['inactivity-7d', 'inactivity-30d'])
        .eq('status', 'pending')
        .limit(1);
      ...
      if (inactivityTime >= INACTIVITY_30D) {
        templateSlug = 'inactivity-30d';
      ...
      const result = await queueEmail({
        ...
        priority: templateSlug === 'inactivity-30d' ? 1 : 0,
    ...
    console.log(`[Cron/Inactivity] Completed: ${queued7d} 7-day, ${queued30d} 30-day, ${skipped} skipped`);
