...
export default function ALQuickActionsPopup({ 
  ...
  const handleActionClick = useCallback((action) => {
    ...
    if (context.upgrades?.length > 0) {
      const upgradeNames = context.upgrades.map(getUpgradeName).filter(n => n !== 'Unknown').join(', ');
      if (upgradeNames) {
...
export const defaultPartsPageActions = [
  {
    ...
    prompt: 'I\'m just getting started with modifying my car. What are the best first modifications for performance gains? Consider cost-effectiveness and ease of installation.',
  },
  {
    label: 'Best bang for buck mods',
    prompt: 'What are the best bang-for-buck modifications for my car? Rank them by performance gained per dollar spent.',
  },
  {
    label: 'Stage 1 vs Stage 2 explained',
    prompt: 'Explain the difference between Stage 1 and Stage 2 modifications. What parts are typically included in each stage and what are the power gains?',
  },
  {
    label: 'Will mods void my warranty?',
    prompt: 'Which common modifications might void my factory warranty? What can I do to protect myself?',
  },
];
...
export const buildPageActions = [
  {
    ...
    prompt: 'Based on my car, recommend modifications that would best achieve my performance goals. Consider daily driveability, reliability, and budget.',
  },
  {
    label: 'Stage 1 vs Stage 2 comparison',
    prompt: 'Compare Stage 1 vs Stage 2 for my car. What are the power gains, costs, reliability impacts, and required supporting mods for each?',
  },
  {
    label: 'Best bang for buck mods',
    prompt: 'What are the best bang-for-buck modifications for my car? Rank them by HP gained per dollar spent.',
  },
  {
    label: 'Will this void my warranty?',
    prompt: 'Which of my planned modifications might void my factory warranty? What can I do to protect myself?',
  },
];
...
export const performancePageActions = [
  {
    ...
    prompt: 'How accurate are the performance estimates for my build? What factors could cause my actual results to differ?',
  },
  {
    label: 'What affects 0-60 the most?',
    prompt: 'What modifications have the biggest impact on 0-60 times for my car? What are the diminishing returns?',
  },
  {
    label: 'Improve track performance',
    prompt: 'What should I focus on to improve my lap times? Consider power, handling, braking, and weight.',
  },
];
