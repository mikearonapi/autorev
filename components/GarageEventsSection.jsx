...
export default function GarageEventsSection({ vehicles = [], favorites = [] }) {
  ...
  // Get car slugs from owned vehicles (for API calls)
  const carSlugs = useMemo(
    () => vehicles.map((v) => v.vehicle?.matchedCarSlug || v.matchedCarSlug).filter(Boolean),
    [vehicles]
  );
  ...
  const seeAllText = hasVehicles ? 'See all events for your cars →' : 'Browse all events →';
