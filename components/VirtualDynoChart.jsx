 ...
 * - ENHANCED: Shows data source badges when values are from actual dyno data
 *
...
const FORCED_INDUCTION_PROFILES = {
  ...
  na: {
    torquePeakRatio: 0.7, // Torque peaks at 70% of HP peak RPM
...
export default function VirtualDynoChart({
  ...
  hasUserDynoData = false, // Whether values come from user-provided dyno data
  ...
  const safeEstimatedHp =
    typeof estimatedHp === 'number' && !isNaN(estimatedHp) && estimatedHp > 0
      ...
      : safeStockHp;
  const safeStockTq =
    typeof stockTorque === 'number' && !isNaN(stockTorque) && stockTorque > 0
      ? stockTorque
      : safeStockHp * 0.85;
  const safeEstimatedTq =
    typeof estimatedTq === 'number' && !isNaN(estimatedTq) && estimatedTq > 0
      ? estimatedTq
      : safeEstimatedHp * 0.88;
  ...
  const stockFiCharacteristics =
    FORCED_INDUCTION_PROFILES[stockFiProfile] || FORCED_INDUCTION_PROFILES['na'];
  const modFiCharacteristics =
    FORCED_INDUCTION_PROFILES[modFiProfile] || FORCED_INDUCTION_PROFILES['na'];
  ...
  const generateCurve = useMemo(() => {
    return (peakPower, peakTq, fiChar) => {
      ...
      for (let rpm = startRpm; rpm <= endRpm; rpm += step) {
        ...
        let powerFactor =
          rpmRatio < 1 ? Math.pow(rpmRatio, 1.8) : 1 - Math.pow((rpmRatio - 1) * 2, 2) * 0.3;
  ...
  const maxValue = Math.max(safeEstimatedHp, safeEstimatedTq, safeStockHp, safeStockTq) * 1.15;
  ...
  const _alDisplayMessage = hasModifications
    ? `Explain my dyno curve (+${hpGain} HP)`
    : 'Explain my power curve';

  // Determine actual data source for display
  const effectiveSource = dataSource || (hasUserDynoData ? 'measured' : null);
  const isDynoData = effectiveSource === 'measured' || effectiveSource === 'verified';
    ...
    <div className={`${styles.virtualDyno} ${compact ? styles.compact : ''}`}>
      ...
      <div className={styles.dynoHeader}>
        <div className={styles.dynoTitleSection}>
          <div className={styles.dynoTitleRow}>
            <span className={`${styles.dynoTitle} text-display`}>
              {isDynoData ? 'Dyno Results' : 'Virtual Dyno'}
            </span>
            {effectiveSource && effectiveSource !== 'estimated' && (
              <DataSourceBadge source={effectiveSource} variant="compact" detail={dynoShop} />
            )}
          </div>
          <span className={styles.dynoSubtitle}>
            {isDynoData
              ? `Actual dyno data${dynoShop ? ` from ${dynoShop}` : ''}`
              : hasModifications
        ...
        {hasModifications && (
          <>
            <span className={styles.dynoLegendItem}>
              <span
                className={styles.dynoLegendLine}
                style={{ background: CHART_COLORS.hp, opacity: 0.4 }}
              />
      ...
      <div className={styles.dynoChart}>
        ...
        >
          ...
          <svg className={styles.dynoCurveSvg} viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            <path
              d={
                (hasModifications ? modCurve : stockCurve)
                  .map((p, i) => {
                    const x = ((p.rpm - 1000) / 8000) * 100;
                    const y = 100 - (p.hp / maxValue) * 100;
                    return `${i === 0 ? 'M' : 'L'} ${x} ${y}`;
          ...
          <svg className={styles.dynoCurveSvg} viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            <path
              d={(hasModifications ? modCurve : stockCurve)
                .map((p, i) => {
                  const x = ((p.rpm - 1000) / 8000) * 100;
                  const y = 100 - (p.tq / maxValue) * 100;
                  return `${i === 0 ? 'M' : 'L'} ${x} ${y}`;
              ...
              {hasModifications ? (
                <>
                  <div className={styles.dynoHoverRow}>
                    ...
                    <span className={styles.dynoHoverGain}>
                      +{hoverData.modHp - hoverData.stockHp}
                    </span>
                  </div>
                  <div className={styles.dynoHoverRow}>
                    ...
                    <span className={styles.dynoHoverGain}>
                      +{hoverData.modTq - hoverData.stockTq}
                    </span>
                  </div>
                </>
              ) : (
          ...
          {/* Hover vertical line */}
          {hoverData && <div className={styles.dynoHoverLine} style={{ left: hoverData.x }} />}
