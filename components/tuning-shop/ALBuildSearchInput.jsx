 ...
 */
function generateBuildPageActions(
  selectedUpgrades = [],
  carName = 'my car',
  goal = null,
  totalHpGain = 0
  ...
  if (upgradeCount > 0) {
    ...
    const upgradeKeys = selectedUpgrades
      .map((u) => {
    ...
    const hasExhaust = upgradeKeys.some(
      (k) => keyIncludes(k, 'exhaust') || keyIncludes(k, 'downpipe') || k === 'headers'
    );

    // Get upgrade names for prompts
    const upgradeNameList = selectedUpgrades
      .map(getUpgradeName)
      .filter((n) => n !== 'Unknown')
      .join(', ');
...
export default function ALBuildSearchInput({
  ...
  const _effectiveCarId = carId || car?.id; // Reserved for future AL context
  ...
  const handleSubmit = useCallback(
    (e) => {
      ...
      const upgradeNames = selectedUpgrades
        .map(getUpgradeName)
        .filter((n) => n !== 'Unknown')
        .join(', ');
      ...
      // Note: effectiveCarId available for future use in chat context
      openChatWithPrompt(
        fullPrompt,
        {
    ...
    [query, carName, effectiveCarSlug, selectedUpgrades, totalHpGain, goal, openChatWithPrompt]
  );
