        ...
        {categories.map((category) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
            >
              <div className={styles.listRowIcon}>{getIcon(category)}</div>
              <div className={styles.listRowContent}>
                <span className={styles.listRowName}>
                  {getName(category)}
                  {isRecommended && (
                    <span style={{ marginLeft: '6px', color: 'var(--color-accent-amber-light)' }}>
                      ★
                    </span>
                  )}
                </span>
                {count > 0 && (
                  <span className={styles.listRowSubtext}>{count} upgrades selected</span>
                )}
              </div>
              {count > 0 ? (
      ...
      {categories.map((category) => {
        ...
        return (
          ...
          >
            ...
            {count > 0 && <span className={styles.verticalBadge}>{count}</span>}
          </button>
        );
      })}
    </nav>
