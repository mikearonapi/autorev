 ...
 * @property {string} carId - Car UUID (PRIMARY identifier for car reference)
 * @property {string} carSlug - Car slug (for URL generation, obtained via JOIN)
 ...
 * @property {function(string): SavedBuild[]} getBuildsByCarId - Get builds for a car by carId
...
export function SavedBuildsProvider({ children }) {
  ...
  const fetchBuilds = useCallback(
    async (
      ...
      forceRefetch = false,
      currentBuildsCount = 0,
      timeout = 5000
    ) => {
      if (!isAuthenticated || !user?.id) {
        // Not authenticated - load from localStorage (clearing user data)
        console.log(
          '[SavedBuildsProvider] Not authenticated, clearing user data and loading from localStorage'
        );
      ...
      if (prefetchedBuilds) {
        ...
        const transformedBuilds = prefetchedBuilds.map((build) => {
          ...
          const heroImage =
            uploadedImages.find((img) => img.is_primary && img.media_type !== 'video') ||
            uploadedImages.find((img) => img.media_type !== 'video') ||
            null;

          return {
            id: build.id,
            carId: build.car_id, // PRIMARY identifier for car reference
            carSlug: build.car_slug || build.cars?.slug, // For URL generation (from JOIN)
            ...
            parts: Array.isArray(build.user_project_parts)
              ? build.user_project_parts.map((p) => ({
                  id: p.id,
                  partId: p.part_id,
                  quantity: p.quantity,
      ...
      try {
        ...
        if (data) {
          ...
          console.log(
            '[SavedBuildsProvider] Fetched',
            transformedBuilds.length,
            'builds from server'
          );
   ...
   */
  const canSaveBuilds = IS_BETA
    ? isAuthenticated
    : hasAccess(userTier, 'buildProjects', isAuthenticated);
    ...
    async (buildData) => {
      ...
      if (isAuthenticated && user?.id) {
        ...
        if (!error && data) {
          const newBuild = {
            id: data.id,
            carId: data.car_id, // PRIMARY identifier for car reference
            carSlug: data.car_slug || data.cars?.slug, // For URL generation
            ...
            factoryConfig:
              data.selected_upgrades?.factoryConfig || buildData?.factoryConfig || null,
            wheelFitment: data.selected_upgrades?.wheelFitment || buildData?.wheelFitment || null,
            sizeSelections:
              data.selected_upgrades?.sizeSelections || buildData?.sizeSelections || null,
            ...
            advancedSpecs:
              data.selected_upgrades?.advancedSpecs || buildData?.advancedSpecs || null,
      ...
      const newBuild = {
        ...
        carId: buildData.carId, // PRIMARY identifier for car reference
        carSlug: buildData.carSlug, // For URL generation
   ...
   */
  const updateBuildHandler = useCallback(
    async (buildId, updates) => {
      ...
      if (isAuthenticated && user?.id) {
        ...
        if (!error) {
          console.log('[SavedBuildsProvider] updateBuild success, updating local state:', {
            ...
            installedParts: updates?.selectedParts
                ...
                if (data) {
                  return {
                    ...
                    upgrades: Array.isArray(data.selected_upgrades)
                      ...
                      : data.selected_upgrades?.upgrades || [],
                    factoryConfig:
                      data.selected_upgrades?.factoryConfig ||
                      updates?.factoryConfig ||
                      build.factoryConfig,
                    wheelFitment:
                      data.selected_upgrades?.wheelFitment ||
                      updates?.wheelFitment ||
                      build.wheelFitment,
                    sizeSelections:
                      data.selected_upgrades?.sizeSelections ||
                      updates?.sizeSelections ||
                      build.sizeSelections,
                    heroSource:
                      data.selected_upgrades?.heroSource ||
                      updates?.heroSource ||
                      build.heroSource ||
                      'stock',
                    heroImageId:
                      data.selected_upgrades?.heroImageId ||
                      updates?.heroImageId ||
                      build.heroImageId ||
                      null,
   ...
   * Delete a build
   */
  const deleteBuildHandler = useCallback(
    async (buildId) => {
  ...
  /**
   * Get builds for a specific car by carId
   * @param {string} carId - UUID of the car
   */
  const getBuildsByCarId = useCallback(
    (carId) => {
    ...
    (buildId, buildData) => {
      ...
      autoSaveTimerRef.current = setTimeout(async () => {
        ...
        try {
          ...
          // This prevents duplicate builds when navigating to /garage/my-build?car=<carId>
          // NOTE: We use buildsRef.current to get the CURRENT state, not the potentially stale closure value
          if (!effectiveId && data.carId) {
            const existingBuild = buildsRef.current.find((b) => b.carId === data.carId);
            if (existingBuild) {
              console.log(
                '[SavedBuildsProvider] Found existing build for car, updating instead of creating new:',
                existingBuild.id
              );
          ...
          if (effectiveId) {
            ...
            setBuilds((prev) => {
              const existing = prev.find((b) => b.carId === data.carId);
              if (existing) {
                return prev.map((b) =>
                  b.id === existing.id ? { ...b, ...data, updatedAt: new Date().toISOString() } : b
                );
              }
 ...
 * @returns {SavedBuild[]}
 */
export function useCarBuilds(carId) {
  const { getBuildsByCarId } = useSavedBuilds();
  return getBuildsByCarId(carId);
}
