...
function generateBlurDataURL(seed = 'default') {
  ...
  const encoded = typeof window !== 'undefined' ? btoa(svg) : Buffer.from(svg).toString('base64');
...
export default function CarImage({
  ...
  // NOTE: Currently disabled in Image component to test iOS black bar issue
  ...
  useEffect(() => {
    ...
    const imageUrl =
      variant === 'garage' && !usingFallback ? getCarGarageImage(car) : getCarHeroImage(car);
    ...
    <div className={`${styles.container} ${variantClass} ${className}`}>
      ...
      {imageUrl && (
        <Image
          ...
          onLoad={handleLoad}
          priority={!lazy}
          // DEBUG: Disabled blur placeholder to test if it causes iOS black bar
          // placeholder="blur"
...
export function CarThumbnail({ car, className = '' }) {
  return (
    <CarImage car={car} variant="thumbnail" className={className} showName={false} lazy={true} />
  );
}
...
export function CarCardImage({ car, className = '' }) {
  return <CarImage car={car} variant="card" className={className} showName={true} lazy={true} />;
}
