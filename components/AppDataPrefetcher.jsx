...
export function AppDataPrefetcher() {
  ...
  useEffect(() => {
    ...
    // CRITICAL: All data prefetched during splash screen should be hydrated here
    ...
    // Hydrate vehicles - used by any component that queries vehicles via React Query
    const prefetchedVehicles = getPrefetchedData('vehicles', userId);
    if (prefetchedVehicles) {
      queryClient.setQueryData(userKeys.vehicles(userId), prefetchedVehicles);
      console.log('[AppDataPrefetcher] Hydrated vehicles:', prefetchedVehicles.length);
    }

    // Hydrate insights - prefetched during splash screen for instant Insights page
    const prefetchedInsights = getPrefetchedData('insights', userId);
    if (prefetchedInsights) {
      queryClient.setQueryData([...userKeys.all, userId, 'insights'], prefetchedInsights);
      console.log('[AppDataPrefetcher] Hydrated insights data');
    }

    // Secondary prefetch: If insights weren't prefetched during splash, fetch them now
    // This is a fallback for edge cases where splash prefetch failed/timed out
    if (!prefetchedInsights) {
      queryClient
        .prefetchQuery({
        ...
        .catch((err) => {
          console.warn('[AppDataPrefetcher] Insights fallback prefetch failed:', err);
        });
    }
