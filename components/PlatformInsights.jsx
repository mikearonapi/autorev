...
export default function PlatformInsights({
  ...
  // Combine community tips with robust error handling
  const communityTips = useMemo(() => {
    const tips = platformInsights?.community_tips || [];

    // Helper to safely extract text from various tip formats
    const extractTipText = (tip) => {
      if (typeof tip === 'string' && tip.trim()) return tip.trim();
      if (tip && typeof tip === 'object') {
        // Handle { text: "..." } format
        if (typeof tip.text === 'string' && tip.text.trim()) return tip.text.trim();
        // Handle { title: "..." } format (fallback)
        if (typeof tip.title === 'string' && tip.title.trim()) return tip.title.trim();
      }
      return null; // Invalid tip format
    };

    // Also include key_points from YouTube as tips (these should be strings)
    const ytKeyPoints = (youtubeData.keyPoints || [])
      .map((point) => extractTipText(point))
      .filter(Boolean)
      .map((text) => ({ text, source: 'youtube' }));

    // Process manual tips with error handling
    const manualTips = tips
      .map((tip) => extractTipText(tip))
      .filter(Boolean)
      .map((text) => ({ text, source: 'community' }));
